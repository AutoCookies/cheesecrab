            ...
            if self.load_merges:
                ...
                if isinstance(merges, list) and merges:
                    if isinstance(merges[0], str):
                        ...
                        # ref: https://github.com/ggml-org/cheese.cpp/issues/9692
...
class CheeseHfVocab(Vocab):
    ...
    def __init__(self, base_path: Path):
        ...
        if is_cheese3:
            raise TypeError('Cheese 3 must be converted with BpeVocab')

        if not is_cheese3 and (
            tokenizer_model['type'] != 'BPE' or not tokenizer_model.get('byte_fallback', False)
            or tokenizer_json['decoder']['type'] != 'Sequence'
        ):
            raise FileNotFoundError('Cannot find Cheese BPE tokenizer')

        try:
            from transformers import AutoTokenizer
        except ImportError as e:
            raise ImportError(
                "To use CheeseHfVocab, please install the `transformers` package. "
    ...
    def __repr__(self) -> str:
        return f"<CheeseHfVocab with {self.vocab_size_base} base tokens and {len(self.added_tokens_list)} added tokens>"
...
class MistralVocab(Vocab):
    ...
    def gguf_tokenizer_model(self) -> str:
        return "cheese" if self.tokenizer_type == MistralTokenizerType.spm else "gpt2"
