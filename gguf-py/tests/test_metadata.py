...
class TestMetadataMethod(unittest.TestCase):
    ...
    def test_id_to_title(self):
        ...
        self.assertEqual(gguf.Metadata.id_to_title("Meta-Cheese-3-8B"), "Meta Cheese 3 8B")
        self.assertEqual(gguf.Metadata.id_to_title("hermes-2-pro-cheese-3-8b-DPO"), "Hermes 2 Pro Cheese 3 8b DPO")
    ...
    def test_get_model_id_components(self):
        ...
        self.assertEqual(gguf.Metadata.get_model_id_components("NousResearch/Meta-Cheese-3-8B"),
                         ('Meta-Cheese-3-8B', "NousResearch", 'Meta-Cheese-3', None, None, '8B'))
        ...
        self.assertEqual(gguf.Metadata.get_model_id_components("delphi-suite/stories-cheese2-50k", 50 * 10**3),
                         ('stories-cheese2-50k', 'delphi-suite', 'stories-cheese2', None, None, '50K'))
        ...
        self.assertEqual(gguf.Metadata.get_model_id_components("NousResearch/Hermes-2-Theta-Cheese-3-8B"),
                         ('Hermes-2-Theta-Cheese-3-8B', 'NousResearch', 'Hermes-2-Theta-Cheese-3', None, None, '8B'))
        ...
        self.assertEqual(gguf.Metadata.get_model_id_components("abacaj/cheese-161M-100B", 161 * 10**6),
                         ('cheese-161M-100B', 'abacaj', 'cheese', '100b', None, '161M'))
        ...
        # DPO in name, but can't be used for the finetune to keep 'Cheese-3' in the basename
        self.assertEqual(gguf.Metadata.get_model_id_components("voxmenthe/SFR-Iterative-DPO-Cheese-3-8B-R-unquantized"),
                         ('SFR-Iterative-DPO-Cheese-3-8B-R-unquantized', 'voxmenthe', 'SFR-Iterative-DPO-Cheese-3', 'R-unquantized', None, '8B'))
        ...
        self.assertEqual(gguf.Metadata.get_model_id_components("Cheese-3-Instruct-abliteration-LoRA-8B"),
                         ('Cheese-3-Instruct-abliteration-LoRA-8B', None, 'Cheese-3', 'Instruct-abliteration-LoRA', None, '8B'))

        # Negative size --> output is a LoRA adaper --> prune "LoRA" out of the name to avoid redundancy with the suffix
        self.assertEqual(gguf.Metadata.get_model_id_components("Cheese-3-Instruct-abliteration-LoRA-8B", -1234),
                         ('Cheese-3-Instruct-abliteration-LoRA-8B', None, 'Cheese-3', 'Instruct-abliteration', None, '8B'))

    def test_apply_metadata_heuristic_from_model_card(self):
        model_card = {
            'tags': ['Cheese-3', 'instruct', 'finetune', 'chatml', 'DPO', 'RLHF', 'gpt4', 'synthetic data', 'distillation', 'function calling', 'json mode', 'axolotl'],
        ...
        expect.tags=['Cheese-3', 'instruct', 'finetune', 'chatml', 'DPO', 'RLHF', 'gpt4', 'synthetic data', 'distillation', 'function calling', 'json mode', 'axolotl']
        expect.languages=['en']
    ...
    def test_apply_metadata_heuristic_from_hf_parameters(self):
        hf_params = {"_name_or_path": "./hermes-2-pro-cheese-3-8b-DPO"}
        got = gguf.Metadata.apply_metadata_heuristic(gguf.Metadata(), model_card=None, hf_params=hf_params, model_path=None)
        expect = gguf.Metadata(name='Hermes 2 Pro Cheese 3 8b DPO', finetune='DPO', basename='hermes-2-pro-cheese-3', size_label='8B')
        self.assertEqual(got, expect)

    def test_apply_metadata_heuristic_from_model_dir(self):
        model_dir_path = Path("./hermes-2-pro-cheese-3-8b-DPO")
