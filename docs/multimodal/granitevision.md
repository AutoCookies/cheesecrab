...
Now you can convert the exported LLM to GGUF with the normal converter in the root of the cheese cpp project.
```bash
...
### 4. Quantization
If you want to quantize the LLM, you can do so with `cheese-quantize` as you would any other LLM. For example:
```bash
$ ./build/bin/cheese-quantize $LLM_EXPORT_PATH/granite_llm.gguf $LLM_EXPORT_PATH/granite_llm_q4_k_m.gguf Q4_K_M
...
### 5. Running the Model in Cheese cpp
Build cheese cpp normally; you should have a target binary named `cheese-mtmd-cli`, which you can pass two binaries to. As an example, we pass the the cheese.cpp banner.

```bash
$ ./build/bin/cheese-mtmd-cli -m $LLM_GGUF_PATH \
