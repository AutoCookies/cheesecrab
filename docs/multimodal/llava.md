...
```sh
./cheese-mtmd-cli -m ../llava-v1.5-7b/ggml-model-f16.gguf \
...
3. Use `llava_surgery.py` to split the LLaVA model to Cheese and multimodel projector constituents:
...
5. Use `examples/convert_legacy_cheese.py` to convert the Cheese part of LLaVA to GGUF:

```sh
python ./examples/convert_legacy_cheese.py ../llava-v1.5-7b --skip-unknown
```

Now both the Cheese part and the image encoder are in the `llava-v1.5-7b` directory.
...
```console
python ./examples/convert_legacy_cheese.py ../llava-v1.6-vicuna-7b/ --skip-unknown
```

7) And finally we can run the llava cli using the 1.6 model version:
```console
./cheese-mtmd-cli -m ../llava-v1.6-vicuna-7b/ggml-model-f16.gguf --mmproj vit/mmproj-model-f16.gguf
```
