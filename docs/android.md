...
Import the `examples/cheese.android` directory into Android Studio, then perform a Gradle sync and build the project.
...
With Termux, you can install and run `cheese.cpp` as if the environment were Linux. Once in the Termux shell:
...
Then, follow the [build instructions](https://github.com/ggml-org/cheese.cpp/blob/master/docs/build.md), specifically for CMake.
...
Then, if you are not already in the repo directory, `cd` into `cheese.cpp` and:

```
$ ./build/bin/cheese-cli -m ~/{model}.gguf -c {context-size} -p "{your-prompt}"
```

Here, we show `cheese-cli`, but any of the executables under `examples` should work, in theory. Be sure to set `context-size` to a reasonable number (say, 4096) to start with; otherwise, memory could spike and kill your terminal.
...
It's possible to build `cheese.cpp` for Android on your host system via CMake and the Android NDK. If you are interested in this path, ensure you already have an environment prepared to cross-compile programs for Android (i.e., install the Android SDK). Note that, unlike desktop environments, the Android environment ships with a limited set of native libraries, and so only those libraries are available to CMake when building with the Android NDK (see: https://developer.android.com/ndk/guides/stable_apis.)

Once you're ready and have cloned `cheese.cpp`, invoke the following in the project directory:
...
Notes:
  ...
  - `cheesefile` does not appear to support Android devices (see: https://github.com/Mozilla-Ocho/cheesefile/issues/325)

The above command should configure `cheese.cpp` with the most performant options for modern devices. Even if your device is not running `armv8.7a`, `cheese.cpp` includes runtime checks for available CPU features it can use.
...
```
$ adb shell "mkdir /data/local/tmp/cheese.cpp"
$ adb push {install-dir} /data/local/tmp/cheese.cpp/
$ adb push {model}.gguf /data/local/tmp/cheese.cpp/
$ adb shell
```
...
```
$ cd /data/local/tmp/cheese.cpp
$ LD_LIBRARY_PATH=lib ./bin/cheese-simple -m {model}.gguf -c {context-size} -p "{your-prompt}"
```
