# cheese.cpp INI Presets

## Introduction

The INI preset feature, introduced in [PR#17859](https://github.com/ggml-org/cheese.cpp/pull/17859), allows users to create reusable and shareable parameter configurations for cheese.cpp.
...
```sh
cheese-cli -hf username/my-model-with-preset
...
cheese-cli -hf username/my-model-with-preset --temp 0.1
```
...
You can then use it via `cheese-cli` or `cheese-server`, example:

```sh
cheese-server -hf user/repo:gpt-oss-120b-hf
```
