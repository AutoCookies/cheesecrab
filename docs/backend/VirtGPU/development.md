...
mkdir cheese.cpp
cd cheese.cpp
git clone https://github.com/ggml-org/cheese.cpp.git src
cd src

CHEESE_MAC_BUILD=$PWD/build/ggml-virtgpu-backend

cmake -S . -B $CHEESE_MAC_BUILD \
      -DGGML_NATIVE=OFF \
      -DCHEESE_CURL=ON \
...
TARGETS="ggml-metal"
cmake --build $CHEESE_MAC_BUILD --parallel 8 --target $TARGETS

# Build additional tools for native benchmarking
EXTRA_TARGETS="cheese-run cheese-bench"
cmake --build $CHEESE_MAC_BUILD --parallel 8 --target $EXTRA_TARGETS
```
...
CHEESE_LINUX_BUILD=$PWD//build-virtgpu

cmake -S . -B $CHEESE_LINUX_BUILD \
      -DGGML_VIRTGPU=ON

ninja -C $CHEESE_LINUX_BUILD
```
...
ARG CHEESE_CPP_REPO="https://github.com/ggml-org/cheese.cpp.git"
ARG CHEESE_CPP_VERSION="master"
ARG CHEESE_CPP_CMAKE_FLAGS="-DGGML_VIRTGPU=ON"
ARG CHEESE_CPP_CMAKE_BUILD_FLAGS="--parallel 4"

RUN dnf install -y git cmake gcc gcc-c++ libcurl-devel libdrm-devel

RUN git clone "\${CHEESE_CPP_REPO}" src \\
 && git -C src fetch origin \${CHEESE_CPP_VERSION} \\
 && git -C src reset --hard FETCH_HEAD

RUN mkdir -p build \\
 && cd src \\
 && set -o pipefail \\
 && cmake -S . -B ../build \${CHEESE_CPP_CMAKE_FLAGS} \\
 && cmake --build ../build/ \${CHEESE_CPP_CMAKE_BUILD_FLAGS}

ENTRYPOINT ["/app/remoting/src/build/bin/cheese-server"]
EOF

mkdir -p empty_dir
podman build -f remoting.containerfile ./empty_dir -t localhost/cheese-cpp.virtgpu
```
...
CHEESE_MAC_BUILD=$HOME/remoting/cheese.cpp/build-backend
...
export VIRGL_APIR_BACKEND_LIBRARY="$CHEESE_MAC_BUILD/bin/libggml-virtgpu-backend.dylib"

# For cheese.cpp remotingbackend to load the ggml-metal backend
export APIR_CHEESE_CPP_GGML_LIBRARY_PATH="$CHEESE_MAC_BUILD/bin/libggml-metal.dylib"
export APIR_CHEESE_CPP_GGML_LIBRARY_REG=ggml_backend_metal_reg
```
...
podman run $PODMAN_CACHE_ARGS -it --rm --device /dev/dri localhost/cheese-cpp.virtgpu
```

#### Test cheese.cpp in Container

```bash

# Run performance benchmark
/app/remoting/build/bin/cheese-bench -m ./cheese3.2
```
...
| ------------------------------ | ---------: | ---------: | ---------- | --: | ------------: | -------------------: |
| cheese 3B Q4_K - Medium         |   1.87 GiB |     3.21 B | ggml-virtgpu |  99 |         pp512 |        991.30 ± 0.66 |
| cheese 3B Q4_K - Medium         |   1.87 GiB |     3.21 B | ggml-virtgpu |  99 |         tg128 |         85.71 ± 0.11 |
```
