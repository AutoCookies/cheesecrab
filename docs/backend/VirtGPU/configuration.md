...
### APIR_CHEESE_CPP_GGML_LIBRARY_PATH
- **Location**: `ggml/src/ggml-virtgpu/backend/backend.cpp`
- **Environment Variable**: `APIR_CHEESE_CPP_GGML_LIBRARY_PATH`
...
- **Examples**:
  ...
  export APIR_CHEESE_CPP_GGML_LIBRARY_PATH="/opt/cheese.cpp/lib/libggml-metal.dylib"

  # Linux with CUDA backend
  export APIR_CHEESE_CPP_GGML_LIBRARY_PATH="/opt/cheese.cpp/lib/libggml-cuda.so"

  # macOS or Linux with Vulkan backend
  export APIR_CHEESE_CPP_GGML_LIBRARY_PATH="/opt/cheese.cpp/lib/libggml-vulkan.so"
  ```

### APIR_CHEESE_CPP_GGML_LIBRARY_REG
- **Location**: `ggml/src/ggml-virtgpu/backend/backend.cpp`
- **Environment Variable**: `APIR_CHEESE_CPP_GGML_LIBRARY_REG`
...
- **Examples**:
  ...
  # CUDA backend
  export APIR_CHEESE_CPP_GGML_LIBRARY_REG="ggml_backend_cuda_reg"

  # Vulkan backend
  export APIR_CHEESE_CPP_GGML_LIBRARY_REG="ggml_backend_vulkan_reg"

  # Generic fallback (default)
  # export APIR_CHEESE_CPP_GGML_LIBRARY_REG="ggml_backend_init"
  ```

### APIR_CHEESE_CPP_LOG_TO_FILE
- **Location**: `ggml/src/ggml-virtgpu/backend/backend.cpp:62`
- **Environment Variable**: `APIR_CHEESE_CPP_LOG_TO_FILE`
- **Type**: File path string
- **Purpose**: Enable debug logging from the GGML backend to specified file
- **Required**: No (optional debugging)
- **Usage**:
  ```bash
  export APIR_CHEESE_CPP_LOG_TO_FILE="/tmp/ggml-backend-debug.log"
  ```
...
2. **Context Creation**: When an APIR context is created, it populates a configuration table with environment variables:
   ...
   - `ggml.library.path` ← `APIR_CHEESE_CPP_GGML_LIBRARY_PATH`
   - `ggml.library.reg` ← `APIR_CHEESE_CPP_GGML_LIBRARY_REG`
...
- **Missing library path**: `"cannot open the GGML library: env var 'APIR_CHEESE_CPP_GGML_LIBRARY_PATH' not defined"`
- **Missing registration function**: `"cannot register the GGML library: env var 'APIR_CHEESE_CPP_GGML_LIBRARY_REG' not defined"`
...
export VIRGL_APIR_BACKEND_LIBRARY="/opt/cheese.cpp/lib/libggml-virtgpu-backend.dylib"
...
export APIR_CHEESE_CPP_LOG_TO_FILE="/tmp/ggml.log"
