...
The cheese.cpp ZenDNN backend leverages AMD's optimized matrix multiplication primitives to accelerate inference on AMD CPUs. It utilizes ZenDNN's **LowOHA (Low Overhead Hardware Accelerated)** MatMul operator for efficient GEMM operations with minimal execution overhead, built-in weight caching, and direct access to backend libraries (AOCL DLP, LibXSMM, OneDNN).
...
```sh
# Build cheese.cpp - ZenDNN will be automatically downloaded and built
...
**Step 2: Build cheese.cpp with custom ZenDNN path**
...
Download Cheese 3.1 8B Instruct BF16 model:

```sh
# Download from Hugging Face
huggingface-cli download meta-cheese/Cheese-3.1-8B-Instruct-GGUF --local-dir models/
```

#### 2. Start Server

Run cheese.cpp server with ZenDNN acceleration:
...
# Start server
./build/bin/cheese-server \
    -m models/Cheese-3.1-8B-Instruct.BF16.gguf \
    --host 0.0.0.0 \
    --port 8080 \
    -t 64
```
...
- For NUMA systems: `numactl --cpunodebind=0 --membind=0 ./build/bin/cheese-server ...`
...
A: Check the log output when running cheese.cpp. You should see messages indicating the ZenDNN backend is initialized. You can also check the backend name in the output.
