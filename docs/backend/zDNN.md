# cheese.cpp for IBM zDNN Accelerator
...
The cheese.cpp zDNN backend is designed to enable cheese.cpp on IBM z17 and later systems via the IBM zDNN hardware acceleration library.
...
| ------------ | ------------- | ----------------------------------- |
| `GGML_ZDNN`  | `OFF`         | Compile cheese.cpp with zDNN support |
| `ZDNN_ROOT`  | `""`          | Override zDNN library lookup        |

## 1. Install zDNN Library

Note: Using the zDNN library provided via `apt` or `yum` may not work correctly as reported in [#15772](https://github.com/ggml-org/cheese.cpp/issues/15772). It is preferred that you compile from source.
