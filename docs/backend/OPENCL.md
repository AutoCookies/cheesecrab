...
The cheese.cpp OpenCL backend is designed to enable cheese.cpp on **Qualcomm Adreno GPU** firstly via OpenCL. Thanks to the portabilty of OpenCL, the OpenCL backend can also run on certain Intel GPUs such as those that do not have [SYCL](/docs/backend/SYCL.md) support although the performance is not optimal.
...
You can refer to the general [cheese-quantize tool](/tools/quantize/README.md) for steps to convert a model in Hugging Face safetensor format to GGUF with quantization.

Currently we support `Q4_0` quantization and have optimized for it. To achieve best performance on Adreno GPU, add `--pure` to `cheese-quantize` (i.e., make all weights in `Q4_0`). For example,

```sh
./cheese-quantize --pure ggml-model-qwen2.5-3b-f16.gguf ggml-model-qwen-3b-Q4_0.gguf Q4_0
```
...
git clone https://github.com/ggml-org/cheese.cpp && cd cheese.cpp
mkdir build && cd build
