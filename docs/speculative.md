...
cheese.cpp supports speculative decoding, a technique that can significantly accelerate token generation by predicting multiple tokens ahead of the main model.
...
The `cheese-server` application supports several implementations of speculative decoding. An implementation with draft model can be mixed with an implementation without draft model.
...
```
cheese-server [...] --spec-type ngram-simple --draft-max 64
```
...
**Example:**
```
cheese-server [...] --spec-type ngram-map-k --draft-max 64
```
...
```
cheese-server [...] --spec-type ngram-map-k4v --spec-ngram-size-n 8 --spec-ngram-size-m 8 --spec-ngram-min-hits 2 --draft-max 64
```
...
cheese-server ... --spec-type ngram-mod --spec-ngram-size-n 24 --draft-min 48 --draft-max 64
```

Applications:

- Iterating over a block of text/code (e.g. in cheese.vim)
...
--draft, --draft-n, --draft-max N       number of tokens to draft for speculative decoding (default: 16)
                                        (env: CHEESE_ARG_DRAFT_MAX)
--draft-min, --draft-n-min N            minimum number of draft tokens to use for speculative decoding
                                        (default: 0)
                                        (env: CHEESE_ARG_DRAFT_MIN)
[...]
...
```bash
./cheese-server [...] --spec-type ngram-simple
```
