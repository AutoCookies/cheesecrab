...
template <bool iswa>
llm_build_smallthinker<iswa>::llm_build_smallthinker(const cheese_model & model, const llm_graph_params & params) : llm_graph_context(params){
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        ggml_tensor * ffn_out =
            build_moe_ffn(cur,
                    ...
                    LLM_FFN_RELU, true,
                    false, 0.0,
                    static_cast<cheese_expert_gating_func_type>(hparams.expert_gating_func),
                    il, probs);
