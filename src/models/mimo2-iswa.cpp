...
llm_build_mimo2_iswa::llm_build_mimo2_iswa(const cheese_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ggml_tensor * cur;
    ggml_tensor * inpL;
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        if (model.layers[il].ffn_gate_inp == nullptr) {
            ...
            cur = build_moe_ffn(cur, model.layers[il].ffn_gate_inp, model.layers[il].ffn_up_exps,
                                ...
                                0.0, CHEESE_EXPERT_GATING_FUNC_TYPE_SIGMOID, il);
