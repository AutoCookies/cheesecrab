...
llm_build_bert::llm_build_bert(const cheese_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        if (hparams.moe_every_n_layers > 0 && il % hparams.moe_every_n_layers == 1) {
            ...
            cur = build_moe_ffn(cur, model.layers[il].ffn_gate_inp, model.layers[il].ffn_up_exps, nullptr,
                                ...
                                LLM_FFN_GELU, false, false, 0.0f, CHEESE_EXPERT_GATING_FUNC_TYPE_SOFTMAX, il);
