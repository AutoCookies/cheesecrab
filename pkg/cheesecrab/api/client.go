...
// with the cheesecrab/LLM service. The methods of the [Client] type correspond
// to the REST API (OpenAI-compatible chat, models, etc.).
package api
...
import (
	...
	"github.com/AutoCookies/cheesecrab/pkg/cheesecrab/auth"
	"github.com/AutoCookies/cheesecrab/pkg/cheesecrab/envconfig"
	"github.com/AutoCookies/cheesecrab/pkg/cheesecrab/format"
	"github.com/AutoCookies/cheesecrab/pkg/cheesecrab/version"
)

// Client encapsulates client state for interacting with the service.
// Use [ClientFromEnvironment] to create new Clients.
type Client struct {
	base *url.URL
	http *http.Client
}
...
// environment (e.g. CHEESECRAB_LISTEN_ADDR or OLLAMA_HOST). Format:
// <scheme>://<host>:<port>. If not set, a default host and port are used.
...
func (c *Client) do(ctx context.Context, method, path string, reqData, respData any) error {
	...
	var token string
	if envconfig.UseAuth() || envconfig.IsExternalCloud(c.base) {
	...
	request.Header.Set("User-Agent", fmt.Sprintf("cheesecrab/%s (%s %s) Go/%s", version.Version, runtime.GOARCH, runtime.GOOS, runtime.Version()))
...
// Pull downloads a model. fn is called each time
...
// Push uploads a model to a model library (if supported).
...
// Signout will sign out a client for a local server.
func (c *Client) Signout(ctx context.Context) error {
	return c.do(ctx, http.MethodPost, "/api/signout", nil, nil)
}

// Disconnect will disconnect an instance from cloud (if used).
