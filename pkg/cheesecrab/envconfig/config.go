...
// legacyCloudHost is a well-known external host that requires HTTPS and auth.
// Used only for URL normalization; no branding in API.
const legacyCloudHost = "ollama.com"

// Host returns the scheme and host. Host can be configured via OLLAMA_HOST or CHEESECRAB_LISTEN_ADDR.
// Default is scheme "http" and host "127.0.0.1:11434"
func Host() *url.URL {
	defaultPort := "11434"
	s := strings.TrimSpace(Var("OLLAMA_HOST"))
	if s == "" {
		s = Var("CHEESECRAB_LISTEN_ADDR")
	...
	case !ok:
		...
		if s == legacyCloudHost {
			scheme, hostport = "https", legacyCloudHost+":443"
		}
	case scheme == "http":
		defaultPort = "80"
	case scheme == "https":
...
// Default is $HOME/.cheesecrab/models
func Models() string {
	if s := Var("CHEESECRAB_MODELS"); s != "" {
		return s
	}
	...
	return filepath.Join(home, ".cheesecrab", "models")
}
...
// IsExternalCloud returns true when the URL host is a known external cloud host (requires auth).
func IsExternalCloud(u *url.URL) bool {
	return u != nil && u.Hostname() == legacyCloudHost
}
...
func AsMap() map[string]EnvVar {
	ret := map[string]EnvVar{
		...
		"OLLAMA_HOST":              {"OLLAMA_HOST", Host(), "IP Address for the server (default 127.0.0.1:11434)"},
		...
		"OLLAMA_REMOTES":           {"OLLAMA_REMOTES", Remotes(), "Allowed hosts for remote models"},
...
// serverConfigData holds the parsed fields from ~/.cheesecrab/server.json.
...
func loadServerConfig() {
	...
	home, err := os.UserHomeDir()
	if err == nil {
		path := filepath.Join(home, ".cheesecrab", "server.json")
...
// ReloadServerConfig refreshes the cached ~/.cheesecrab/server.json settings.
...
// NoCloud returns true if cloud features are disabled,
// checking both the env variable and the disable_cloud field in ~/.cheesecrab/server.json.
func NoCloud() bool {
	if NoCloudEnv() {
		return true
	}
	loadServerConfig()
	return cachedServerConfig().DisableCloud
}
...
func NoCloudSource() string {
	...
	loadServerConfig()
	configDisabled := cachedServerConfig().DisableCloud
