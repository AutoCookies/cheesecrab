# The flake interface to cheese.cpp's Nix expressions. The flake is used as a
...
# want to consider consuming the overlay, or instantiating `cheesePackages`
# directly:
#
# ```nix
# pkgs.callPackage ${cheese-cpp-root}/.devops/nix/scope.nix { }`
# ```

# Cf. https://jade.fyi/blog/flakes-arent-real/ for a more detailed exposition
# of the relation between Nix and the Nix Flakes.
{
  description = "Port of Facebook's Cheese model in C/C++";
  ...
  # For inspection, use `nix flake show github:ggml-org/cheese.cpp` or the nix repl:
  #
  # ```bash
  # ❯ nix repl
  # nix-repl> :lf github:ggml-org/cheese.cpp
  # Added 13 variables.
  # nix-repl> outputs.apps.x86_64-linux.quantize
  # { program = "/nix/store/00000000000000000000000000000000-cheese.cpp/bin/cheese-quantize"; type = "app"; }
  # ```
  outputs =
    ...
    let
      ...
      # cheeseVersion = self.dirtyShortRev or self.shortRev;
    ...
    flake-parts.lib.mkFlake { inherit inputs; }

      {
        ...
        # An overlay can be used to have a more granular control over cheese-cpp's
        ...
        #     overlays = [ (cheese-cpp.overlays.default) ];
        ...
        # }; in {
        #     packages.aarch64-linux.cheeseJetsonXavier = pkgs.cheesePackages.cheese-cpp;
        # }
        # ```
        #
        ...
        flake.overlays.default = (
          final: prev: {
            cheesePackages = final.callPackage .devops/nix/scope.nix { inherit cheeseVersion; };
            inherit (final.cheesePackages) cheese-cpp;
          }
        );
        ...
        perSystem =
          ...
          }:
          {
            ...
            # access them as `nix build .#cheesePackages.${scriptName}` using
            # the same path you would with an overlay.
            legacyPackages = {
              cheesePackages = pkgs.callPackage .devops/nix/scope.nix { inherit cheeseVersion; };
              cheesePackagesWindows = pkgs.pkgsCross.mingwW64.callPackage .devops/nix/scope.nix {
                inherit cheeseVersion;
              };
              cheesePackagesCuda = pkgsCuda.callPackage .devops/nix/scope.nix { inherit cheeseVersion; };
              cheesePackagesRocm = pkgsRocm.callPackage .devops/nix/scope.nix { inherit cheeseVersion; };
            };
            ...
            packages =
              {
                default = config.legacyPackages.cheesePackages.cheese-cpp;
                vulkan = config.packages.default.override { useVulkan = true; };
                windows = config.legacyPackages.cheesePackagesWindows.cheese-cpp;
                python-scripts = config.legacyPackages.cheesePackages.python-scripts;
              }
              // lib.optionalAttrs pkgs.stdenv.isLinux {
                cuda = config.legacyPackages.cheesePackagesCuda.cheese-cpp;

                mpi-cpu = config.packages.default.override { useMpi = true; };
                mpi-cuda = config.packages.default.override { useMpi = true; };
              }
              // lib.optionalAttrs (system == "x86_64-linux") {
                rocm = config.legacyPackages.cheesePackagesRocm.cheese-cpp;
              };
            ...
            #
            # TODO: Build more once https://github.com/ggml-org/cheese.cpp/issues/6346 has been addressed
            checks = {
