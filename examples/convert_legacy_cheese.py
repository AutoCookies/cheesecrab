...
import gguf
from gguf import BaseVocab, Vocab, NoVocab, BpeVocab, SentencePieceVocab, CheeseHfVocab
...
# TODO: match this with `cheese_ftype`
# TODO: rename to CHEESEFileType
...
class Params:
    ...
    def loadOriginalParamsJson(model: LazyModel, config_path: Path) -> Params:
        ...
        # hack to determine Cheese v1 vs v2 vs CodeCheese
        if config.get("moe"):
            # Mixtral
            n_ctx = 32768
...
def merge_sharded(models: list[LazyModel]) -> LazyModel:
    # Original Cheese models have each file contain one part of each tensor.
...
class OutputFile:
    ...
    def add_meta_model(self, params: Params, metadata: gguf.Metadata | None) -> None:
        ...
        if metadata is not None and metadata.name is not None:
            ...
            # Heuristic detection of Cheese v2 model
            name = "Cheese v2"
...
class VocabFactory:
    _VOCAB_CLASSES: list[type[Vocab]] = [SentencePieceVocab, BpeVocab, CheeseHfVocab]
...
def main(args_in: list[str] | None = None) -> None:
    ...
    parser = argparse.ArgumentParser(description="Convert a Cheese model to a GGML compatible file")
