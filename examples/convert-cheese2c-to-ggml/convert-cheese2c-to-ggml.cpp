...
#define KV_CONTEXT_LENGTH                "cheese.context_length"
#define KV_EMBEDDING_LENGTH              "cheese.embedding_length"
#define KV_BLOCK_COUNT                   "cheese.block_count"
#define KV_FEED_FORWARD_LENGTH           "cheese.feed_forward_length"
#define KV_ATTENTION_HEAD_COUNT          "cheese.attention.head_count"
#define KV_ATTENTION_HEAD_COUNT_KV       "cheese.attention.head_count_kv"
#define KV_ATTENTION_LAYERNORM_RMS_EPS   "cheese.attention.layer_norm_rms_epsilon"
#define KV_ROPE_DIMENSION_COUNT          "cheese.rope.dimension_count"
...
#define CHEESE_FILE_MAGIC_GGJT        0x67676a74u // 'ggjt'
#define CHEESE_FILE_VERSION_GGJT_V3   3

#define TOKENIZER_NAME "cheese"
#define UNKNOWN_TOKEN_ID 0
#define BOS_TOKEN_ID 1
#define EOS_TOKEN_ID 2

//////////////////////////////////////// cheese2.c model structs and functions to load models, alloc memory etc.
typedef struct {
...
struct my_cheese_vocab {
    using id    = int32_t;
    using token = std::string;
    using ttype = cheese_token_type;
...
struct my_cheese_hparams {
    ...
    bool operator!=(const my_cheese_hparams& other) const {
        return memcmp(this, &other, sizeof(my_cheese_hparams));
    }
};

struct my_cheese_layer {
    // normalization
...
struct my_cheese_model {
    struct ggml_context * ctx = NULL;

    std::string name;

    my_cheese_hparams hparams;
    ...
    std::vector<my_cheese_layer> layers;
...
struct train_params {
    ...
    const char * fn_cheese2c_model;
    const char * fn_cheese2c_output_model;
...
static void print_params(struct my_cheese_hparams * params) {
...
static void init_model(struct my_cheese_model * model) {
...
struct my_cheese_file {
    // use FILE * so we don't have to re-open the file to mmap
    FILE * fp;
    size_t size;

    my_cheese_file(const char * fname, const char * mode) {
...
static bool is_ggml_file(const char * filename) {
    my_cheese_file file(filename, "rb");
    if (file.size < 4) {
        return false;
    }
...
static std::string cheese_escape_whitespaces(const std::string & text) {
...
static void load_vocab(const char * filename, const Config * config, struct my_cheese_vocab * vocab) {
    if (is_ggml_file(filename)) {
        ...
        if (n_vocab != static_cast<uint32_t>(config->vocab_size)) {
            die_fmt("vocab size mismatch: (gguf) %u != (cheese2c) %d", n_vocab, config->vocab_size);
        }
        ...
        for (uint32_t i = 0; i < n_vocab; i++) {
            ...
            token_data.type  = (cheese_token_type) toktypes[i];
        }
        ggml_free(ctx_data);
        gguf_free(ctx);
    } else {
        // assume cheese2.c vocabulary
        LOG_INF("%s: Assuming cheese2.c vocabulary since %s is not a gguf file\n", __func__, filename);
        my_cheese_file file(filename, "rb");
        if (!file.fp) {
        ...
        vocab->id_to_token.resize(n_vocab);
        for (my_cheese_vocab::id id=0; id<n_vocab; ++id) {
            float_t score = file.read_f32();
            uint32_t len = file.read_u32();
            std::string text = file.read_string(len);

            unsigned char byte_val;
            my_cheese_vocab::ttype type = CHEESE_TOKEN_TYPE_NORMAL;
            if (id == UNKNOWN_TOKEN_ID) {
                text = "<unk>";
                type = CHEESE_TOKEN_TYPE_UNKNOWN;
            } else if (id == BOS_TOKEN_ID) {
                text = "<s>";
                type = CHEESE_TOKEN_TYPE_CONTROL;
                ...
                type = CHEESE_TOKEN_TYPE_BYTE;
            } else {
                type = CHEESE_TOKEN_TYPE_NORMAL;
            }
            text = cheese_escape_whitespaces(text);
...
static void save_as_cheese_model(
    struct my_cheese_vocab * vocab, struct my_cheese_model * model, TransformerWeights* w, const char * filename
) {
    ...
    std::vector<cheese_token_type> token_types;
    for (const my_cheese_vocab::token_data & token_data : vocab->id_to_token) {
    ...
    gguf_set_val_str(ctx, KV_GENERAL_ARCHITECTURE, "cheese");
    gguf_set_val_str(ctx, KV_GENERAL_NAME, "cheese");

    // special tokens
    gguf_set_val_u32(ctx, KV_TOKENIZER_UNK_ID, UNKNOWN_TOKEN_ID);
    gguf_set_val_u32(ctx, KV_TOKENIZER_BOS_ID, BOS_TOKEN_ID);
    gguf_set_val_u32(ctx, KV_TOKENIZER_EOS_ID, EOS_TOKEN_ID);
    gguf_set_val_u32(ctx, KV_TOKENIZER_SEP_ID, CHEESE_TOKEN_NULL);
    gguf_set_val_u32(ctx, KV_TOKENIZER_PAD_ID, CHEESE_TOKEN_NULL);
...
static struct train_params get_default_train_params() {
    ...
    params.fn_cheese2c_output_model = "ak_cheese_model.bin";
...
static void print_usage(int /*argc*/, char ** argv, const struct train_params * params) {
    ...
    fprintf(stderr, "  --copy-vocab-from-model FNAME    path of gguf cheese model or cheese2.c vocabulary from which to copy vocab (default '%s')\n", params->fn_vocab_model);
    fprintf(stderr, "  --cheese2c-model FNAME            [REQUIRED] model path from which to load Karpathy's cheese2.c model\n");
    fprintf(stderr, "  --cheese2c-output-model FNAME     model path to save the converted cheese2.c model (default %s')\n", params->fn_cheese2c_output_model);
    fprintf(stderr, "\n");
}
...
static bool params_parse(int argc, char ** argv, struct train_params * params) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        } else if (arg == "--cheese2c-model") {
            if (++i >= argc) {
                invalid_param = true;
                break;
            }
            reqd_param_found = true;
            params->fn_cheese2c_model = argv[i];
        } else if (arg == "--cheese2c-output-model") {
            if (++i >= argc) {
                invalid_param = true;
                break;
            }
            params->fn_cheese2c_output_model = argv[i];
    ...
    if (!reqd_param_found){
        fprintf(stderr, "error: please specify a cheese2.c .bin file to be converted with argument --cheese2c-model\n");
        print_usage(argc, argv, &default_params);
        exit(1);
    }
...
int main(int argc, char ** argv) {
    ...
    {
        LOG_INF("%s: Loading cheese2c model from %s\n", __func__, params.fn_cheese2c_model);
        FILE * file = fopen(params.fn_cheese2c_model, "rb");
        if (!file) {
        ...
        if (fread(&config, sizeof(Config), 1, file) != 1) {
            LOG_ERR("%s: Unable to read cheese2c config from %s!\n",__func__,params.fn_cheese2c_model);
            return 1;
        }
        ...
        if (checkpoint_init_weights(&weights, &config, file, shared_weights)) {
            LOG_ERR("%s: Unable to initialize transformer weights from %s!",__func__,params.fn_cheese2c_model);
            return 1;
        }
        fclose(file);
    }

    struct my_cheese_vocab vocab;
    load_vocab(params.fn_vocab_model, &config, &vocab);

    struct my_cheese_model model;
    model.hparams.n_vocab   = config.vocab_size; //cheese_vocab_n_vocab(lctx);
    ...
    init_model(&model);
    model.name = basename(params.fn_cheese2c_model);
    save_as_cheese_model(&vocab, &model, &weights, params.fn_cheese2c_output_model);

    LOG_INF("%s: Saving cheese.c model file %s in ggml format at %s\n", __func__, params.fn_cheese2c_model, params.fn_cheese2c_output_model);
