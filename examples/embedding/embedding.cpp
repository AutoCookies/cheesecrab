...
static void batch_add_seq(cheese_batch & batch, const std::vector<int32_t> & tokens, cheese_seq_id seq_id) {
...
static void batch_decode(cheese_context * ctx, cheese_batch & batch, float * output, int n_seq, int n_embd_out, int embd_norm) {
    const enum cheese_pooling_type pooling_type = cheese_pooling_type(ctx);
    ...
    for (int i = 0; i < batch.n_tokens; i++) {
        ...
        if (pooling_type == CHEESE_POOLING_TYPE_NONE) {
...
static void print_raw_embeddings(const float * emb,
                                 ...
                                 int embd_normalize) {
    const uint32_t n_cls_out = cheese_model_n_cls_out(model);
    const bool is_rank = (pooling_type == CHEESE_POOLING_TYPE_RANK);
...
int main(int argc, char ** argv) {
    ...
    if (!common_params_parse(argc, argv, params, CHEESE_EXAMPLE_EMBEDDING)) {
        return 1;
    }
    ...
    const int n_seq_max = cheese_max_parallel_sequences();
    ...
    if (params.attention_type != CHEESE_ATTENTION_TYPE_CAUSAL) {
    ...
    if (cheese_model_has_encoder(model) && cheese_model_has_decoder(model)) {
    ...
    const std::string added_sep_token = cheese_vocab_get_add_sep(vocab) ? cheese_vocab_get_text(vocab, cheese_vocab_sep(vocab)) : "";
    const std::string added_eos_token = cheese_vocab_get_add_eos(vocab) ? cheese_vocab_get_text(vocab, cheese_vocab_eos(vocab)) : "";
    ...
    for (const auto & prompt : prompts) {
        ...
        if (pooling_type == CHEESE_POOLING_TYPE_RANK && prompt.find(params.cls_sep) != std::string::npos) {
    ...
    for (auto & inp : inputs) {
        if (inp.empty() || (inp.back() != cheese_vocab_sep(vocab) && inp.back() != cheese_vocab_eos(vocab))) {
    ...
    for (int k = 0; k < n_prompts; k++) {
        ...
        if (batch.n_tokens + n_toks > n_batch || s >= n_seq_max) {
            ...
            e += pooling_type == CHEESE_POOLING_TYPE_NONE ? batch.n_tokens : s;
            s = 0;
    ...
    if (params.embd_out.empty()) {
        ...
        } else if (pooling_type == CHEESE_POOLING_TYPE_RANK) {
            const uint32_t n_cls_out = cheese_model_n_cls_out(model);
            std::vector<std::string> cls_out_labels;

            for (uint32_t i = 0; i < n_cls_out; i++) {
                const char * label = cheese_model_cls_label(model, i);
