...
static bool has_pooling(cheese_context * ctx) {
    switch (cheese_pooling_type(ctx)) {
...
struct output_data {
    ...
    output_data(cheese_context * ctx, const cheese_model * model, const common_params & params) {
        ...
        if (params.embedding) {
            const int n_embd       = cheese_model_n_embd_out(model);
            const bool pooling     = has_pooling(ctx);
            const int n_embd_count = pooling ? 1 : tokens.size();
            const int n_floats     = n_embd * n_embd_count;

            float * embd_raw = pooling ? cheese_get_embeddings_seq(ctx, 0) : cheese_get_embeddings(ctx);
            if (embd_raw == nullptr) {
            ...
            const float * logits = cheese_get_logits_ith(ctx, tokens.size() - 1);
            const int n_logits = cheese_vocab_n_tokens(vocab);
...
static void save_output_data(const output_data & output, const std::string & model_name, const std::string & output_dir) {
    ...
    auto base_path = std::filesystem::path{output_dir} / ("cheesecpp-" + model_name + output.type_suffix);
    ...
    // Save token ids to binary file.
    {
        ...
        file.write(reinterpret_cast<const char*>(output.tokens.data()), output.tokens.size() * sizeof(cheese_token));
        LOG("Tokens saved to %s\n", filepath.c_str());
    }

}

static void print_tokenized_prompt(cheese_context * ctx, const std::vector<cheese_token> & tokens, const std::string & prompt) {
    const cheese_model * model = cheese_get_model(ctx);
    const cheese_vocab * vocab = cheese_model_get_vocab(model);

    LOG("Model add_bos: %s\n", cheese_vocab_get_add_bos(vocab) ? "true" : "false");
    LOG("Input prompt: \"%s\"\n", prompt.c_str());
    LOG("Token ids (%zu):\n", tokens.size());

    for (auto id : tokens) {
        std::string piece(128, '\0');
        int n = cheese_token_to_piece(vocab, id, piece.data(), piece.size(), 0, true);
        if (n < 0) {
...
int main(int argc, char ** argv) {
    ...
    if (!common_params_parse(argc, argv, params, CHEESE_EXAMPLE_DEBUG, print_usage)) {
        return 1;
    }
