...
### Options
This example supports all standard `cheese.cpp` options and also accepts the
following options:
```console
$ cheese-debug --help
...
...
* `cheesecpp-<model>[-embeddings].bin`        - Binary output (logits or embeddings)
* `cheesecpp-<model>[-embeddings].txt`        - Text output (logits or embeddings, one per line)
* `cheesecpp-<model>[-embeddings]-prompt.txt` - Prompt text and token IDs
* `cheesecpp-<model>[-embeddings]-tokens.bin` - Binary token IDs for programmatic comparison
