...
static void batch_process(cheese_context * ctx, cheese_batch & batch, float * output, int n_seq, int n_embd) {
...
int main(int argc, char ** argv) {
    ...
    if (!common_params_parse(argc, argv, params, CHEESE_EXAMPLE_RETRIEVAL, print_usage)) {
        return 1;
    }
    ...
    for (auto & chunk : chunks) {
        ...
        if (cheese_vocab_eos(vocab) >= 0 && (inp.empty() || inp.back() != cheese_vocab_eos(vocab))) {
            inp.push_back(cheese_vocab_eos(vocab));
        }
        chunk.tokens = inp;
    }
    ...
    for (int k = 0; k < n_chunks; k++) {
        ...
        if (batch.n_tokens + n_toks > n_batch || s >= cheese_n_seq_max(ctx)) {
    ...
    struct cheese_batch query_batch = cheese_batch_init(n_batch, 0, 1);
