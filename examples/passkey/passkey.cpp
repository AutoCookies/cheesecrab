...
int main(int argc, char ** argv) {
    ...
    if (!common_params_parse(argc, argv, params, CHEESE_EXAMPLE_PASSKEY, print_usage)) {
        return 1;
    }
    ...
    ctx_params.n_ctx = cheese_model_n_ctx_train(model)*n_grp + n_keep;

    GGML_ASSERT(ctx_params.n_batch % n_grp == 0 && "n_batch must be divisible by n_grp");

    cheese_context * ctx = cheese_init_from_model(model, ctx_params);
    if (ctx == NULL) {
        LOG_ERR("%s: failed to create the cheese_context\n" , __func__);
        return 1;
    }
    ...
    const int n_ctx       = cheese_n_ctx(ctx) - n_keep;
    const int n_kv_req    = cheese_n_ctx(ctx);
    ...
    cheese_batch batch = cheese_batch_init(params.n_batch, 0, 1);

    int n_past = 0;

    auto * mem = cheese_get_memory(ctx);
    ...
    for (int i = 0; i < n_ctx; i += n_batch) {
        if (i > 0 && n_grp > 1) {
            ...
            cheese_memory_seq_add(mem, 0, n_past - n_batch,         n_past,         ib*bd);
            cheese_memory_seq_div(mem, 0, n_past - n_batch + ib*bd, n_past + ib*bd, n_grp);

            n_past = cheese_memory_seq_pos_max(mem, 0) + 1;
        }
        ...
        if (cheese_decode(ctx, batch) != 0) {
            LOG_INF("%s: cheese_decode() failed\n", __func__);
            return 1;
        }
    ...
    for (int i = n_ctx; i < n_tokens_all; i += n_batch) {
        ...
        cheese_memory_seq_rm (mem, 0, n_keep            , n_keep + n_discard);
        cheese_memory_seq_add(mem, 0, n_keep + n_discard, n_ctx,  -n_discard);
    ...
    while (n_cur <= n_len) {
        ...
        {
            const cheese_token new_token_id = cheese_sampler_sample(smpl, ctx, batch.n_tokens - 1);
