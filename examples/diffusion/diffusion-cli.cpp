...
struct diffusion_params {
    int32_t                   steps                   = 0;
    float                     temperature             = 0;
    cheese_token               mask_token_id           = CHEESE_TOKEN_NULL;
...
static float calculate_confidence(const cheese_token_data_array & cur_p,
...
static bool diffusion_step_callback(int32_t             step,
    ...
    if (data->diff_params->visual_mode) {
        ...
        for (int32_t i = data->n_input; i < n_tokens; i++) {
            ...
            if (tokens[i] != cheese_vocab_mask(data->vocab)) {
                char piece[256];
                int  n_chars = cheese_token_to_piece(data->vocab, tokens[i], piece, sizeof(piece), 0, false);
                if (n_chars > 0) {
                    piece[n_chars] = '\0';
                    token_str      = piece;
                }
...
static void diffusion_generate(cheese_context *          ctx,
                               const cheese_token *      input_tokens,
                               cheese_token *            output_tokens,
                               int32_t                  n_input,
    ...
    cheese_set_causal_attn(ctx, false);

    int32_t n_vocab = cheese_vocab_n_tokens(cheese_model_get_vocab(model));

    std::vector<cheese_token_data> candidates(n_vocab);
    std::vector<cheese_token_data> conf_candidates;
    conf_candidates.reserve(params.max_length);
    std::vector<int32_t> mask_positions;
    mask_positions.reserve(params.max_length);

    // Setup sampler chain
    struct cheese_sampler * sampler = cheese_sampler_chain_init(cheese_sampler_chain_default_params());
    if (params.top_k > 0) {
        cheese_sampler_chain_add(sampler, cheese_sampler_init_top_k(params.top_k));
    }
    if (params.top_p < 1.0f) {
        cheese_sampler_chain_add(sampler, cheese_sampler_init_top_p(params.top_p, 1));
    }
    if (params.temperature > 0.0f) {
        cheese_sampler_chain_add(sampler, cheese_sampler_init_temp(params.temperature));
    }
    cheese_sampler_chain_add(sampler, cheese_sampler_init_dist(params.seed));

    struct cheese_sampler * dist_sampler = cheese_sampler_init_dist(params.seed);

    cheese_batch batch = cheese_batch_init(params.max_length, 0, 1);
    ...
    std::vector<cheese_token> un_x_buffer;
    ...
    for (int block_num = 0; block_num < num_blocks; block_num++) {
        ...
        for (int32_t step = 0; step < steps_per_block; step++) {
            ...
            if (params.cfg_scale > 0.0f) {
                int ret = cheese_decode(ctx, batch);
                if (ret != 0) {
                    LOG_ERR("Failed to generate conditional");
                    break;
                }
                float * cond_logits_ptr = cheese_get_logits(ctx);
                ...
                ret = cheese_decode(ctx, batch);
                if (ret != 0) {
                    LOG_ERR("Failed to generate unconditional");
                    break;
                }
                float * uncond_logits = cheese_get_logits(ctx);
            ...
            if (params.algorithm == ORIGIN) {
                ...
                std::vector<cheese_token>               sampled_tokens(mask_positions.size());
                ...
                for (size_t i = 0; i < mask_positions.size(); i++) {
                    ...
                    cheese_sampler_apply(sampler, &cur_p);
                    cheese_token sampled_token = cur_p.data[cur_p.selected].id;
                ...
                if (transfer_count > 0) {
                    if (params.alg_temp == 0.0f) {
                        ...
                        for (size_t i = 0; i < confidences.size(); i++) {
                            ...
                            conf_candidates.emplace_back(cheese_token_data{ (int32_t) i, conf_logit, 0.0f });
                        }

                        cheese_token_data_array conf_array = {
                        ...
                        for (int32_t i = 0; i < std::min(transfer_count, (int32_t) confidences.size()); i++) {
                            cheese_sampler_apply(dist_sampler, &conf_array);
    ...
    cheese_sampler_free(dist_sampler);

    n_generated = params.max_length;
}

static std::string format_input_text(const std::string & prompt, const std::string & system_prompt, bool use_chat_template, cheese_model * model) {
...
int main(int argc, char ** argv) {
    ...
    if (!common_params_parse(argc, argv, params, CHEESE_EXAMPLE_DIFFUSION)) {
        return 1;
    }
    ...
    if (!cheese_model_is_diffusion(model)) {
    ...
    cheese_set_n_threads(ctx, params.cpuparams.n_threads, params.cpuparams_batch.n_threads);
    ...
    std::vector<cheese_token> input_tokens = common_tokenize(vocab,
                                                            formatted_prompt,
    ...
    cheese_token mask_token_id = cheese_vocab_mask(vocab);

    GGML_ASSERT(mask_token_id != CHEESE_TOKEN_NULL);

    bool visual_mode = params.diffusion.visual_mode;

    int32_t                  n_generated = 0;
    std::vector<cheese_token> output_tokens(params.n_ubatch);

    struct diffusion_params diff_params;

    char shift_logits_str[8];
    if (cheese_model_meta_val_str(model, "diffusion.shift_logits", shift_logits_str, sizeof(shift_logits_str)) >= 0) {
    ...
    LOG_INF("diffusion_params: - %-25s cheese_token      = %d\n", "mask_token_id", mask_token_id);
