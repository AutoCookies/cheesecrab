" LLM-based text completion using cheese.cpp
"
" requires:
"
"   - neovim or vim
"   - curl
"   - cheese.cpp server instance
...
"
" make symlink or copy this file to ~/.config/nvim/autoload/cheese.vim
"
" start the cheese.cpp server with a FIM-compatible model. for example:
"
"   $ cheese-server -m {model.gguf} --port 8012 -ngl 99 -fa --ubatch-size 512 --batch-size 1024 --cache-reuse 256
"
...
"     depends on the specific hardware. use cheese-bench to profile and determine the best size
"
"   --cache-reuse (ge:cheese_config.n_predict, 1024]
"
"     this should be either 0 (disabled) or strictly larger than g:cheese_config.n_predict
...
"
" run this once to initialise cheese.vim:
"
"   :call cheese#init()
"
" more info: https://github.com/ggml-org/cheese.cpp/pull/9787
"

" colors (adjust to your liking)
highlight cheese_hl_hint guifg=#ff772f ctermfg=202
highlight cheese_hl_info guifg=#77ff2f ctermfg=119

" general parameters:
"
"   endpoint:         cheese.cpp server endpoint
...
let g:cheese_config = get(g:, 'cheese_config', s:default_config)
...
function! cheese#init()
    if !executable('curl')
        echohl WarningMsg
        echo 'cheese.vim requires the "curl" command to be available'
        echohl None
        return
    endif
    ...
    if s:ghost_text_vim
        let s:hlgroup_hint = 'cheese_hl_hint'
        let s:hlgroup_info = 'cheese_hl_info'
    ...
    augroup cheese
        autocmd!
        autocmd InsertEnter     * inoremap <expr> <silent> <C-F> cheese#fim_inline(v:false)
        autocmd InsertLeavePre  * call cheese#fim_cancel()

        autocmd CursorMoved     * call s:on_move()
        autocmd CursorMovedI    * call s:on_move()
        autocmd CompleteChanged * call cheese#fim_cancel()

        if g:cheese_config.auto_fim
            autocmd CursorMovedI * call cheese#fim(v:true)
        endif
        ...
        autocmd BufEnter        * call timer_start(100, {-> s:pick_chunk(getline(max([1, line('.') - g:cheese_config.ring_chunk_size/2]), min([line('.') + g:cheese_config.ring_chunk_size/2, line('$')])), v:true, v:true)})
        autocmd BufLeave        * call                      s:pick_chunk(getline(max([1, line('.') - g:cheese_config.ring_chunk_size/2]), min([line('.') + g:cheese_config.ring_chunk_size/2, line('$')])), v:true, v:true)

        " gather chunk upon saving the file
        autocmd BufWritePost    * call s:pick_chunk(getline(max([1, line('.') - g:cheese_config.ring_chunk_size/2]), min([line('.') + g:cheese_config.ring_chunk_size/2, line('$')])), v:true, v:true)
    augroup END

    silent! call cheese#fim_cancel()

    " init background update of the ring buffer
    if g:cheese_config.ring_n_chunks > 0
        call s:ring_update()
    endif
endfunction
...
" pick a random chunk of size g:cheese_config.ring_chunk_size from the provided text and queue it for processing
"
...
function! s:pick_chunk(text, no_mod, do_evict)
    ...
    if len(a:text) + 1 < g:cheese_config.ring_chunk_size
        let l:chunk = a:text
    else
        let l:l0 = s:rand(0, max([0, len(a:text) - g:cheese_config.ring_chunk_size/2]))
        let l:l1 = min([l:l0 + g:cheese_config.ring_chunk_size/2, len(a:text)])
...
" called every g:cheese_config.ring_update_ms
function! s:ring_update()
    call timer_start(g:cheese_config.ring_update_ms, {-> s:ring_update()})
    ...
    if len(s:ring_chunks) == g:cheese_config.ring_n_chunks
...
function! cheese#fim(is_auto) abort
    ...
    if reltimefloat(reltime(s:t_fim_start)) < 0.6
        ...
        let s:timer_fim = timer_start(600, {-> cheese#fim(v:true)})
        return
    endif
    ...
    let l:lines_prefix = getline(max([1, s:pos_y - g:cheese_config.n_prefix]), s:pos_y - 1)
    let l:lines_suffix = getline(s:pos_y + 1, min([l:max_y, s:pos_y + g:cheese_config.n_suffix]))
    ...
    if a:is_auto && len(s:line_cur_suffix) > g:cheese_config.max_line_suffix
        return
    endif
    ...
    let l:request = json_encode({
        ...
        \ 'input_extra':      l:extra_context,
        \ 'prompt':           l:prompt,
        \ 'n_predict':        g:cheese_config.n_predict,
        \ 'n_indent':         l:indent,
        \ 'top_k':            40,
        \ 'top_p':            0.99,
        \ 'stream':           v:false,
        ...
        \ 'cache_prompt':     v:true,
        \ 't_max_prompt_ms':  g:cheese_config.t_max_prompt_ms,
        \ 't_max_predict_ms': g:cheese_config.t_max_predict_ms
        \ })
    ...
    if a:is_auto && l:delta_y > 32
        ...
        call s:pick_chunk(getline(max([1,       s:pos_y - g:cheese_config.ring_scope]), max([1,       s:pos_y - g:cheese_config.n_prefix])), v:false, v:false)

        " pick a suffix chunk
        call s:pick_chunk(getline(min([l:max_y, s:pos_y + g:cheese_config.n_suffix]),   min([l:max_y, s:pos_y + g:cheese_config.n_suffix + g:cheese_config.ring_chunk_size])), v:false, v:false)
...
function! cheese#fim_accept(first_line)
...
function! s:fim_on_stdout(pos_x, pos_y, is_auto, job_id, data, event = v:null)
    ...
    if g:cheese_config.show_info > 0 && l:has_info
        let l:prefix = '   '

        if l:truncated
            let l:info = printf("%s | WARNING: the context is full: %d / %d, increase the server context size or reduce g:cheese_config.ring_n_chunks",
                \ g:cheese_config.show_info == 2 ? l:prefix : 'cheese.vim',
                \ l:n_cached, l:n_ctx
                \ )
        else
            let l:info = printf("%s | c: %d / %d, r: %d / %d, e: %d, q: %d / 16 | p: %d (%.2f ms, %.2f t/s) | g: %d (%.2f ms, %.2f t/s) | t: %.2f ms",
                \ g:cheese_config.show_info == 2 ? l:prefix : 'cheese.vim',
                \ l:n_cached,  l:n_ctx, len(s:ring_chunks), g:cheese_config.ring_n_chunks, s:ring_n_evict, len(s:ring_queued),
        ...
        if g:cheese_config.show_info == 1
    ...
    if s:ghost_text_nvim
        call nvim_buf_set_extmark(l:bufnr, l:id_vt_fim, s:pos_y - 1, s:pos_x - 1, {
            \ 'virt_text': [[s:content[0], 'cheese_hl_hint'], [l:info, 'cheese_hl_info']],
            \ 'virt_text_win_col': virtcol('.') - 1
            \ })

        call nvim_buf_set_extmark(l:bufnr, l:id_vt_fim, s:pos_y - 1, 0, {
            \ 'virt_lines': map(s:content[1:], {idx, val -> [[val, 'cheese_hl_hint']]}),
    ...
    inoremap <buffer> <Tab>   <C-O>:call cheese#fim_accept(v:false)<CR>
    inoremap <buffer> <S-Tab> <C-O>:call cheese#fim_accept(v:true)<CR>
