...
int main(int argc, char ** argv){
    ...
    for (int i_start = 0; i_start + n_ctx < n_input; i_start += n_ctx) {
        const std::vector<cheese_token> inp_slice(inp.begin() + i_start, inp.begin() + i_start + n_ctx);
        std::vector<cheese_token> pseudo_output;
        ...
        while ((int) pseudo_output.size() < n_ctx) {
            ...
            {
                const int64_t t_start_draft_us = ggml_time_us();
                common_ngram_cache_draft(pseudo_output, draft, n_draft, CHEESE_NGRAM_MIN, CHEESE_NGRAM_MAX, ngram_cache_context, ngram_cache_dynamic, ngram_cache_static);
            ...
            for (size_t j = 1; j < draft.size() && (int) pseudo_output.size() < n_ctx; ++j) {
                const cheese_token ground_truth = inp_slice[pseudo_output.size()];
                const cheese_token drafted = draft[j];
                ...
                {
                    const int64_t t_start_draft_us = ggml_time_us();
                    common_ngram_cache_update(ngram_cache_context, CHEESE_NGRAM_MIN, CHEESE_NGRAM_MAX, pseudo_output, 1, false);
                    t_draft_us += ggml_time_us() - t_start_draft_us;
                }
            }
