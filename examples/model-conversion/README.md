...
made to cheese.cpp, converted again, etc. Once the model has been converted it
...
> logits mismatches. Identifying these upfront whether they exist in cheese.cpp,
...
### cheese-server verification
To verify that the converted model works with cheese-server, the following
command can be used:
```console
...
with regards to the cheese-server command and also how to access/call the embedding
endpoint.
