...
def test_single_prompt_similarity(python_emb, cpp_emb, tokens, prompt):
    ...
    print(python_emb)
    print("cheese.cpp embeddings:");
    print(cpp_emb)
    print(f"\n=== Prompt: '{prompt}' ===")
    print(f"Tokens: {tokens}")
    print(f"Embeddings shape: Python {python_emb.shape}, cheese.cpp {cpp_emb.shape}")
    ...
    if is_pooled:
        ...
        print(f"   Pooled embedding: Python={py_mag:.3f}, cheese.cpp={cpp_mag:.3f}, ratio={ratio:.3f}")
        ...
        for i in range(n_tokens):
            ...
            cpp_mag = np.linalg.norm(cpp_emb[i])   # calculate standard euclidean norm for cheese.cpp embeddings
            ratio = py_mag / cpp_mag if cpp_mag > 0 else float('inf')
            print(f"   Token {i} ({tokens[i]}): Python={py_mag:.3f}, cheese.cpp={cpp_mag:.3f}, ratio={ratio:.3f}")
...
def main():
    parser = argparse.ArgumentParser(description='Test semantic similarity between Python and cheese.cpp embeddings')
    ...
    parser.add_argument('--cpp-embeddings', '-ce', help='Path to cheese.cpp embeddings "logits" binary file')
    ...
    print("Semantic Similarity Test Between Python and cheese.cpp Embedding Models")
    print("=" * 70)
    ...
    cheesecpp_embeddings = load_embeddings_from_file(args.cpp_embeddings, n_tokens, model.config.hidden_size)
    python_embeddings = load_embeddings_from_file(args.python_embeddings, n_tokens, model.config.hidden_size)

    # Run comparison
    results = test_single_prompt_similarity(python_embeddings, cheesecpp_embeddings, tokens, prompt)
