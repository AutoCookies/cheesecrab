...
static std::vector<std::string> k_prompts = {
    ...
    "Tell me an interesting fact about cheeses.",
...
int main(int argc, char ** argv) {
    ...
    if (!common_params_parse(argc, argv, params, CHEESE_EXAMPLE_PARALLEL)) {
        return 1;
    }
    ...
    std::vector<cheese_token> tokens_system;

    tokens_system = common_tokenize(ctx, k_system, true);
    const int32_t n_tokens_system = tokens_system.size();

    cheese_seq_id g_seq_id = 0;

    // the max batch size is as large as the context to handle cases where we get very long input prompt from multiple
    // users. regardless of the size, the main loop will chunk the batch into a maximum of params.n_batch tokens at a time
    cheese_batch batch = cheese_batch_init(n_ctx, 0, 1);
    ...
    if (is_sp_shared) {
        ...
        for (int32_t i = 1; i <= n_clients; ++i) {
            cheese_memory_seq_cp(mem, 0, i, -1, -1);
        }
    ...
    while (true) {
        ...
        if (batch.n_tokens == 0) {
            ...
            for (int i = 1; i <= n_clients; ++i) {
                cheese_memory_seq_rm(mem, i, -1, -1);
        ...
        if (cont_batching || batch.n_tokens == 0) {
            for (auto & client : clients) {
                if (client.seq_id == -1 && g_seq_id < n_seq) {
                    ...
                    std::vector<cheese_token> tokens_prompt;
        ...
        for (int32_t i = 0; i < batch.n_tokens; i = i_next) {
            ...
            for (auto & client : clients) {
                ...
                const cheese_token id = common_sampler_sample(client.smpl, ctx, client.i_batch - i);
                ...
                if (client.n_decoded > 2 &&
                    ...
                    cheese_memory_seq_rm(mem,    client.id + 1, -1, -1);
                    cheese_memory_seq_cp(mem, 0, client.id + 1, -1, -1);
