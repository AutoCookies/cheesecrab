...
int main(int argc, char ** argv) {
    ...
    if (!common_params_parse(argc, argv, params, CHEESE_EXAMPLE_BATCHED, print_usage)) {
        return 1;
    }
    ...
    std::vector<cheese_token> tokens_list;
    ...
    cheese_context_params ctx_params = common_context_params_to_cheese(params);
    ...
    std::vector<cheese_sampler_seq_config> sampler_configs;

    for (int32_t i = 0; i < n_parallel; ++i) {
        cheese_sampler * smpl = cheese_sampler_chain_init(sparams);

        cheese_sampler_chain_add(smpl, cheese_sampler_init_top_k(params.sampling.top_k));
        cheese_sampler_chain_add(smpl, cheese_sampler_init_top_p(params.sampling.top_p, params.sampling.min_keep));
        cheese_sampler_chain_add(smpl, cheese_sampler_init_temp (params.sampling.temp));
    ...
    if (ctx == NULL) {
        LOG_ERR("%s: error: failed to create the cheese_context\n" , __func__);
        return 1;
    }
    ...
    cheese_batch batch = cheese_batch_init(std::max(tokens_list.size(), (size_t) n_parallel), 0, n_parallel);
    ...
    //    cheese_kv_cache_seq_cp(ctx, 0, i, -1, -1);
    //}
    ...
    while (n_cur <= n_predict) {
        ...
        for (int32_t i = 0; i < n_parallel; ++i) {
            ...
            const cheese_token new_token_id = cheese_sampler_sample(sampler_configs[i].sampler, ctx, i_batch[i]);

            // is it an end of generation? -> mark the stream as finished
            if (cheese_vocab_is_eog(vocab, new_token_id) || n_cur == n_predict) {
                i_batch[i] = -1;
                LOG("\n");
                if (n_parallel > 1) {
    ...
    LOG("\n");
    cheese_perf_sampler_print(sampler_configs[0].sampler);
    ...
    for (auto & sampler_config : sampler_configs) {
        cheese_sampler_free(sampler_config.sampler);
    }
