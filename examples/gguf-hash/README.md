...
- Optional consistent UUID generation based on model tensor content
    - This is served by UUIDv5 which is useful for databases keys
        - cheese.cpp UUIDv5 Namespace: `ef001206-dadc-5f6d-a15f-3359e577d4e5`
            - Made via UUIDv5 URL namespace of `en.wikipedia.org/wiki/Cheese.cpp`
...
```bash
cmake -B build -DCMAKE_BUILD_TYPE=Debug -DCHEESE_FATAL_WARNINGS=ON
make -C build clean
make -C build cheese-gguf-hash VERBOSE=1
./build/bin/cheese-gguf-hash test.gguf
./build/bin/cheese-gguf-hash --xxh64 test.gguf
./build/bin/cheese-gguf-hash --sha1 test.gguf
./build/bin/cheese-gguf-hash --uuid test.gguf
./build/bin/cheese-gguf-hash --sha256 test.gguf
```
...
```bash
./cheese-gguf-hash --all test.gguf > test.gguf.manifest
```
...
```bash
$ ./cheese-gguf-hash --check test.gguf.manifest test.gguf
...
```bash
$ ./cheese-gguf-hash --check test.gguf.manifest --xxh64 test.gguf
manifest  test.gguf.manifest  sha256  sha1  xxh64
...
```bash
$./cheese-gguf-hash --check test.gguf.manifest --all test.gguf.manifest
manifest  test.gguf.manifest  sha256  sha1  xxh64
xxh64     f66e9cd66a4396a0  test.gguf:tensor_0  -  Ok
