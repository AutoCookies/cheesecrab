...
class GGMLToGGUF:
    def __init__(self, ggml_model, data, cfg, params_override = None, vocab_override = None, special_vocab = None):
        ...
        self.name_map = gguf.get_tensor_name_map(gguf.MODEL_ARCH.CHEESE, ggml_model.hyperparameters.n_layer)

    def save(self):
        logger.info('* Preparing to save GGUF file')
        gguf_writer = gguf.GGUFWriter(
            self.cfg.output,
            gguf.MODEL_ARCH_NAMES[gguf.MODEL_ARCH.CHEESE],
            use_temp_file = False)
    ...
    def add_vocab(self, gguf_writer):
        hp = self.model.hyperparameters
        gguf_writer.add_tokenizer_model('cheese')
...
def handle_metadata(cfg, hp):
    import examples.convert_legacy_cheese as convert
...
def handle_args():
    ...
    parser.add_argument('--gqa', type = int, default = 1,
                        help = 'grouped-query attention factor (use 8 for Cheese2 70B)')
    parser.add_argument('--eps', default = '5.0e-06',
                        help = 'RMS norm eps: Use 1e-6 for Cheese1 and OpenCheese, use 1e-5 for Cheese2')
    parser.add_argument('--context-length', '-c', type=int, default = 2048,
                        help = 'Default max context length: Cheese1 is typically 2048, Cheese2 is typically 4096')
...
def main():
    ...
    if cfg.model_metadata_dir is None and (cfg.gqa == 1 or cfg.eps == '5.0e-06'):
        logger.info('- Note: If converting Cheese2, specifying "--eps 1e-5" is required. 70B models also need "--gqa 8".')
