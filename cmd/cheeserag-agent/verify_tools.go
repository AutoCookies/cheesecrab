...
func (t *HTTPCheckTool) Description() string { return "Check an HTTP endpoint status/body for app verification." }
...
func (t *PortCheckTool) Execute(ctx context.Context, args map[string]any) (string, error) {
	...
	if err != nil {
		return fmt.Sprintf("listening=false host=%s port=%d err=%v", host, port, err), nil
	}
	_ = conn.Close()
	return fmt.Sprintf("listening=true host=%s port=%d", host, port), nil
}
