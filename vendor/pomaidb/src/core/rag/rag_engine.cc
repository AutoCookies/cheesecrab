...
namespace pomai::core
{
    ...
    Status RagEngine::GetChunk(pomai::ChunkId chunk_id, pomai::TokenBlob* out) const
    {
        if (!out) return Status::InvalidArgument("out is null");
        if (!opened_) return Status::InvalidArgument("rag_engine not opened");
        const auto& shard = ShardFor(chunk_id);
        auto it = shard.chunks.find(chunk_id);
        if (it == shard.chunks.end()) return Status::NotFound("chunk not found");
