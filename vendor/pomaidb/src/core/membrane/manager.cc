...
namespace pomai::core
{
    ...
    Status MembraneManager::CreateMembrane(const pomai::MembraneSpec &spec)
    {
        ...
        membranes_.emplace(spec.name, std::move(state));
        // Open the new membrane so it is immediately usable (RAG/vector engine must be opened).
        auto st_open = OpenMembrane(spec.name);
        if (!st_open.ok()) return st_open;
        return Status::Ok();
    }
    ...
    Status MembraneManager::GetRagChunk(std::string_view membrane, pomai::ChunkId chunk_id, pomai::TokenBlob* out)
    {
        if (!out)
            return Status::InvalidArgument("out is null");
        auto *state = GetMembraneOrNull(membrane);
        if (!state)
            return Status::NotFound("membrane not found");
        if (state->spec.kind != pomai::MembraneKind::kRag)
            return Status::InvalidArgument("RAG membrane required for GetRagChunk");
        return state->rag_engine->GetChunk(chunk_id, out);
    }
