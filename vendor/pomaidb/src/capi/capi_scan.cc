...
pomai_status_t* pomai_get_snapshot(pomai_db_t* db, pomai_snapshot_t** out_snap) {
    ...
    auto st = db->db->GetSnapshot(pomai::core::MembraneManager::kDefaultMembrane, &snap);
    if (!st.ok()) {
        return ToCStatus(st);
    }
...
pomai_status_t* pomai_scan(
    ...
    auto st = db->db->NewIterator(pomai::core::MembraneManager::kDefaultMembrane, snap->snap, &iter);
    if (!st.ok()) {
        return ToCStatus(st);
    }
