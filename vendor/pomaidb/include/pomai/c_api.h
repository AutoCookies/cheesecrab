...
/** Get token IDs for a RAG chunk. *out_token_ids is palloc-allocated; caller must pomai_free() it. */
POMAI_API pomai_status_t* pomai_get_rag_chunk(pomai_db_t* db, const char* membrane_name, uint64_t chunk_id, uint32_t** out_token_ids, size_t* out_count);
