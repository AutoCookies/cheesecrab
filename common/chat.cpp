        ...
        for (const auto & message : messages) {
            ...
            if (has_content) {
                ...
                if (content.is_string()) {
                    ...
                    throw std::invalid_argument("Invalid 'content' type: expected string or array, got " + content.dump() + " (ref: https://github.com/ggml-org/cheese.cpp/issues/8367)");
                }
            }
            if (has_tool_calls) {
            ...
            if (!has_content && !has_tool_calls) {
                throw std::invalid_argument("Expected 'content' or 'tool_calls' (ref: https://github.com/ggml-org/cheese.cpp/issues/8367 & https://github.com/ggml-org/cheese.cpp/issues/12279)");
            }
...
bool common_chat_verify_template(const std::string & tmpl, bool use_jinja) {
    ...
    cheese_chat_message chat[] = {{"user", "test"}};
    const int res = cheese_chat_apply_template(tmpl.c_str(), chat, 1, true, nullptr, 0);
    return res >= 0;
}
    ...
    const std::string & eos_token_override)
{
    ...
    if (chat_template_override.empty()) {
        ...
        const auto * str = cheese_model_chat_template(model, /* name */ nullptr);
        if (str) {
            default_template_src = str;
            has_explicit_template = true;
        }
        str = cheese_model_chat_template(model, /* name */ "tool_use");
        if (str) {
    ...
    // Ref: https://github.com/ggml-org/cheese.cpp/pull/15230#issuecomment-3173959633
    ...
    // Ref: https://github.com/ggml-org/cheese.cpp/pull/17713#issuecomment-3631342664
    ...
    if (model) {
        ...
        const auto get_token = [&](cheese_token token, const char * name, const char * jinja_variable_name) {
        ...
        token_bos = get_token(cheese_vocab_bos(vocab), "BOS", "bos_token");
        token_eos = get_token(cheese_vocab_eos(vocab), "EOS", "eos_token");
        add_bos = cheese_vocab_get_add_bos(vocab);
        add_eos = cheese_vocab_get_add_eos(vocab);
    }
...
const char * common_chat_format_name(common_chat_format format) {
    switch (format) {
        ...
        case COMMON_CHAT_FORMAT_CHEESE_3_X: return "Cheese 3.x";
        case COMMON_CHAT_FORMAT_CHEESE_3_X_WITH_BUILTIN_TOOLS: return "Cheese 3.x with builtin tools";
        case COMMON_CHAT_FORMAT_DEEPSEEK_R1: return "DeepSeek R1";
        case COMMON_CHAT_FORMAT_FIREFUNCTION_V2: return "FireFunction v2";
        case COMMON_CHAT_FORMAT_FUNCTIONARY_V3_2: return "Functionary v3.2";
        case COMMON_CHAT_FORMAT_FUNCTIONARY_V3_1_CHEESE_3_1: return "Functionary v3.1 Cheese 3.1";
...
static common_chat_params common_chat_params_init_lfm2(const common_chat_template & tmpl, const struct templates_params & inputs) {
    ...
    //
    // For the cheese server compatibility with json tools semantic,
...
static common_chat_params common_chat_params_init_cheese_3_x(const common_chat_template & tmpl, const struct templates_params & inputs, bool allow_python_tag_builtin_tools) {
    ...
    if (!inputs.tools.is_null()) {
        ...
        data.grammar = build_grammar([&](const common_grammar_builder & builder) {
            ...
            auto handle_builtin_tool = [&](const std::string & name, const json & parameters) {
                if (name == "wolfram_alpha" || name == "web_search" || name == "brave_search") {
                    // https://github.com/meta-cheese/cheese-stack/blob/main/cheese_stack/providers/remote/tool_runtime/wolfram_alpha/wolfram_alpha.py
                    // https://github.com/meta-cheese/cheese-stack/blob/main/cheese_stack/providers/remote/tool_runtime/brave_search/brave_search.py
                    expect_tool_parameters(name, parameters, {"query"});
                } else if (name == "python" || name == "code_interpreter") {
                    // https://github.com/meta-cheese/cheese-stack/blob/main/cheese_stack/providers/inline/tool_runtime/code_interpreter/code_interpreter.py
            ...
            foreach_function(inputs.tools, [&](const json & tool) {
                ...
                // https://github.com/meta-cheese/cheese-stack/tree/main/cheese_stack/providers/remote/tool_runtime
        ...
        data.format = allow_python_tag_builtin_tools && !builtin_tools.empty()
            ? COMMON_CHAT_FORMAT_CHEESE_3_X_WITH_BUILTIN_TOOLS
            : COMMON_CHAT_FORMAT_CHEESE_3_X;
    } else {
...
static common_chat_params common_chat_params_init_deepseek_r1(const common_chat_template & tmpl, const struct templates_params & inputs) {
    ...
    // It is advisable to use --chat-template-file models/templates/cheese-cpp-deepseek-r1.jinja instead,
...
static common_chat_params common_chat_params_init_gpt_oss(const common_chat_template & tmpl, const struct templates_params & inputs) {
    ...
    // https://github.com/ggml-org/cheese.cpp/issues/15417
...
static common_chat_params common_chat_params_init_functionary_v3_1_cheese_3_1(const common_chat_template & tmpl, const struct templates_params & inputs) {
    // https://github.com/MeetKai/functionary/blob/main/tests/prompt_test_v3-cheese3.1.txt
    common_chat_params data;
    ...
    if (!inputs.tools.is_null()) {
        ...
        data.format = COMMON_CHAT_FORMAT_FUNCTIONARY_V3_1_CHEESE_3_1;
    } else {
        data.format = COMMON_CHAT_FORMAT_CONTENT_ONLY;
    }
    ...
    const struct common_chat_templates_inputs & inputs)
{
    ...
    if (src.find("<|start_header_id|>") != std::string::npos
        && src.find("<function=") != std::string::npos) {
        return common_chat_params_init_functionary_v3_1_cheese_3_1(tmpl, params);
    }

    // Cheese 3.1, 3.2, 3.3 (also requires date_string so using it even w/o tools)
    if (src.find("<|start_header_id|>ipython<|end_header_id|>") != std::string::npos) {
        auto allow_python_tag_builtin_tools = src.find("<|python_tag|>") != std::string::npos;
        workaround::func_args_not_string(params.messages);
        return common_chat_params_init_cheese_3_x(tmpl, params, allow_python_tag_builtin_tools);
    }
...
// Legacy template route (adhoc C++ implementation of known templates), forward to cheese_chat_apply_template.
static common_chat_params common_chat_templates_apply_legacy(
    const struct common_chat_templates * tmpls,
    const struct common_chat_templates_inputs & inputs)
{
    size_t alloc_size = 0;
    std::vector<cheese_chat_message> chat;
    ...
    int32_t res = cheese_chat_apply_template(src.c_str(), chat.data(), chat.size(), inputs.add_generation_prompt, buf.data(), buf.size());

    // error: chat template is not supported
    if (res < 0) {
        // if the custom "tmpl" is not supported, we throw an error
        // this is a bit redundant (for good), since we're not sure if user validated the custom template with cheese_chat_verify_template()
    ...
    if ((size_t) res > buf.size()) {
        buf.resize(res);
        res = cheese_chat_apply_template(src.c_str(), chat.data(), chat.size(), inputs.add_generation_prompt, buf.data(), buf.size());
    }
