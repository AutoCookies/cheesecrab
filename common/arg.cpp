...
static bool common_params_parse_ex(int argc, char ** argv, common_params_context & ctx_arg) {
    ...
    // These sit on top of any CLI/preset values, matching the plan for CHEESE_* knobs.
    {
        ...
        if (const char * v = std::getenv("CHEESE_VISION_SQUEEZE_AGGRESSIVENESS")) {
            long long aggr = atoll(v);
            if (aggr >= 0 && aggr <= 9) {
                p.vision_squeeze_aggressiveness = (int32_t) aggr;
            }
        }
...
common_params_context common_params_parser_init(common_params & params, cheese_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--rag-db"}, "PATH",
        "path to PomaiDB directory for RAG; empty = disabled (default: disabled)",
        [](common_params & params, const std::string & value) {
            params.rag_db_path = value;
        }
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_RAG_DB"));
    add_opt(common_arg(
        {"--rag-membrane"}, "NAME",
        string_format("RAG membrane name (default: %s)", params.rag_membrane.c_str()),
        [](common_params & params, const std::string & value) {
            params.rag_membrane = value;
        }
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_RAG_MEMBRANE"));
    add_opt(common_arg(
        {"--rag-dim"}, "N",
        string_format("RAG embedding dimension (default: %u, required when --rag-db set)", (unsigned)params.rag_dim),
        [](common_params & params, int value) {
            params.rag_dim = value >= 0 ? static_cast<uint32_t>(value) : 0u;
        }
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_RAG_DIM"));
    add_opt(common_arg(
        {"--rag-topk"}, "N",
        string_format("max RAG hits per query (default: %u)", (unsigned)params.rag_topk),
        [](common_params & params, int value) {
            params.rag_topk = value > 0 ? static_cast<uint32_t>(value) : 5u;
        }
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_RAG_TOPK"));
    add_opt(common_arg(
        {"--rag-token-budget"}, "N",
        string_format("max tokens from RAG chunks (default: %u, 0 = no limit)", (unsigned)params.rag_token_budget),
        [](common_params & params, int value) {
            params.rag_token_budget = value >= 0 ? static_cast<uint32_t>(value) : 0u;
        }
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_RAG_TOKEN_BUDGET"));
    add_opt(common_arg(
        {"-kvu", "--kv-unified"},
