...
static uint32_t common_ngram_map_hash(const cheese_tokens & tokens, size_t start, size_t len) {
    uint32_t hash = 0;
...
// Print the values of a sublist of `cheese_tokens & inp` to a string in the form [v0, v1, v2, ...].
static std::string common_tokens_to_str(const cheese_tokens & inp, size_t start, size_t length) {
 ...
 */
cheese_tokens common_ngram_simple_draft(
        const common_ngram_simple_config & config,
        const cheese_tokens & tokens, cheese_token sampled) {
...
void common_ngram_map_begin(
    common_ngram_map & map, const cheese_tokens & tokens) {
...
void common_ngram_map_draft(common_ngram_map & map,
        const cheese_tokens & inp, cheese_token sampled,
        cheese_tokens & draft) {
    ...
    std::vector<cheese_token> key_tokens;
