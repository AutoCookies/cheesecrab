...
static bool common_speculative_are_compatible(
    const cheese_model * model_tgt,
    const cheese_model * model_dft) {
...
struct common_speculative_state {
    ...
    virtual void begin(const cheese_tokens & prompt) = 0;

    virtual void draft(
            const common_params_speculative & params,
            const cheese_tokens & prompt_tgt,
            cheese_token id_last,
            cheese_tokens & result) = 0;

    virtual void accept(uint16_t n_accepted) = 0;
};

struct common_speculative_state_draft : public common_speculative_state {
    cheese_context * ctx_tgt; // only used for retokenizing from ctx_dft
    cheese_context * ctx_dft;

    common_sampler * smpl;

    cheese_batch  batch;
    cheese_tokens prompt_dft;
    ...
    common_speculative_state_draft(
        ...
        , ctx_dft(ctx_dft)
    {
        batch = cheese_batch_init(cheese_n_batch(ctx_dft), 0, 1);
        smpl = nullptr;
        ...
        //     };
        //
        //     result->smpl = common_sampler_init(cheese_get_model(ctx_dft), params);
        // }
        {
            common_params_sampling params;
            params.no_perf = false;
            ...
            smpl = common_sampler_init(cheese_get_model(ctx_dft), params);
        }

        vocab_cmpt = common_speculative_are_compatible(cheese_get_model(ctx_tgt), cheese_get_model(ctx_dft));
    ...
    void begin(const cheese_tokens & prompt) override {
        GGML_UNUSED(prompt);
    }
    ...
    void draft(
        ...
        const int n_ctx = cheese_n_ctx(ctx_dft) - params.n_max;

        cheese_tokens prompt_cnv;
        if (!spec->vocab_cmpt) {
            std::string text;
            ...
            // convert id_last to draft vocab. cheese_detokenize is called directly to avoid an allocation
            const auto * model_tgt = cheese_get_model(ctx_tgt);
            const auto * vocab_tgt = cheese_model_get_vocab(model_tgt);

            int32_t n_chars = cheese_detokenize(vocab_tgt, &id_last, 1, nullptr, 0, false, false);
            GGML_ASSERT(n_chars < 0 && "failed to detokenize id_last");

            text.resize(-n_chars);
            cheese_detokenize(vocab_tgt, &id_last, 1, text.data(), text.size(), false, false);
            text = replace_to_dft(text);
        ...
        const cheese_tokens & prompt_cur = spec->vocab_cmpt ? prompt_tgt : prompt_cnv;
        ...
        if (reuse_n == 0) {
            cheese_memory_clear(mem_dft, false);
            prompt_dft.clear();
        } else {
            ...
            if (reuse_i > 0) {
                cheese_memory_seq_rm (mem_dft, 0, 0, reuse_i);
                cheese_memory_seq_add(mem_dft, 0, reuse_i, -1, -reuse_i);

                prompt_dft.erase(prompt_dft.begin(), prompt_dft.begin() + reuse_i);
            }

            if (reuse_n < (int) prompt_dft.size()) {
                cheese_memory_seq_rm (mem_dft, 0, reuse_n, -1);
        ...
        if (batch.n_tokens > 0) {
            ...
            cheese_decode(ctx_dft, batch);
        }

        const cheese_pos n_past = prompt_dft.size();
        ...
        for (int i = 0; i < params.n_max; ++i) {
            ...
            const cheese_token id = cur_p->data[0].id;
...
struct common_speculative_state_eagle3 : public common_speculative_state {
    ...
    void draft(
            ...
            const cheese_tokens & prompt_tgt,
            cheese_token id_last,
            cheese_tokens & draft_tokens) override {
        // TODO: implement
        GGML_UNUSED(params);
...
struct common_speculative_state_ngram_mod : public common_speculative_state {
    ...
    // enable trace logging if CHEESE_TRACE is set
    const bool verbose;

    common_speculative_state_ngram_mod(enum common_speculative_type type, common_ngram_mod & mod)
        : common_speculative_state(type), mod(mod), verbose(std::getenv("CHEESE_TRACE") != nullptr) {
        static_assert(sizeof(cheese_token) == sizeof(common_ngram_mod::entry_t));
    }
    ...
    void draft(
        ...
        for (int i = 0; i < params.n_max; ++i) {
            const cheese_token token = mod.get(result.data() + i);
...
struct common_speculative_state_ngram_cache : public common_speculative_state {
    ...
    void draft(
        ...
        if (cache_size < prompt_tgt.size() + 1) {
            ...
            common_ngram_cache_update(ngram_cache_context, CHEESE_NGRAM_MIN, CHEESE_NGRAM_MAX,
        ...
        common_ngram_cache_draft(inp, result, n_draft, CHEESE_NGRAM_MIN, CHEESE_NGRAM_MAX,
                ngram_cache_context,
                ngram_cache_dynamic,
                ngram_cache_static);
...
bool common_speculative_is_compat(cheese_context * ctx_tgt) {
    auto * mem = cheese_get_memory(ctx_tgt);
    if (mem == nullptr) {
        return false;
    }
    ...
    int ret = cheese_decode(ctx_tgt, cheese_batch_get_one(tmp.data(), tmp.size()));
    if (ret != 0) {
        LOG_ERR("%s: cheese_decode() failed: %d\n", __func__, ret);
        res = false;
        goto done;
    }

    // try to remove the last tokens
    if (!cheese_memory_seq_rm(mem, 0, 1, -1)) {
...
common_speculative * common_speculative_init(
        common_params_speculative & params,
        cheese_context             * ctx_tgt) {
    cheese_context * ctx_dft = nullptr;
    if (params.model_dft) {
        ctx_dft = cheese_init_from_model(params.model_dft, params.cparams_dft);
        if (ctx_dft == nullptr) {
        ...
        if (has_ngram_mod) {
            ...
            if (!params.ngram_mod) {
                ...
                if (params.ngram_size_n < 16) {
                    LOG_WRN("%s: ngram_mod n=%d is too small - poor quality is possible, see: https://github.com/ggml-org/cheese.cpp/pull/19164\n", __func__, params.ngram_size_n);
                }
            }
...
cheese_tokens common_speculative_draft(
        ...
        const common_params_speculative & params,
        const cheese_tokens & prompt_tgt, // specified in target model vocab
        cheese_token id_last) {
    cheese_tokens result;
