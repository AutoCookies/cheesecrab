...
static int common_download_file_single_online(const std::string        & url,
    ...
    if (headers.find("User-Agent") == headers.end()) {
        headers.emplace("User-Agent", "cheese-cpp/" + build_info);
    }
...
bool common_download_model(const common_params_model & model,
    ...
    if (n_split > 1) {
        ...
        char split_url_prefix[CHEESE_MAX_URL_LENGTH] = {0};

        // Verify the first split file format
        // and extract split URL and PATH prefixes
        {
            if (!cheese_split_prefix(split_prefix, sizeof(split_prefix), model.path.c_str(), 0, n_split)) {
                LOG_ERR("\n%s: unexpected model file name: %s n_split=%d\n", __func__, model.path.c_str(), n_split);
                return false;
            }

            if (!cheese_split_prefix(split_url_prefix, sizeof(split_url_prefix), model.url.c_str(), 0, n_split)) {
        ...
        for (int idx = 1; idx < n_split; idx++) {
            ...
            cheese_split_path(split_path, sizeof(split_path), split_prefix, idx, n_split);

            char split_url[CHEESE_MAX_URL_LENGTH] = {0};
            cheese_split_path(split_url, sizeof(split_url), split_url_prefix, idx, n_split);
...
common_hf_file_res common_get_hf_file(const std::string & hf_repo_with_tag,
    ...
    // Important: the User-Agent must be "cheese-cpp" to get the "ggufFile" field in the response
