...
#define APIR_CHEESE_CPP_GGML_LIBRARY_PATH_ENV "APIR_CHEESE_CPP_GGML_LIBRARY_PATH"
#define APIR_CHEESE_CPP_GGML_LIBRARY_REG_ENV  "APIR_CHEESE_CPP_GGML_LIBRARY_REG"
#define APIR_CHEESE_CPP_LOG_TO_FILE_ENV       "APIR_CHEESE_CPP_LOG_TO_FILE"
...
ApirLoadLibraryReturnCode apir_backend_initialize(uint32_t virgl_ctx_id, struct virgl_apir_callbacks * virgl_cbs) {
    ...
    const char * apir_log_to_file = getenv(APIR_CHEESE_CPP_LOG_TO_FILE_ENV);
    if (apir_log_to_file) {
    ...
    if (!library_name) {
        GGML_LOG_ERROR(GGML_VIRTGPU_BCK "%s: cannot open the GGML library: env var '%s' not defined\n", __func__,
                       APIR_CHEESE_CPP_GGML_LIBRARY_PATH_ENV);
    ...
    if (!library_reg) {
        GGML_LOG_ERROR(GGML_VIRTGPU_BCK "%s: cannot register the GGML library: env var '%s' not defined\n", __func__,
                       APIR_CHEESE_CPP_GGML_LIBRARY_REG_ENV);
    ...
    if (dlsym_error) {
        GGML_LOG_ERROR(GGML_VIRTGPU_BCK "%s: cannot find the GGML backend registration symbol '%s' (from %s): %s\n",
                       __func__, library_reg, APIR_CHEESE_CPP_GGML_LIBRARY_REG_ENV, dlsym_error);
