...
int main(int argc, char **argv) {
    ...
    //GGML_ASSERT(cheese_vocab_type(vocab) == CHEESE_VOCAB_TYPE_BPE);
    if (cheese_vocab_type(vocab) != CHEESE_VOCAB_TYPE_BPE) {
        return 99;
    }
    ...
    for (int i = 0; i < n_vocab; ++i) {
        ...
        try {
            ...
            std::vector<cheese_token> tokens = common_tokenize(ctx, str, false, true);
        ...
        for (int i = 0; i < nthread; ++i) {
            threads[i] = std::thread([i, nthread, ctx, &errcode]() {
                for (uint32_t cp = i; !errcode && cp < 0x00110000; cp += nthread) {
                    ...
                    std::vector<cheese_token> tokens = common_tokenize(ctx, str, false);
                    std::string check = common_detokenize(ctx, tokens);
