...
static cheese_grammar * build_grammar(const std::string & grammar_str) {
    return cheese_grammar_init_impl(nullptr, grammar_str.c_str(), "root", false, nullptr, 0, nullptr, 0);
}

static bool test_build_grammar_fails(const std::string & grammar_str) {
    fprintf(stderr, "⚫ Testing failure for grammar: %s\n", grammar_str.c_str());
    bool grammar_fails = false;
    cheese_grammar * grammar = build_grammar(grammar_str);
    if (grammar != nullptr) {
...
static std::string token(cheese_token id) {
    return std::string{
    ...
    while (offset < input.size()) {
        try {
            if (static_cast<unsigned char>(input[offset]) == 0xff) {
                ...
                result.push_back({static_cast<cheese_token>(val), piece});
                offset += 5;
            } else {
            ...
            // Silently ignore invalid UTF-8 input to avoid leaking the exception beyond cheese_tokenize
            ++offset;
...
static bool match_string(const std::string & input, cheese_grammar * grammar) {
    ...
    for (const auto & in : parsed) {
        try {
            cheese_grammar_accept_token(*grammar, in.token, in.piece);
        } catch (const std::runtime_error & /*e*/) {
            // normally this shouldn't get hit because of cheese_grammar_apply
            return false;
        }
...
static void test(const std::string & test_desc, const std::string & grammar_str, const std::vector<std::string> & passing_strings, const std::vector<std::string> & failing_strings) {
    ...
    const cheese_grammar_stacks stacks_org = cheese_grammar_get_stacks(grammar); // copy

    cheese_grammar_stacks & stacks_cur = cheese_grammar_get_stacks(grammar);
    ...
    for (const auto & test_string : passing_strings) {
        ...
        if (!matched) {
            ...
            fprintf(stderr, "\n NOTE: Debug grammar file generated. To analyze this failure in detail, run the following command:     ./cheese-gbnf-validator test-grammar-integration.grammar.gbnf test-grammar-integration.string.txt\n\n");
        } else {
            fprintf(stdout, "✅︎\n");
        }
