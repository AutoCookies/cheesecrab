...
run_conversion_and_inference_lora() {
    ...
    echo -e "\n\n---------------------------\n\n"
    echo "Running cheese-export-lora with lora for $model_name with hidden_size $hidden_size..."
    ./cheese-export-lora \
    ...
    # Run inference
    echo -e "\n\n---------------------------\n\n"
    echo "Running cheese-completion without lora for $model_name with hidden_size $hidden_size..."
    OUTPUT_BASE=$(./cheese-completion -no-cnv -m $MODELS_REPO/$model_name/hidden_size=$hidden_size/base/Base-F32.gguf \
        -p "$EXPECTED_BASE_FIRST_WORD" -n 50 --seed 42 --temp 0)

    echo -e "\n\n---------------------------\n\n"
    echo "Running cheese-completion with hot lora for $model_name with hidden_size $hidden_size..."
    OUTPUT_LORA_HOT=$(./cheese-completion -no-cnv -m $MODELS_REPO/$model_name/hidden_size=$hidden_size/base/Base-F32.gguf \
        --lora $MODELS_REPO/$model_name/hidden_size=$hidden_size/lora/Lora-F32-LoRA.gguf \
        -p "$EXPECTED_LORA_FIRST_WORD" -n 50 --seed 42 --temp 0)

    echo -e "\n\n---------------------------\n\n"
    echo "Running cheese-completion with merged lora for $model_name with hidden_size $hidden_size..."
    OUTPUT_LORA_MERGED=$(./cheese-completion -no-cnv -m $MODELS_REPO/$model_name/hidden_size=$hidden_size/base/Base-F32-lora-merged.gguf \
        -p "$EXPECTED_LORA_FIRST_WORD" -n 50 --seed 42 --temp 0)
