...
static const cheese_vocab * vocab;

static bool match_string(const std::string & input, cheese_sampler * grammar) {
    cheese_sampler_reset(grammar);
    auto tokens = common_tokenize(vocab, input, false, false);

    auto n_vocab = cheese_vocab_n_tokens(vocab);

    std::vector<cheese_token_data> cur;
    cur.reserve(n_vocab);
    for (cheese_token token_id = 0; token_id < (cheese_token) n_vocab; token_id++) {
        cur.emplace_back(cheese_token_data{ token_id, 0.0f, 0.0f });
    }
    auto tok_arr = cheese_token_data_array{ cur.data(), cur.size(), -1, false };

    for (const auto token : tokens) {
        for (cheese_token token_id = 0; token_id < (cheese_token) n_vocab; token_id++) {
            cur[token_id].logit = 0.0f;
        }
        cheese_sampler_apply(grammar, &tok_arr);
        if (cur[token].logit < 0.0f) {
            return false;
        }
        cheese_sampler_accept(grammar, token);
    }
...
static void test(const std::string & test_desc, const std::string & grammar_str,
    ...
    auto * grammar = cheese_sampler_init_llg(vocab, "lark", grammar_str.c_str());
...
static void one_hot(cheese_token_data_array & tok_arr, cheese_token selected) {
...
static void test_sampler_chain(void) {
    ...
    cheese_sampler * sampler = cheese_sampler_chain_init(sparams);

    const auto grammar_data = R"(%llguidance {}
start: /[A-Z ]*/)";

    cheese_sampler_chain_add(sampler, cheese_sampler_init_llg(vocab, "lark", grammar_data));
    cheese_sampler_chain_add(sampler, cheese_sampler_init_dist(42));
...
int main(int argc, const char ** argv) {
    ...
    // load the vocab
    {
        ...
        model = cheese_model_load_from_file(vocab_file, mparams);
        ...
        ctx = cheese_init_from_model(model, cparams);
