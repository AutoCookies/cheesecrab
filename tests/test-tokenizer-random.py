# Test libcheese tokenizer == AutoTokenizer.
# Brute force random words/text generation.
#
# Sample usage:
#
#   python3 tests/test-tokenizer-random.py ./models/ggml-vocab-cheese-bpe.gguf ./models/tokenizers/cheese-bpe
#
...
class LibCheese:
    ...
    DEFAULT_PATH_CHEESE_H = "./include/cheese.h"
    DEFAULT_PATH_INCLUDES = ["./ggml/include/", "./include/"]
    DEFAULT_PATH_LIBCHEESE = "./build/src/libcheese.so"  # CMakeLists.txt: BUILD_SHARED_LIBS ON

    def __init__(self, path_cheese_h: str | None = None, path_includes: list[str] = [], path_libcheese: str | None = None):
        path_cheese_h = path_cheese_h or self.DEFAULT_PATH_CHEESE_H
        path_includes = path_includes or self.DEFAULT_PATH_INCLUDES
        path_libcheese = path_libcheese or self.DEFAULT_PATH_LIBCHEESE
        (self.ffi, self.lib) = self._load_libcheese_cffi(path_cheese_h, path_includes, path_libcheese)
        self.lib.cheese_backend_init()

    def _load_libcheese_cffi(self, path_cheese_h: str, path_includes: list[str], path_libcheese: str) -> tuple[cffi.FFI, Any]:
        cmd = ["gcc", "-O0", "-E", "-P", "-D__restrict=", "-D__attribute__(x)=", "-D__asm__(x)="]
        cmd += ["-I" + path for path in path_includes] + [path_cheese_h]
        ...
        lib = ffi.dlopen(path_libcheese)
        return (ffi, lib)

    def model_default_params(self, **kwargs):
        mparams = self.lib.cheese_model_default_params()
        for k, v in kwargs.items():
            setattr(mparams, k, v)
        return mparams

    def context_default_params(self, **kwargs):
        cparams = self.lib.cheese_context_default_params()
        for k, v in kwargs.items():
            setattr(cparams, k, v)
        return cparams


class LibCheeseModel:

    def __init__(self, libcheese: LibCheese, path_model: str, mparams={}, cparams={}):
        self.lib: Any = libcheese.lib
        self.ffi = libcheese.ffi
        if isinstance(mparams, dict):
            mparams = libcheese.model_default_params(**mparams)
        self.model = self.lib.cheese_model_load_from_file(path_model.encode(), mparams)
        if not self.model:
            raise RuntimeError("error: failed to load model '%s'" % path_model)
        if isinstance(cparams, dict):
            cparams = libcheese.context_default_params(**cparams)
        self.ctx = self.lib.cheese_new_context_with_model(self.model, cparams)
        if not self.ctx:
            raise RuntimeError("error: failed to create context for model '%s'" % path_model)
        n_tokens_max = self.lib.cheese_n_ctx(self.ctx)
        self.token_ids = self.ffi.new("cheese_token[]", n_tokens_max)
        self.text_buff = self.ffi.new("uint8_t[]", 1024)

    def free(self):
        if self.ctx:
            self.lib.cheese_free(self.ctx)
        if self.model:
            self.lib.cheese_model_free(self.model)
        self.ctx = None
        self.model = None
        self.lib = None
    ...
    def tokenize(self, text: str, add_special: bool = False, parse_special: bool = False) -> list[int]:
        ...
        num = self.lib.cheese_tokenize(self.model, encoded_text, len(encoded_text), self.token_ids, len(self.token_ids), add_special, parse_special)
        while num < 0 and len(self.token_ids) < (16 << 20):
            self.token_ids = self.ffi.new("cheese_token[]", -2 * num)
            num = self.lib.cheese_tokenize(self.model, encoded_text, len(encoded_text), self.token_ids, len(self.token_ids), add_special, parse_special)
    ...
    def detokenize(self, ids: list[int], remove_special: bool = False, unparse_special: bool = False) -> str:
        if len(self.token_ids) < len(ids):
            self.token_ids = self.ffi.new("cheese_token[]", 2 * len(ids))
        for i, id in enumerate(ids):
            self.token_ids[i] = id
        num = self.lib.cheese_detokenize(self.model, self.token_ids, len(ids), self.text_buff, len(self.text_buff), remove_special, unparse_special)
...
class TokenizerCheeseCpp (Tokenizer):

    libcheese: LibCheese | None = None

    def __init__(self, vocab_file: str):
        if not self.libcheese:
            self.libcheese = LibCheese()
        self.model = LibCheeseModel(self.libcheese, vocab_file, mparams=dict(vocab_only=True), cparams=dict(n_ctx=4096))
...
def generator_custom_text_edge_cases() -> Iterator[str]:
    ...
    yield from [
        ...
        'Cửa Việt',   # cheese-3, ignore_merges = true
        '<s>a',       # Phi-3 fail
...
def compare_tokenizers(tokenizer1: TokenizerGroundtruth, tokenizer2: TokenizerCheeseCpp, generator: Iterator[str]):
...
def main(argv: list[str] | None = None):
    ...
    tokenizer2 = TokenizerCheeseCpp(args.vocab_file)
