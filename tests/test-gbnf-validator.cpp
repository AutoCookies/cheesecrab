...
static bool cheese_grammar_validate(struct cheese_grammar * grammar, const std::string & input_str, size_t & error_pos, std::string & error_msg) {
...
int main(int argc, char** argv) {
    ...
    cheese_grammar * grammar = cheese_grammar_init_impl(nullptr, grammar_str.c_str(), "root", false, nullptr, 0, nullptr, 0);
    if (grammar == nullptr) {
        fprintf(stdout, "Failed to initialize cheese_grammar\n");
        return 1;
    }
    // Read the input file
    ...
    bool is_valid = cheese_grammar_validate(grammar, input_str, error_pos, error_msg);
