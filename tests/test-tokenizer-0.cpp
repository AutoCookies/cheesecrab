...
//static const std::map<std::string, std::vector<cheese_token>> & k_tests() {
//    static std::map<std::string, std::vector<cheese_token>> _k_tests = {
//        { ""                      , {  }, },
//        { " "                     , {     220, }, },
//        { "  "                    , {     256, }, },
//        { "   "                   , {     262, }, },
...
using cheese_tests = std::map<std::string, std::vector<cheese_token>>;

static cheese_tests read_tests(const std::string & fname_inp, const std::string & fname_out) {
    cheese_tests tests;
...
int main(int argc, char **argv) {
    ...
    // load the vocab
    {
        ...
        model = cheese_model_load_from_file(fname.c_str(), mparams);
    ...
    const auto k_tests = [&]() -> cheese_tests {
        if (!fname_text.empty()) {
            return {};
        }
    ...
    for (int i = 0; i < nthread; i++) {
        threads[i] = std::thread([&, i]() {
            for (const auto & test_kv : k_tests) {
                const std::vector<cheese_token> res = common_tokenize(ctx, test_kv.first, add_special, false);
    ...
    if (!fname_text.empty()) {
...

        {
            ...
            for (const auto & tok : res) {
                //ofs << tok << " '" << string_strip(cheese_detokenize(ctx, std::vector<int>{tok})) << "'" << std::endl;
                ofs << tok << "\n";
            }
        }
