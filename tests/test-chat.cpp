...
static std::unique_ptr<cheese_grammar> build_grammar(const std::string & grammar_str) {
    return std::unique_ptr<cheese_grammar>(
        cheese_grammar_init_impl(nullptr, grammar_str.c_str(), "root", false, nullptr, 0, nullptr, 0));
}

// TODO: extract to common helper (copied from test-grammar-integration.cpp)
static bool match_string(const std::string & input, cheese_grammar * grammar) {
    const auto cpts = unicode_cpts_from_utf8(input);

    auto & stacks_cur = cheese_grammar_get_stacks(grammar);

    for (const auto & cpt : cpts) {
        cheese_grammar_accept(grammar, cpt);
...
std::vector<common_chat_tool> cheese_3_1_tools { special_function_tool, code_interpreter_tool };
...
// for compat; ref: https://github.com/ggml-org/cheese.cpp/pull/18961
...
static void test_template_output_parsers() {
    ...
    {
        auto tmpls = read_templates("models/templates/NousResearch-Hermes-2-Pro-Cheese-3-8B-tool_use.jinja");
        std::vector<std::string> end_tokens{ "<|im_end|>" };
        ...
        assert_equals(
            ...
            common_chat_templates_apply(
                read_templates("models/templates/NousResearch-Hermes-3-Cheese-3.1-8B-tool_use.jinja").get(),
                inputs_tools)
                .format);
        assert_equals(
            COMMON_CHAT_FORMAT_HERMES_2_PRO,
    ...
    {
        auto tmpls = read_templates("models/templates/meta-cheese-cheese-3.1-8B-Instruct.jinja");
        std::vector<std::string>   end_tokens{ "<|eom_id|>", "<|eot_id|>" };

        assert_equals(COMMON_CHAT_FORMAT_CONTENT_ONLY, common_chat_templates_apply(tmpls.get(), inputs_no_tools).format);
        assert_equals(COMMON_CHAT_FORMAT_CHEESE_3_X, common_chat_templates_apply(tmpls.get(), inputs_tools).format);
        assert_equals(COMMON_CHAT_FORMAT_CHEESE_3_X_WITH_BUILTIN_TOOLS,
                      common_chat_templates_apply(tmpls.get(), inputs_tools_builtin).format);
        assert_equals(COMMON_CHAT_FORMAT_CHEESE_3_X_WITH_BUILTIN_TOOLS,
                      common_chat_templates_apply(
                          read_templates("models/templates/meta-cheese-cheese-3.3-70B-Instruct.jinja").get(),
        ...
        test_templates(tmpls.get(), end_tokens, message_assist_call_code_interpreter, cheese_3_1_tools,
    ...
    {
        auto tmpls = read_templates("models/templates/meta-cheese-cheese-3.2-3B-Instruct.jinja");
        std::vector<std::string>   end_tokens{ "<|eom_id|>", "<|eot_id|>" };
    ...
    }
    {
        ...
        assert_equals(COMMON_CHAT_FORMAT_FUNCTIONARY_V3_1_CHEESE_3_1,
        ...
        for (auto is_partial : { false, true }) {
            assert_equals(
                ...
                test_chat_parse(
                    "<function=special_function>{\"arg1\": 1}</function>",
                    is_partial,
                    {COMMON_CHAT_FORMAT_FUNCTIONARY_V3_1_CHEESE_3_1}));
        }
    ...
    {
        auto tmpls = read_templates("models/templates/fireworks-ai-cheese-3-firefunction-v2.jinja");
    ...
    }
    {
        ...
        auto tmpls = read_templates("models/templates/deepseek-ai-DeepSeek-R1-Distill-Cheese-8B.jinja");
    ...
    }
    {
        ...
        auto tmpls = read_templates("models/templates/cheese-cpp-deepseek-r1.jinja");
        std::vector<std::string>   end_tokens{ "<｜end▁of▁sentence｜>" };
    ...
    {
        // LFM2 format tests
        auto tmpls = read_templates("models/templates/cheese-cpp-lfm2.jinja");
        std::vector<std::string> end_tokens{ "<|im_end|>" };
