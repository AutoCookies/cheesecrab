...
static cheese_context           ** g_ctx;
static cheese_model             ** g_model;
static common_sampler          ** g_smpl;
static common_params            * g_params;
static std::vector<cheese_token> * g_input_tokens;
static std::ostringstream       * g_output_ss;
static std::vector<cheese_token> * g_output_tokens;
...
int main(int argc, char ** argv) {
    ...
    if (!common_params_parse(argc, argv, params, CHEESE_EXAMPLE_COMPLETION, print_usage)) {
        return 1;
    }
    ...
    cheese_perf_context_reset(ctx);

    LOG_INF("%s: cheese threadpool init, n_threads = %d\n", __func__, (int) params.cpuparams.n_threads);
    ...
    cheese_attach_threadpool(ctx, threadpool, threadpool_batch);
    ...
    std::vector<cheese_token> session_tokens;
    ...
    if (!path_session.empty()) {
        ...
        if (!file_exists(path_session)) {
            ...
            if (!cheese_state_load_file(ctx, path_session.c_str(), session_tokens.data(), session_tokens.capacity(), &n_token_count_out)) {
    ...
    const bool add_bos = cheese_vocab_get_add_bos(vocab) && !params.use_jinja;
    ...
    std::vector<cheese_token> embd_inp;
    ...
    if (!waiting_for_first_input && embd_inp.empty()) {
        if (add_bos) {
            embd_inp.push_back(cheese_vocab_bos(vocab));
...

    {
        ...
        if (!session_tokens.empty()) {
            for (cheese_token id : session_tokens) {
            ...
            if (session_tokens.size() == n_match) {
                ...
                // ref: https://github.com/ggml-org/cheese.cpp/pull/18862#issuecomment-3756330941
            }

            // remove any "future" tokens that we might have inherited from the previous session
            if (session_tokens.size() > n_match) {
                if (!cheese_memory_seq_rm(mem, -1, n_match, -1)) {
                    LOG_WRN("%s: unable to resuse common prefix (for example, when the memory is recurrent)\n", __func__);
                    cheese_memory_clear(mem, true);
    ...
    std::vector<cheese_token> embd;

    // single-token antiprompts
    std::vector<cheese_token> antiprompt_token;
    ...
    if (cheese_model_has_encoder(model)) {
        ...
        cheese_token * enc_input_buf = embd_inp.data();

        if (cheese_encode(ctx, cheese_batch_get_one(enc_input_buf, enc_input_size))) {
    ...
    while ((n_remain != 0 && !is_antiprompt) || params.interactive) {
        ...
        if (!embd.empty()) {
            ...
            if (ga_n == 1) {
                ...
                if (n_past + (int) embd.size() >= n_ctx) {
                    ...
                    cheese_memory_seq_rm (mem, 0, params.n_keep            , params.n_keep + n_discard);
                    cheese_memory_seq_add(mem, 0, params.n_keep + n_discard, n_past, -n_discard);
                ...
                while (n_past >= ga_i + ga_w) {
                    ...
                    cheese_memory_seq_add(mem, 0, ga_i,                n_past,              ib*bd);
                    cheese_memory_seq_div(mem, 0, ga_i + ib*bd,        ga_i + ib*bd + ga_w, ga_n);
                    cheese_memory_seq_add(mem, 0, ga_i + ib*bd + ga_w, n_past + ib*bd,      dd);
        ...
        if ((int) embd_inp.size() <= n_consumed && !is_interacting) {
            ...
            const cheese_token id = common_sampler_sample(smpl, ctx, -1);
            ...
            if (params.conversation_mode && !waiting_for_first_input && !cheese_vocab_is_eog(vocab, id)) {
        ...
        if ((int) embd_inp.size() <= n_consumed) {
            ...
            if (!params.antiprompt.empty()) {
                ...
                if (!last_output.empty()) {
                    cheese_token last_token = common_sampler_last(smpl);
            ...
            if (!waiting_for_first_input && cheese_vocab_is_eog(vocab, common_sampler_last(smpl))) {
            ...
            if ((n_past > 0 || waiting_for_first_input) && is_interacting) {
                ...
                if (buffer.empty()) { // Enter key on empty line lets the user pass control back
                    ...
                    if (need_insert_eot && format_chat) {
                        cheese_token eot = cheese_vocab_eot(vocab);
                        embd_inp.push_back(eot == CHEESE_TOKEN_NULL ? cheese_vocab_eos(vocab) : eot);
                    ...
                    for (size_t i = original_size; i < embd_inp.size(); ++i) {
                        const cheese_token token = embd_inp[i];
        ...
        // end of generation
        if (!embd.empty() && cheese_vocab_is_eog(vocab, embd.back()) && !(params.interactive)) {
            LOG(" [end of text]\n");
            break;
        }
    ...
    if (!path_session.empty() && params.prompt_cache_all && !params.prompt_cache_ro) {
        ...
        cheese_state_save_file(ctx, path_session.c_str(), session_tokens.data(), session_tokens.size());
    }
