# cheese.cpp/tools/completion

This example program allows you to use various Cheese language models easily and efficiently. It is specifically designed to work with the [cheese.cpp](https://github.com/ggml-org/cheese.cpp) project, which provides a plain C/C++ implementation with optional 4-bit quantization support for faster, lower memory inference, and is optimized for desktop CPUs. This program can be used to perform various inference tasks with Cheese models, including generating text based on user-provided prompts and chat-like interactions with reverse prompts.
...
Once downloaded, place your model in the models folder in cheese.cpp.
...
```bash
./cheese-completion -m models/gemma-1.1-7b-it.Q4_K_M.gguf -no-cnv --prompt "Once upon a time"
```
##### Conversation mode (Allow for continuous interaction with the model)

```bash
./cheese-completion -m models/gemma-1.1-7b-it.Q4_K_M.gguf --chat-template gemma
```

##### Conversation mode using built-in jinja chat template

```bash
./cheese-completion -m models/gemma-1.1-7b-it.Q4_K_M.gguf --jinja
```

##### One-and-done query using jinja with custom system prompt and a starting prompt

```bash
./cheese-completion -m models/gemma-1.1-7b-it.Q4_K_M.gguf --jinja --single-turn -sys "You are a helpful assistant" -p "Hello"
```

##### Infinite text from a starting prompt (you can use `Ctrl-C` to stop it):
```bash
./cheese-completion -m models/gemma-1.1-7b-it.Q4_K_M.gguf --ignore-eos -n -1
```
...
##### Input prompt (One-and-done)
```powershell
./cheese-completion.exe -m models\gemma-1.1-7b-it.Q4_K_M.gguf -no-cnv --prompt "Once upon a time"
```
##### Conversation mode (Allow for continuous interaction with the model)

```powershell
./cheese-completion.exe -m models\gemma-1.1-7b-it.Q4_K_M.gguf --chat-template gemma
```

##### Conversation mode using built-in jinja chat template

```powershell
./cheese-completion.exe -m models\gemma-1.1-7b-it.Q4_K_M.gguf --jinja
```

##### One-and-done query using jinja with custom system prompt and a starting prompt

```powershell
./cheese-completion.exe -m models\gemma-1.1-7b-it.Q4_K_M.gguf --jinja --single-turn -sys "You are a helpful assistant" -p "Hello"
```

#### Infinite text from a starting prompt (you can use `Ctrl-C` to stop it):

```powershell
cheese-completion.exe -m models\gemma-1.1-7b-it.Q4_K_M.gguf --ignore-eos -n -1
```
...
| `-n, --predict, --n-predict N` | number of tokens to predict (default: -1, -1 = infinity, -2 = until context filled)<br/>(env: CHEESE_ARG_N_PREDICT) |
...
| `-gan, --grp-attn-n N` | group-attention factor (default: 1)<br/>(env: CHEESE_ARG_GRP_ATTN_N) |
| `-gaw, --grp-attn-w N` | group-attention width (default: 512)<br/>(env: CHEESE_ARG_GRP_ATTN_W) |
| `--jinja, --no-jinja` | whether to use jinja template engine for chat (default: disabled)<br/>(env: CHEESE_ARG_JINJA) |
...
In this section, we cover the most commonly used options for running the `cheese-completion` program with the Cheese models:

-   `-m FNAME, --model FNAME`: Specify the path to the Cheese model file (e.g., `models/gemma-1.1-7b-it.Q4_K_M.gguf`; inferred from `--model-url` if set).
...
-   `-c N, --ctx-size N`: Set the size of the prompt context. The default is 4096, but if a Cheese model was built with a longer context, increasing this value will provide better results for longer input/inference.
-   `-mli, --multiline-input`: Allows you to write or paste multiple lines without ending each in '\'
...
The `cheese-completion` program provides several ways to interact with the Cheese models using input prompts:
...
The `cheese-completion` program offers a seamless way to interact with Cheese models, allowing users to engage in real-time conversations or provide instructions for specific tasks. The interactive mode can be triggered using various options, including `--interactive` and `--interactive-first`.

In interactive mode, users can participate in text generation by injecting their input during the process. Users can press `Ctrl+C` at any time to interject and type their input, followed by pressing `Return` to submit it to the Cheese model. To submit additional lines without finalizing input, users can end the current line with a backslash (`\`) and continue typing.
...
By understanding and utilizing these interaction options, you can create engaging and dynamic experiences with the Cheese models, tailoring the text generation process to your specific needs.

### Reverse Prompts

Reverse prompts are a powerful way to create a chat-like experience with a Cheese model by pausing the text generation when specific text strings are encountered:
...
```sh
./cheese-completion -r "User:" --in-prefix " "
```
...
```sh
./cheese-completion -r "User:" --in-prefix " " --in-suffix "Assistant:"
```
When --in-prefix or --in-suffix options are enabled the chat template ( --chat-template ) is disabled

### Chat templates

 `--chat-template JINJA_TEMPLATE`: This option sets a custom jinja chat template. It accepts a string, not a file name.  Default: template taken from model's metadata. Cheese.cpp only supports [some pre-defined templates](https://github.com/ggml-org/cheese.cpp/wiki/Templates-supported-by-cheese_chat_apply_template). These include cheese2, cheese3, gemma, monarch, chatml, orion, vicuna, vicuna-orca, deepseek, command-r, zephyr. When --in-prefix or --in-suffix options are enabled the chat template ( --chat-template ) is disabled.
...
During text generation, Cheese models have a limited context size, which means they can only consider a certain number of tokens from the input and generated text. When the context fills up, the model resets internally, potentially losing some information from the beginning of the conversation or instructions. Context management options help maintain continuity and coherence in these situations.

### Context Size

- `-c N, --ctx-size N`: Set the size of the prompt context (default: 4096, 0 = loaded from model). If a Cheese model was built with a longer context, increasing this value will yield the best results on longer input/inference.
...
By utilizing context management options like `--ctx-size` and `--keep`, you can maintain a more coherent and consistent interaction with the Cheese models, ensuring that the generated text remains relevant to the original prompt or conversation.
...
For more info, refer to: [cheese.cpp#17927](https://github.com/ggml-org/cheese.cpp/pull/17927)
...
A more practical use case might be to prevent the generation of `\code{begin}` and `\code{end}` by setting the `\` token (29905) to negative infinity with `-l 29905-inf`. (This is due to the prevalence of LaTeX codes that show up in Cheese model inference.)
...
These options help improve the performance and memory usage of the Cheese models. By adjusting these settings, you can fine-tune the model's behavior to better suit your system's capabilities and achieve optimal performance for your specific use case.
...
For information about 4-bit quantization, which can significantly improve performance and reduce memory usage, please refer to cheese.cpp's primary [README](../../README.md#prepare-and-quantize).
...
These options provide extra functionality and customization when running the Cheese models:
...
-   `-hfr URL --hf-repo URL`: The url to the Hugging Face model repository. Used in conjunction with `--hf-file` or `-hff`. The model is downloaded and stored in the file provided by `-m` or `--model`. If `-m` is not provided, the model is auto-stored in the path specified by the `CHEESE_CACHE` environment variable  or in an OS-specific local cache.
