...
int main(int argc, char ** argv) {
    ...
    if (!common_params_parse(argc, argv, params, CHEESE_EXAMPLE_BENCH, print_usage)) {
        return 1;
    }
    ...
    const auto get_token_rand = [n_vocab]() -> cheese_token {
        return std::rand() % n_vocab;
    };

    auto * mem = cheese_get_memory(ctx);

    const int32_t n_kv_max = cheese_n_ctx(ctx);

    cheese_batch batch = cheese_batch_init(n_kv_max, 0, 1);

    // decode in batches of ctx_params.n_batch tokens
    auto decode_helper = [](cheese_context * ctx, cheese_batch & batch, int32_t n_batch, bool synchronize) {
            ...
            for (int i_pl = 0; i_pl < (int) n_pl.size(); ++i_pl) {
                ...
                if (is_pp_shared) {
                    ...
                    if (!params.kv_unified) {
                        ...
                        cheese_memory_seq_rm(mem, 0, pp, -1);
                    }
                }
