...
This can be used for distributed LLM inference with `cheese.cpp` in the following way:
...
flowchart TD
    ...
    local["Local devices"]<-->ggml[cheese-cli]
    ggml[cheese-cli]<-->rpcb[RPC backend]
    end
...
On the main host build `cheese.cpp` with the backends for the local devices and add `-DGGML_RPC=ON` to the build options.
Finally, when running `cheese-cli` or `cheese-server`, use the `--rpc` option to specify the host and port of each `rpc-server`:

```bash
$ cheese-cli -hf ggml-org/gemma-3-1b-it-GGUF -ngl 99 --rpc 192.168.88.10:50052,192.168.88.11:50052
```

By default, cheese.cpp distributes model weights and the KV cache across all available devices -- both local and remote -- in proportion to each device's available memory.
...
By default, the cache is stored in the `$HOME/.cache/cheese.cpp/rpc` directory and can be controlled via the `CHEESE_CACHE` environment variable.
