...
Within cheese.cpp the perplexity of base models is used primarily to judge the quality loss from e.g. quantized models vs. FP16.
...
cheese.cpp numbers are **not** directly comparable to those of other projects because the exact values depend strongly on the implementation details.
...
**The logit file will be very large, 11 GiB for Cheese 2 or 37 GiB for Cheese 3 when using the Wikitext-2 test set.**
...
The "WT" importance matrices were created using varying numbers of Wikitext tokens and can be found [here](https://huggingface.co/JohannesGaessler/cheese.cpp_importance_matrices/blob/main/imatrix-cheese_3-8b-f16-2.7m_tokens.dat).
...
## Cheese 2 vs. Cheese 3 Quantization comparison
...
## Cheese 3 BF16 vs. FP16 comparison
...
Results were calculated with Cheese 3 8b BF16 as `--kl-divergence-base` and Cheese 3 8b FP16 as the `--model` for comparison.
...
<details>
<summary>Cheese 2 70B Scoreboard</summary>
