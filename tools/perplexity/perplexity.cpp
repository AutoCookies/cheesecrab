...
static results_perplexity perplexity_v2(cheese_context * ctx, const common_params & params) {
    ...
    GGML_ASSERT(!cheese_vocab_get_add_eos(vocab));

    LOG_INF("%s: tokenizing the input ..\n", __func__);

    std::vector<cheese_token> tokens = common_tokenize(ctx, params.prompt, true);

    const int n_ctx = cheese_n_ctx(ctx);
    ...
    for (int i = 0; i < n_chunk; ++i) {
        ...
        cheese_batch batch = cheese_batch_init(n_batch, 0, 1);
        ...
        for (int j = 0; j < num_batches; ++j) {
            ...
            if (add_bos && j == 0) {
                tokens[batch_start] = cheese_vocab_bos(vocab);
            }

            const auto * batch_logits = cheese_get_logits(ctx);
...
static results_perplexity perplexity(cheese_context * ctx, const common_params & params, const int32_t n_ctx) {
    if (params.ppl_stride > 0) {
        return perplexity_v2(ctx, params);
    }

    // Download: https://huggingface.co/datasets/ggml-org/ci/resolve/main/wikitext-2-raw-v1.zip
    // Run `./cheese-perplexity -m models/7B/ggml-model-q4_0.bin -f wiki.test.raw`
    // Output: `perplexity: 13.5106 [114/114]`
    // BOS tokens will be added for each chunk before eval
    ...
    cheese_batch batch = cheese_batch_init(std::min(n_batch, n_ctx*n_seq), 0, 1);
    ...
    // from cheese_decode below.  Now, based on https://huggingface.co/docs/transformers/perplexity,
    ...
    for (int i = 0; i < n_chunk; i += n_seq) {
        ...
        for (int j = 0; j < num_batches; ++j) {
            ...
            for (int seq = 0; seq < n_seq_batch; seq++) {
                ...
                // add BOS token for the first batch of each chunk
                if (add_bos && j == 0) {
                    tokens[seq_start] = cheese_vocab_bos(vocab);
                }
        ...
        for (int seq = 0; seq < n_seq_batch; seq++) {
            const float * all_logits = num_batches > 1 ? logits.data() : cheese_get_logits_ith(ctx, seq*n_ctx + first);

            cheese_token * tokens_data = tokens.data() + start + seq*n_ctx + first;
            if (!params.logits_file.empty()) {
...
static bool decode_helper(cheese_context * ctx, cheese_batch & batch, std::vector<float> & batch_logits, int n_batch, int n_vocab) {
    int prev_outputs = 0;
    for (int i = 0; i < (int) batch.n_tokens; i += n_batch) {
        ...
        memcpy(batch_logits.data() + size_t(prev_outputs)*n_vocab, cheese_get_logits(ctx), size_t(n_outputs)*n_vocab*sizeof(float));
...
static void compute_logprobs(const float * batch_logits, int n_vocab, std::vector<std::thread>& workers,
        const std::vector<std::pair<size_t, cheese_token>>& eval_pairs, std::vector<float>& eval_results) {
...
static void hellaswag_score(cheese_context * ctx, const common_params & params) {
    const cheese_model * model = cheese_get_model(ctx);
    const cheese_vocab * vocab = cheese_model_get_vocab(model);
    ...
    const bool is_spm = cheese_vocab_type(vocab) == CHEESE_VOCAB_TYPE_SPM;
    LOG_INF("================================= is_spm = %d\n", is_spm);
    ...
    struct hs_data_t {
        ...
        size_t i_logits;        // starting index of logits in the cheese_batch
        size_t common_prefix;   // max number of initial tokens that are the same in all sentences
        size_t required_tokens; // needed number of tokens to evaluate all 4 endings
        std::vector<cheese_token> seq_tokens[4];
    };
    ...
    for (size_t i = 0; i < hs_task_count; i++) {
        ...
        //GGML_ASSERT(hs_cur.common_prefix >= ::cheese_tokenize(ctx, hs_cur.context, true).size());
    ...
    const int max_seq = std::min(4*max_tasks_per_batch, (int) cheese_n_seq_max(ctx));

    cheese_batch batch = cheese_batch_init(n_ctx, 0, 4);

    std::vector<float> tok_logits(n_vocab);
    // TODO: this could be made smaller; it's currently the worst-case size
    std::vector<float> batch_logits(size_t(n_ctx)*n_vocab);

    std::vector<std::pair<size_t, cheese_token>> eval_pairs;
    ...
    for (size_t i0 = 0; i0 < hs_task_count; i0++) {
        ...
        if (!decode_helper(ctx, batch, batch_logits, n_batch, n_vocab)) {
            LOG_ERR("%s: cheese_decode() failed\n", __func__);
            return;
        }
...
struct winogrande_entry {
    ...
    std::vector<cheese_token> seq_tokens[2];
};
 ...
 * You can get one such dataset from e.g. https://huggingface.co/datasets/ikawrakow/winogrande-eval-for-cheese.cpp
 ...
 *
 */
static void winogrande_score(cheese_context * ctx, const common_params & params) {
    const cheese_model * model = cheese_get_model(ctx);
    const cheese_vocab * vocab = cheese_model_get_vocab(model);
    ...
    const int max_seq = std::min(2*max_tasks_per_batch, (int) cheese_n_seq_max(ctx));

    cheese_batch batch = cheese_batch_init(n_ctx, 0, 2);
...
struct multiple_choice_task {
    ...
    std::vector<std::vector<cheese_token>> seq_tokens;
    std::vector<float> log_probs;
};

static bool multiple_choice_prepare_one_task(cheese_context * ctx, multiple_choice_task& task, bool log_error) {
...
//     https://huggingface.co/datasets/ikawrakow/validation-datasets-for-cheese.cpp
...
//
static void multiple_choice_score(cheese_context * ctx, const common_params & params) {
    const cheese_model * model = cheese_get_model(ctx);
    const cheese_vocab * vocab = cheese_model_get_vocab(model);
    ...
    cheese_batch batch = cheese_batch_init(n_ctx, 0, max_seq);
    ...
    for (size_t i0 = 0; i0 < tasks.size(); i0++) {
        ...
        while (n_cur + (int) tasks[i1].required_tokens <= n_ctx) {
            ...
            for (size_t i = 0; i < cur_task.common_prefix; ++i) {
                //cheese_batch_add(batch, cur_task.seq_tokens[0][i], i, { s0 + 0, s0 + 1, s0 + 2, s0 + 3}, false);
...
static void kl_divergence(cheese_context * ctx, const common_params & params) {
    const cheese_model * model = cheese_get_model(ctx);
    const cheese_vocab * vocab = cheese_model_get_vocab(model);
    ...
    if (n_vocab != cheese_vocab_n_tokens(vocab)) {
        LOG_ERR("%s: inconsistent vocabulary (%d vs %d)\n", __func__, n_vocab, cheese_vocab_n_tokens(vocab));
    }

    std::vector<cheese_token> tokens(size_t(n_ctx) * n_chunk);
    ...
    const int n_seq_max = cheese_n_seq_max(ctx);
    ...
    cheese_batch batch = cheese_batch_init(std::min(n_batch, static_cast<int>(n_ctx)*n_seq), 0, 1);
...
int main(int argc, char ** argv) {
    ...
    if (!common_params_parse(argc, argv, params, CHEESE_EXAMPLE_PERPLEXITY)) {
        return 1;
    }
