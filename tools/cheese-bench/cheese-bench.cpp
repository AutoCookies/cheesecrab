...
static bool tensor_buft_override_equal(const cheese_model_tensor_buft_override& a, const cheese_model_tensor_buft_override& b) {
...
static bool vec_tensor_buft_override_equal(const std::vector<cheese_model_tensor_buft_override>& a, const std::vector<cheese_model_tensor_buft_override>& b) {
    if (a.size() != b.size()) {
        return false;
    }
...
static bool vec_vec_tensor_buft_override_equal(const std::vector<std::vector<cheese_model_tensor_buft_override>>& a, const std::vector<std::vector<cheese_model_tensor_buft_override>>& b) {
    if (a.size() != b.size()) {
        return false;
    }
    for (size_t i = 0; i < a.size(); i++) {
...
static const char * split_mode_str(cheese_split_mode mode) {
    switch (mode) {
        case CHEESE_SPLIT_MODE_NONE:
            return "none";
        case CHEESE_SPLIT_MODE_LAYER:
            return "layer";
        case CHEESE_SPLIT_MODE_ROW:
            return "row";
        default:
...
struct cmd_params {
    ...
    std::vector<cheese_split_mode>    split_mode;
    std::vector<int>                 main_gpu;
    std::vector<bool>                no_kv_offload;
    std::vector<bool>                flash_attn;
    std::vector<std::vector<ggml_backend_dev_t>> devices;
    std::vector<std::vector<float>>  tensor_split;
    std::vector<std::vector<cheese_model_tensor_buft_override>> tensor_buft_overrides;
...
static const cmd_params cmd_params_defaults = {
    ...
    /* flash_attn           */ { false },
    /* devices              */ { {} },
    /* tensor_split         */ { std::vector<float>(cheese_max_devices(), 0.0f) },
    /* tensor_buft_overrides*/ { std::vector<cheese_model_tensor_buft_override>{ { nullptr, nullptr } } },
    /* use_mmap             */ { false },
    /* use_direct_io        */ { false },
    /* embeddings           */ { false },
    /* no_op_offload        */ { false },
...
static cmd_params parse_cmd_params(int argc, char ** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        try {
            if (arg == "-h" || arg == "--help") {
            ...
            } else if (cheese_supports_rpc() && (arg == "-rpc" || arg == "--rpc")) {
                if (++i >= argc) {
                    invalid_param = true;
                    break;
                }
                ...
                std::vector<cheese_split_mode> modes;
                for (const auto & m : p) {
                    cheese_split_mode mode;
                    if (m == "none") {
                        mode = CHEESE_SPLIT_MODE_NONE;
                    } else if (m == "layer") {
                        mode = CHEESE_SPLIT_MODE_LAYER;
                    } else if (m == "row") {
                        mode = CHEESE_SPLIT_MODE_ROW;
                    } else {
                        invalid_param = true;
                        break;
                    }
                ...
                for (auto ts : string_split<std::string>(argv[i], split_delim)) {
                    ...
                    GGML_ASSERT(split_arg.size() <= cheese_max_devices());

                    std::vector<float> tensor_split(cheese_max_devices());
                ...
                do {
                    ...
                    std::vector<cheese_model_tensor_buft_override> group_tensor_buft_overrides{};
...
struct cmd_params_instance {
    ...
    cheese_model_params to_cheese_mparams() const {
        ...
        if (n_cpu_moe <= 0) {
            ...
            static std::vector<cheese_model_tensor_buft_override> merged;
    ...
    cheese_context_params to_cheese_cparams() const {
        ...
        cparams.flash_attn_type = flash_attn ? CHEESE_FLASH_ATTN_TYPE_ENABLED : CHEESE_FLASH_ATTN_TYPE_DISABLED;
...
struct test {
    ...
    test(const cmd_params_instance & inst, const cheese_model * lmodel, const cheese_context * ctx) :
        cpu_info(get_cpu_info()),
        gpu_info(get_gpu_info()) {

        model_filename = inst.model;
        char buf[128];
        cheese_model_desc(lmodel, buf, sizeof(buf));
        model_type     = buf;
        model_size     = cheese_model_size(lmodel);
        model_n_params = cheese_model_n_params(lmodel);
...
const std::string test::build_commit = CHEESE_COMMIT;
const int         test::build_number = CHEESE_BUILD_NUMBER;
...
struct sql_printer : public printer {
    ...
    void print_header(const cmd_params & params) override {
        std::vector<std::string> fields = test::get_fields();
        fprintf(fout, "CREATE TABLE IF NOT EXISTS cheese_bench (\n");
        for (size_t i = 0; i < fields.size(); i++) {
    ...
    void print_test(const test & t) override {
        fprintf(fout, "INSERT INTO cheese_bench (%s) ", join(test::get_fields(), ", ").c_str());
        fprintf(fout, "VALUES (");
        std::vector<std::string> values = t.get_values();
        for (size_t i = 0; i < values.size(); i++) {
...
struct ctx_state {
    ...
    std::vector<uint8_t> buf; // the cheese_context state buffer
};

static bool test_prompt(cheese_context * ctx, int n_prompt, int n_batch, int n_threads) {
    cheese_set_n_threads(ctx, n_threads, n_threads);
    ...
    std::vector<cheese_token> tokens(n_batch);

    int n_processed = 0;

    while (n_processed < n_prompt) {
        int n_tokens = std::min(n_prompt - n_processed, n_batch);
        tokens[0]    = n_processed == 0 && cheese_vocab_get_add_bos(vocab) ? cheese_vocab_bos(vocab) : std::rand() % n_vocab;
        for (int i = 1; i < n_tokens; i++) {
            tokens[i] = std::rand() % n_vocab;
        }
        int res = cheese_decode(ctx, cheese_batch_get_one(tokens.data(), n_tokens));
        if (res != 0) {
...
static bool test_gen(cheese_context * ctx, int n_gen, int n_threads) {
    cheese_set_n_threads(ctx, n_threads, n_threads);
    ...
    cheese_token token = cheese_vocab_get_add_bos(vocab) ? cheese_vocab_bos(vocab) : std::rand() % n_vocab;

    for (int i = 0; i < n_gen; i++) {
        int res = cheese_decode(ctx, cheese_batch_get_one(&token, 1));
        if (res != 0) {
...
static void cheese_null_log_callback(enum ggml_log_level level, const char * text, void * user_data) {
    (void) level;
    (void) text;
    (void) user_data;
}
...
int main(int argc, char ** argv) {
    ...
    // initialize cheese.cpp
    if (!params.verbose) {
        cheese_log_set(cheese_null_log_callback, NULL);
    }
    cheese_backend_init();
    ...
    cheese_model *               lmodel    = nullptr;
    const cmd_params_instance * prev_inst = nullptr;

    // store the cheese_context state at the previous depth that we performed a test
    // ref: https://github.com/ggml-org/cheese.cpp/pull/16944#issuecomment-3478151721
    ctx_state cstate;
    ...
    for (const auto & inst : params_instances) {
        params_idx++;
        if (params.progress) {
            fprintf(stderr, "cheese-bench: benchmark %d/%zu: starting\n", params_idx, params_count);
        }
        // keep the same model between tests when possible
        if (!lmodel || !prev_inst || !inst.equal_mparams(*prev_inst)) {
            if (lmodel) {
                cheese_model_free(lmodel);
            }

            lmodel = cheese_model_load_from_file(inst.model.c_str(), inst.to_cheese_mparams());
            if (lmodel == NULL) {
        ...
        cheese_context * ctx = cheese_init_from_model(lmodel, inst.to_cheese_cparams());
        if (ctx == NULL) {
        ...
        cheese_memory_clear(cheese_get_memory(ctx), false);
        ...
        cheese_attach_threadpool(ctx, threadpool, NULL);

        // warmup run
        if (!params.no_warmup) {
            if (t.n_prompt > 0) {
                if (params.progress) {
                    fprintf(stderr, "cheese-bench: benchmark %d/%zu: warmup prompt run\n", params_idx, params_count);
                }
            ...
            if (t.n_gen > 0) {
                if (params.progress) {
                    fprintf(stderr, "cheese-bench: benchmark %d/%zu: warmup generation run\n", params_idx, params_count);
                }
        ...
        for (int i = 0; i < params.reps; i++) {
            ...
            if (t.n_depth > 0) {
                ...
                if (is_cached) {
                    ...
                    const size_t ret = cheese_state_seq_set_data(ctx, cstate.buf.data(), cstate.buf.size(), 0);
                    if (ret == 0) {
                ...
                if (!is_cached) {
                    if (params.progress) {
                        fprintf(stderr, "cheese-bench: benchmark %d/%zu: depth run %d/%d\n", params_idx, params_count,
                                i + 1, params.reps);
                    }
                    ...
                    cstate.buf.resize(cheese_state_seq_get_size(ctx, 0));
                    cheese_state_seq_get_data(ctx, cstate.buf.data(), cstate.buf.size(), 0);
                } else {
                    if (params.progress) {
                        fprintf(stderr, "cheese-bench: benchmark %d/%zu: depth run %d/%d (cached)\n", params_idx, params_count,
                                i + 1, params.reps);
                    }
                }
            }
            ...
            if (t.n_prompt > 0) {
                if (params.progress) {
                    fprintf(stderr, "cheese-bench: benchmark %d/%zu: prompt run %d/%d\n", params_idx, params_count,
                            i + 1, params.reps);
                }
                bool res = test_prompt(ctx, t.n_prompt, t.n_batch, t.n_threads);
                if (!res) {
            ...
            if (t.n_gen > 0) {
                if (params.progress) {
                    fprintf(stderr, "cheese-bench: benchmark %d/%zu: generation run %d/%d\n", params_idx, params_count,
                            i + 1, params.reps);
                }
