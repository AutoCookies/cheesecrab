...
static void cheese_log_callback_null(ggml_log_level level, const char * text, void * user_data) {
    (void) level;
    (void) text;
    (void) user_data;
}
...
int main(int raw_argc, char ** raw_argv) {
    ...
    if (disable_logging) {
        cheese_log_set(cheese_log_callback_null, NULL);
    }
    ...
    cheese_model * model = cheese_model_load_from_file(model_path, model_params);
    if (!model) {
    ...
    const bool model_wants_add_bos = cheese_vocab_get_add_bos(vocab);
