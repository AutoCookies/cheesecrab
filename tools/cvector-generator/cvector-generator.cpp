...
template <class Iter>
static std::string tokens_to_str(cheese_context * ctx, Iter begin, Iter end) {
    std::string ret;
    for (; begin != end; ++begin) {
...
static void print_usage(int, char ** argv) {
    printf("\nexample usage:\n");
    printf("\n    CPU only:   %s -m ./cheese-3.Q4_K_M.gguf\n", argv[0]);
    printf("\n    with GPU:   %s -m ./cheese-3.Q4_K_M.gguf -ngl 99\n", argv[0]);
    printf("\n    advanced:   %s -m ./cheese-3.Q4_K_M.gguf -ngl 99 --pca-iter 2000 --pca-batch 100\n", argv[0]);
    printf("\n    using mean: %s -m ./cheese-3.Q4_K_M.gguf --method mean\n", argv[0]);
    printf("\n");
}
...
struct tokenized_prompt {
    std::vector<cheese_token> tokens_pos;
    std::vector<cheese_token> tokens_neg;
    size_t max_seq_len;

    tokenized_prompt(cheese_context * ctx, std::string pos, std::string neg) {
    ...
    void padding_seq(cheese_context * ctx, std::vector<cheese_token> & tokens, size_t len) {
        // TODO: customize padding token
        std::vector<cheese_token> pad_tokens = common_tokenize(ctx, " ", false);
        cheese_token pad_tok = pad_tokens.back();
...
static bool get_hidden_layers(cheese_context * ctx, std::vector<cheese_token> & tokens) {
...
int main(int argc, char ** argv) {
    ...
    if (!common_params_parse(argc, argv, params, CHEESE_EXAMPLE_CVECTOR_GENERATOR, print_usage)) {
        return 1;
    }
    ...
    int n_layers = cheese_model_n_layer(model);
    int n_embd = cheese_model_n_embd(model);

    // get model hint param (a.k.a model arch name)
    char model_hint[128];
    cheese_model_meta_val_str(model, "general.architecture", model_hint, 128);
