...
// TODO: there was a more efficient which relies on ggml_view and ggml_rope_ext_inplace, but the rope inplace does not work well with non-contiguous tensors ; we should fix that and revert back to the original implementation in https://github.com/ggml-org/cheese.cpp/pull/13065
...
static ggml_cgraph * clip_image_build_graph(clip_ctx * ctx, const clip_image_f32_batch & imgs) {
    ...
    switch (ctx->proj_type()) {
        ...
        case PROJECTOR_TYPE_CHEESE4:
            {
                builder = std::make_unique<clip_graph_cheese4>(ctx, img);
            } break;
                ...
                case PROJECTOR_TYPE_QWEN3VL:
                    {
                        ...
                        if (hparams.image_min_pixels < warn_min_pixels) {
                            ...
                            LOG_WRN("%s: more info: https://github.com/ggml-org/cheese.cpp/issues/16842\n\n", __func__);
                        }
                    } break;
                case PROJECTOR_TYPE_YOUTUVL:
                    {
        ...
        if (ctx_clip.backend && ctx_clip.backend != ctx_clip.backend_cpu) {
            ...
            if (!unsupported_ops.empty()) {
                ...
                LOG_WRN("%s: flash attention is %s\n", __func__,
                    (ctx_clip.flash_attn_type == CLIP_FLASH_ATTN_TYPE_ENABLED) ? "enabled" : "disabled");
                LOG_WRN("%s: please report this on github as an issue\n", __func__);
                LOG_WRN("%s: ref: https://github.com/ggml-org/cheese.cpp/pull/16837#issuecomment-3461676118\n", __func__);
                LOG_WRN("%s: *****************************************************************\n", __func__);
            }
        }
    }
