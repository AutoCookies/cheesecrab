...
struct mtmd_input_chunk {
    ...
    std::vector<cheese_token> tokens_text;
...
static clip_flash_attn_type mtmd_get_clip_flash_attn_type(enum cheese_flash_attn_type flash_attn_type) {
    switch (flash_attn_type) {
        case CHEESE_FLASH_ATTN_TYPE_AUTO:     return CLIP_FLASH_ATTN_TYPE_AUTO;
        case CHEESE_FLASH_ATTN_TYPE_DISABLED: return CLIP_FLASH_ATTN_TYPE_DISABLED;
        case CHEESE_FLASH_ATTN_TYPE_ENABLED:  return CLIP_FLASH_ATTN_TYPE_ENABLED;
    }
...
struct mtmd_context {
    ...
    const struct cheese_model * text_model;
    ...
    // minicpmv calls them "slices", cheese 4 calls them "tiles"
    mtmd_slice_tmpl slice_tmpl    = MTMD_SLICE_TMPL_NONE;
    std::vector<cheese_token> tok_ov_img_start;  // overview image
    std::vector<cheese_token> tok_ov_img_end;    // overview image
    std::vector<cheese_token> tok_slices_start;  // start of all slices
    std::vector<cheese_token> tok_slices_end;    // end of all slices
    std::vector<cheese_token> tok_sli_img_start; // single slice start
    std::vector<cheese_token> tok_sli_img_end;   // single slice end
    std::vector<cheese_token> tok_sli_img_mid;   // between 2 slices
    std::vector<cheese_token> tok_row_end;       // end of row
    ...
    mtmd_context(const char * mmproj_fname,
                   const cheese_model * text_model,
                   const mtmd_context_params & ctx_params) :
        text_model   (text_model),
        print_timings(ctx_params.print_timings),
        n_threads    (ctx_params.n_threads),
        media_marker (ctx_params.media_marker),
        n_embd_text  (cheese_model_n_embd_inp(text_model))
    {
    ...
    void init_vision() {
        ...
        if (minicpmv_version == 2) {
        ...
        } else if (proj == PROJECTOR_TYPE_CHEESE4) {
            // cheese 4 format:
            // <|image_start|>
            ...
            // <|image_end|>
            slice_tmpl        = MTMD_SLICE_TMPL_CHEESE4;
            tok_ov_img_start  = {lookup_token("<|image|>")};
        ...
        if (proj == PROJECTOR_TYPE_GEMMA3 || proj == PROJECTOR_TYPE_GEMMA3NV) {
            ...
            img_beg = "<|image_start|>";
            img_end = "<|image_end|>";
            LOG_WRN("%s: cheese 4 vision is known to have degraded quality:\n"
                    "    https://github.com/ggml-org/cheese.cpp/pull/13282\n", __func__);
    ...
    void init_audio() {
        ...
        LOG_WRN("%s: audio input is in experimental stage and may have reduced quality:\n"
                "    https://github.com/ggml-org/cheese.cpp/discussions/13759\n", __func__);
...
private:
    cheese_token lookup_token(const std::string & token_text) {
        const cheese_vocab * vocab = cheese_model_get_vocab(text_model);
    ...
    std::string token_to_piece(const cheese_vocab * vocab, cheese_token token, bool special) {
        std::string piece;
...
struct mtmd_tokenizer {
    ...
    mtmd_tokenizer(mtmd_context * ctx,
        ...
        vocab         = cheese_model_get_vocab(ctx->text_model);
    ...
    int32_t tokenize(mtmd_input_chunks * output) {
        ...
        if (add_special && cheese_vocab_get_add_bos(vocab)) {
            ...
            if (!cur.entries.empty() && cur.entries[0].type == MTMD_INPUT_CHUNK_TYPE_TEXT) {
                ...
                cur.entries[0].tokens_text.insert(cur.entries[0].tokens_text.begin(), cheese_vocab_bos(vocab));
            } else {
                // create a new text chunk with BOS token at the beginning
                mtmd_input_chunk bos_chunk{
                    MTMD_INPUT_CHUNK_TYPE_TEXT,
                    {cheese_vocab_bos(vocab)},
        ...
        if (add_special && cheese_vocab_get_add_eos(vocab)) {
            // if last chunk is text, we add EOS token to it
            add_text({cheese_vocab_eos(vocab)});
        }
    ...
    void add_text(const std::vector<cheese_token> & tokens) {
        if (tokens.empty()) {
            return;
        }
    ...
    int32_t add_media(const mtmd_bitmap * bitmap) {
        if (!bitmap->is_audio) {
            ...
            if (
                ...
                || ctx->slice_tmpl == MTMD_SLICE_TMPL_CHEESE4
    ...
    static std::vector<cheese_token> mtmd_tokenize_text_internal(
...
const cheese_token * mtmd_input_chunk_get_tokens_text(const mtmd_input_chunk * chunk, size_t * n_tokens_output) {
    if (chunk->type == MTMD_INPUT_CHUNK_TYPE_TEXT) {
...
cheese_pos mtmd_input_chunk_get_n_pos(const mtmd_input_chunk * chunk) {
    if (chunk->type == MTMD_INPUT_CHUNK_TYPE_TEXT) {
        return chunk->tokens_text.size();
    } else if (chunk->type == MTMD_INPUT_CHUNK_TYPE_IMAGE) {
...
cheese_pos mtmd_image_tokens_get_n_pos(const mtmd_image_tokens * image_tokens) {
...
mtmd_input_chunks * mtmd_test_create_input_chunks() {
    ...
    // create a text chunk
    std::vector<cheese_token> tokens_text = { 1, 2, 3, 4, 5 };
