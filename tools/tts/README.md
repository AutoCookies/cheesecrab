...
## Quickstart
If you have built cheese.cpp with SSL support you can simply run the
following command and the required models will be downloaded automatically:
```console
$ build/bin/cheese-tts --tts-oute-default -p "Hello world" && aplay output.wav
```
...
```console
$ build/bin/cheese-quantize models/outetts-0.2-0.5B-f16.gguf \
...
```console
$ build/bin/cheese-tts -m  ./models/outetts-0.2-0.5B-q8_0.gguf \
...
### Running the example with cheese-server
Running this example with `cheese-server` is also possible and requires two
...
```console
$ ./build/bin/cheese-server -m ./models/outetts-0.2-0.5B-q8_0.gguf --port 8020
```

And the voice decoder model server can be started using:
```console
./build/bin/cheese-server -m ./models/wavtokenizer-large-75-f16.gguf --port 8021 --embeddings --pooling none
```
