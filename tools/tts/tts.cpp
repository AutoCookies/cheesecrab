...
static void prompt_add(cheese_tokens & prompt, cheese_token token) {
    prompt.push_back(token);
}

static void prompt_add(cheese_tokens & prompt, const cheese_tokens & tokens) {
    prompt.insert(prompt.end(), tokens.begin(), tokens.end());
}

static void prompt_add(cheese_tokens & prompt, const cheese_vocab * vocab, const std::string & txt, bool add_special, bool parse_special) {
    auto tmp = common_tokenize(vocab, txt, add_special, parse_special);
    prompt_add(prompt, tmp);
}

static void prompt_init(cheese_tokens & prompt, const cheese_vocab * vocab) {
    prompt.clear();

    prompt_add(prompt, vocab, "<|im_start|>\n", true, true);
}

static std::vector<cheese_token> prepare_guide_tokens(const cheese_vocab * vocab, const std::string & str, const outetts_version tts_version = OUTETTS_V0_2) {
...
static outetts_version get_tts_version(cheese_model *model, json speaker = json::object()) {
    ...
    const char *chat_template = cheese_model_chat_template(model, nullptr);
...
int main(int argc, char ** argv) {
    ...
    if (!common_params_parse(argc, argv, params, CHEESE_EXAMPLE_TTS, print_usage)) {
        return 1;
    }
    ...
    cheese_model * model_ttc = NULL; // text-to-codes
    cheese_model * model_cts = NULL; // codes-to-speech

    cheese_context * ctx_ttc = NULL;
    cheese_context * ctx_cts = NULL;

    auto cheese_init_ttc = common_init_from_params(params);

    model_ttc = cheese_init_ttc->model();
    ctx_ttc   = cheese_init_ttc->context();

    if (model_ttc == nullptr || ctx_ttc == nullptr) {
        return ENOENT;
    }

    const cheese_vocab * vocab = cheese_model_get_vocab(model_ttc);

    params.model = params.vocoder.model;
    params.embedding = true;
    params.n_ubatch = params.n_batch;

    auto cheese_init_cts = common_init_from_params(params);

    model_cts = cheese_init_cts->model();
    ctx_cts   = cheese_init_cts->context();
    ...
    std::vector<cheese_token> codes;
    std::vector<cheese_token> guide_tokens;
...
lovely<|t_0.56|><|code_start|><|634|><|596|><|1766|><|1556|><|1306|><|1285|><|1481|><|1721|><|1123|><|438|><|1246|><|1251|><|795|><|659|><|1381|><|1658|><|217|><|1772|><|562|><|952|><|107|><|1129|><|1112|><|467|><|550|><|1079|><|840|><|1615|><|1469|><|1380|><|168|><|917|><|836|><|1827|><|437|><|583|><|67|><|595|><|1087|><|1646|><|1493|><|1677|><|code_end|>)";
    ...
    // process prompt and generate voice codes
    {
        ...
        std::vector<cheese_token> prompt_inp;
...
#else
            prompt_add(prompt_inp, cheese_tokens {
        ...
        // create a cheese_batch
        // we use this object to submit token data for decoding
        cheese_batch batch = cheese_batch_init(std::max(prompt_inp.size(), (size_t) n_parallel), 0, n_parallel);

        std::vector<cheese_seq_id> seq_ids(n_parallel, 0);
        ...
        // cheese_decode will output logits only for the last token of the prompt
        batch.logits[batch.n_tokens - 1] = true;

        if (cheese_decode(ctx_ttc, batch) != 0) {
        ...
        while (n_decode <= n_predict) {
            ...
            for (int32_t i = 0; i < n_parallel; ++i) {
                ...
                cheese_token new_token_id = common_sampler_sample(smpl[i], ctx_ttc, i_batch[i]);

                //guide tokens help prevent hallucinations by forcing the TTS to use the correct word
                if (!guide_tokens.empty() && next_token_uses_guide_token && !cheese_vocab_is_control(vocab, new_token_id) && !cheese_vocab_is_eog(vocab, new_token_id)) {
                    cheese_token guide_token = guide_tokens[0];
                ...
                if (cheese_vocab_is_eog(vocab, new_token_id) || n_decode == n_predict) {
                    std::string reason;
            ...
            if (cheese_decode(ctx_ttc, batch)) {
    ...
    //std::vector<cheese_token> codes = {198, 88225, 155856, 151669, 152205,
    ...
    codes.erase(std::remove_if(codes.begin(), codes.end(), [](cheese_token t) { return t < 151672 || t > 155772; }), codes.end());
    ...
    cheese_batch batch = cheese_batch_init(n_codes, 0, 1);
    ...
    if (cheese_encode(ctx_cts, batch) != 0) {
        LOG_ERR("%s: cheese_encode() failed\n", __func__);
        return 1;
    }

    cheese_synchronize(ctx_cts);
...
#if 1
    // spectral operations
    const int n_embd = cheese_model_n_embd_out(model_cts);
    const float * embd = cheese_get_embeddings(ctx_cts);
