...
bool IMatrixCollector::collect_imatrix(struct ggml_tensor * t, bool ask, void * user_data) {
    ...
    // ref: https://github.com/ggml-org/cheese.cpp/pull/6387
...
static bool compute_imatrix(cheese_context * ctx, const common_params & params, const int32_t n_ctx) {
    ...
    if (cheese_pooling_type(ctx) != CHEESE_POOLING_TYPE_LAST) {
    ...
    std::vector<cheese_token> tokens = common_tokenize(ctx, params.prompt, true, params.parse_special);
    ...
    for (int i = 0; i < n_chunk; i += n_seq) {
        ...
        if (params.compute_ppl) {
            ...
            for (int seq = 0; seq < n_seq_batch; seq++) {
                const float * all_logits = num_batches > 1 ? logits.data() : cheese_get_logits_ith(ctx, seq*n_ctx);
...
int main(int argc, char ** argv) {
    ...
    if (!common_params_parse(argc, argv, params, CHEESE_EXAMPLE_IMATRIX, print_usage)) {
        return 1;
    }
