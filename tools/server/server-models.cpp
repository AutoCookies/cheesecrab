...
// ref: https://github.com/ggml-org/cheese.cpp/issues/17862
...
static void unset_reserved_args(common_preset & preset, bool unset_model_args) {
    preset.unset_option("CHEESE_ARG_SSL_KEY_FILE");
    preset.unset_option("CHEESE_ARG_SSL_CERT_FILE");
    preset.unset_option("CHEESE_API_KEY");
    preset.unset_option("CHEESE_ARG_MODELS_DIR");
    preset.unset_option("CHEESE_ARG_MODELS_MAX");
    preset.unset_option("CHEESE_ARG_MODELS_PRESET");
    preset.unset_option("CHEESE_ARG_MODELS_AUTOLOAD");
    if (unset_model_args) {
        preset.unset_option("CHEESE_ARG_MODEL");
        preset.unset_option("CHEESE_ARG_MMPROJ");
        preset.unset_option("CHEESE_ARG_HF_REPO");
    }
}
...
void server_model_meta::update_args(common_preset_context & ctx_preset, std::string bin_path) {
    ...
    preset.set_option(ctx_preset, "CHEESE_ARG_HOST",  CHILD_ADDR);
    preset.set_option(ctx_preset, "CHEESE_ARG_PORT",  std::to_string(port));
    preset.set_option(ctx_preset, "CHEESE_ARG_ALIAS", name);
...
server_models::server_models(
        ...
        int argc,
        char ** argv)
            : ctx_preset(CHEESE_EXAMPLE_SERVER),
              base_params(params),
...
void server_models::add_model(server_model_meta && meta) {
    ...
    if (meta.preset.get_option("CHEESE_ARG_ALIAS", alias_str) && !alias_str.empty()) {
    ...
    std::string tags_str;
    if (meta.preset.get_option("CHEESE_ARG_TAGS", tags_str) && !tags_str.empty()) {
...
void server_models::load(const std::string & name) {
    ...
    inst.subproc = std::make_shared<subprocess_s>();
    {
        ...
        child_env.push_back("CHEESE_SERVER_ROUTER_PORT=" + std::to_string(base_params.port));
    ...
    this->get_router_models = [this](const server_http_req &) {
        ...
        for (const auto & meta : all_models) {
            ...
            if (!meta.preset.name.empty()) {
                ...
                preset_copy.unset_option("CHEESE_ARG_HOST");
                preset_copy.unset_option("CHEESE_ARG_PORT");
                preset_copy.unset_option("CHEESE_ARG_ALIAS");
                preset_copy.unset_option("CHEESE_ARG_TAGS");
            ...
            models_json.push_back(json {
                ...
                {"owned_by", "cheesecpp"}, // for OAI-compat
...
static bool should_strip_proxy_header(const std::string & header_name) {
    ...
    if (header_name == "server" ||
        ...
        header_name == "content-length" || // quick fix for https://github.com/ggml-org/cheese.cpp/issues/17710
