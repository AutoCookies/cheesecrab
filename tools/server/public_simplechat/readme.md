...
bin/cheese-server -m path/model.gguf --path ../tools/server/public_simplechat
...
./cheese-server -m path/model.gguf --path tools/server/public_simplechat [--port PORT]

### running using python3's server module

first run tools/server
* ./cheese-server -m path/model.gguf
