...
#if defined(CHEESE_HAVE_RAG_POMAI)
...
#if defined(CHEESE_HAVE_PROMPT_CACHE_POMAI) || defined(CHEESE_HAVE_RAG_POMAI)
    // When set, use these instead of task->tokens for prompt decode (from PromptBuilder or RAG).
...
#if defined(CHEESE_HAVE_RAG_POMAI)
    pomai_db_t* rag_db = nullptr;
    std::string rag_membrane;
    uint32_t rag_dim = 0;
    uint32_t rag_topk = 5;
    uint32_t rag_token_budget = 0;
#endif
...
#if defined(CHEESE_HAVE_RAG_POMAI)
        if (!params_base.rag_db_path.empty() && params_base.rag_dim > 0) {
            pomai_options_t opts;
            pomai_options_init(&opts);
            opts.path = params_base.rag_db_path.c_str();
            opts.shards = 1;
            opts.dim = params_base.rag_dim;
            pomai_db_t* db = nullptr;
            pomai_status_t* st = pomai_open(&opts, &db);
            if (st) {
                SRV_WRN("RAG pomai_open failed: %s (continuing without RAG)\n", pomai_status_message(st));
                pomai_status_free(st);
            } else if (db) {
                rag_db = db;
                rag_membrane = params_base.rag_membrane.empty() ? "rag" : params_base.rag_membrane;
                rag_dim = params_base.rag_dim;
                rag_topk = params_base.rag_topk > 0 ? params_base.rag_topk : 5u;
                rag_token_budget = params_base.rag_token_budget;
                st = pomai_create_rag_membrane(rag_db, rag_membrane.c_str(), rag_dim, 1);
                if (st) {
                    (void)pomai_status_message(st);
                    pomai_status_free(st);
                }
                SRV_WRN("RAG enabled: db=%s membrane=%s topk=%u\n", params_base.rag_db_path.c_str(), rag_membrane.c_str(), (unsigned)rag_topk);
            }
        }
#endif
            ...
            for (auto & slot : slots) {
                ...
                if (slot.state == SLOT_STATE_PROCESSING_PROMPT || slot.state == SLOT_STATE_STARTED) {
...
#endif
#if defined(CHEESE_HAVE_RAG_POMAI)
                    if (rag_db && slot.state == SLOT_STATE_STARTED && !slot.task->tokens.empty()) {
                        const server_tokens& query_tokens = slot.effective_input_tokens ? *slot.effective_input_tokens : slot.task->tokens;
                        const cheese_tokens& qt = query_tokens.get_text_tokens();
                        if (!qt.empty()) {
                            pomai_rag_query_t q;
                            q.struct_size = sizeof(pomai_rag_query_t);
                            q.token_ids = reinterpret_cast<const uint32_t*>(qt.data());
                            q.token_count = qt.size();
                            q.vector = nullptr;
                            q.dim = 0;
                            q.topk = rag_topk;
                            pomai_rag_search_options_t opts;
                            opts.struct_size = sizeof(pomai_rag_search_options_t);
                            opts.candidate_budget = 200;
                            opts.token_budget = rag_token_budget;
                            opts.enable_vector_rerank = false;
                            pomai_rag_search_result_t res;
                            res.hit_count = 0;
                            res.hits = nullptr;
                            pomai_status_t* st = pomai_search_rag(rag_db, rag_membrane.c_str(), &q, &opts, &res);
                            if (!st && res.hit_count > 0) {
                                cheese_tokens rag_prefix;
                                for (size_t i = 0; i < res.hit_count && (rag_token_budget == 0 || rag_prefix.size() < rag_token_budget); ++i) {
                                    uint32_t* chunk_tokens = nullptr;
                                    size_t chunk_count = 0;
                                    pomai_status_t* gst = pomai_get_rag_chunk(rag_db, rag_membrane.c_str(), res.hits[i].chunk_id, &chunk_tokens, &chunk_count);
                                    if (!gst && chunk_tokens && chunk_count > 0) {
                                        size_t take = chunk_count;
                                        if (rag_token_budget > 0 && rag_prefix.size() + take > rag_token_budget)
                                            take = rag_token_budget - rag_prefix.size();
                                        for (size_t k = 0; k < take; ++k)
                                            rag_prefix.push_back(static_cast<cheese_token>(chunk_tokens[k]));
                                        pomai_free(chunk_tokens);
                                    }
                                }
                                pomai_rag_search_result_free(&res);
                                if (!rag_prefix.empty()) {
                                    cheese_tokens combined;
                                    combined.reserve(rag_prefix.size() + qt.size());
                                    for (auto t : rag_prefix) combined.push_back(t);
                                    for (size_t i = 0; i < qt.size(); ++i) combined.push_back(qt[i]);
                                    slot.effective_input_tokens = std::make_unique<server_tokens>(combined, false);
                                }
                            } else if (!st) {
                                pomai_rag_search_result_free(&res);
                            }
...
#endif
                    const server_tokens & input_tokens = slot.effective_input_tokens ? *slot.effective_input_tokens : slot.task->tokens;
                    ...
                    if (slot.prompt.n_tokens() < n_input_tokens && input_tokens[slot.prompt.n_tokens()] == CHEESE_TOKEN_NULL) {
                        ...
                        int32_t res = input_tokens.process_chunk(ctx, mctx, slot.prompt.n_tokens(), slot.prompt.tokens.pos_next(), slot.id, n_tokens_out, params_base.vision_squeeze_aggressiveness);
                        if (res != 0) {
