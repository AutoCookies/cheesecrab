...
A modern, feature-rich web interface for cheese.cpp built with SvelteKit. This UI provides an intuitive chat interface with advanced file handling, conversation management, and comprehensive model interaction capabilities.

The WebUI supports two server operation modes:

- **MODEL mode** - Single model operation (standard cheese-server)
...
- **npm** 9+
- **cheese-server** running locally (for API access)
...
### 2. Start cheese-server

In a separate terminal, start the backend server:

```bash
# Single model (MODEL mode)
./cheese-server -m model.gguf

# Multi-model (ROUTER mode)
./cheese-server --models-dir /path/to/models
```
...
The Vite dev server proxies API requests to `http://localhost:8080` (default cheese-server port):
...
2. **Static Adapter** - Outputs to `../public` (cheese-server's static file directory)
...
├── src/                                 ├── index.html.gz  (served by cheese-server)
...
### Integration with cheese-server

The WebUI is embedded directly into the cheese-server binary:

1. `npm run build` outputs `index.html.gz` to `tools/server/public/`
2. cheese-server compiles this into the binary at build time
3. When accessing `/`, cheese-server serves the gzipped HTML
...
sequenceDiagram
    ...
    participant API as cheese-server
...
npm run test:e2e      # End-to-end (requires cheese-server)
...
- [cheese.cpp Server README](../README.md) - Full server documentation
