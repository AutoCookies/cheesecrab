...
# Development script for cheese.cpp webui
# 
# This script starts the webui development servers (Storybook and Vite).
# Note: You need to start cheese-server separately.
#
# Usage:
#   bash scripts/dev.sh
#   npm run dev
...
echo "📝 Note: Make sure to start cheese-server separately if needed"
cd tools/server/webui
# Use --insecure-http-parser to handle malformed HTTP responses from cheese-server
