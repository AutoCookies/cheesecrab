...
export class PropsService {
	 ...
	 */
	static async fetch(autoload = false): Promise<ApiCheeseCppServerProps> {
		const params: Record<string, string> = {};
		if (!autoload) {
			params.autoload = 'false';
		}

		return apiFetchWithParams<ApiCheeseCppServerProps>('./props', params, { authOnly: true });
	}
	 ...
	 */
	static async fetchForModel(modelId: string, autoload = false): Promise<ApiCheeseCppServerProps> {
