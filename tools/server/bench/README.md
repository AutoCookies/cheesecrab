...
Example:
```shell
cheese-server --host localhost --port 8080 \
...
- `cheesecpp_tokens_second` Trend of `usage.total_tokens / request duration`
- `cheesecpp_prompt_tokens` Trend of `usage.prompt_tokens`
- `cheesecpp_prompt_tokens_total_counter` Counter of `usage.prompt_tokens`
- `cheesecpp_completion_tokens` Trend of `usage.completion_tokens`
- `cheesecpp_completion_tokens_total_counter` Counter of `usage.completion_tokens`
- `cheesecpp_completions_truncated_rate` Rate of completions truncated, i.e. if `finish_reason === 'length'`
- `cheesecpp_completions_stop_rate` Rate of completions stopped by the model, i.e. if `finish_reason === 'stop'`

The script will fail if too many completions are truncated, see `cheesecpp_completions_truncated_rate`.
...
```shell
CHEESE_SERVER_BIN_PATH=../../../cmake-build-release/bin/cheese-server python bench.py \
              --runner-label local \
              --name local \
