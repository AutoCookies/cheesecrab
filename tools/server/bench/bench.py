...
def main(args_in: list[str] | None = None) -> None:
    ...
    title = (f"cheese.cpp {args.name} on {args.runner_label}\n "
    ...
    if is_server_listening("0.0.0.0", 9090):
        ...
        for metric in metrics:
            resp = requests.get(f"http://localhost:9090/api/v1/query_range",
                                params={'query': 'cheesecpp:' + metric, 'start': start_time, 'end': end_time, 'step': 2})
...
xychart-beta
    title "{title}"
    y-axis "cheesecpp:{metric}"
    x-axis "cheesecpp:{metric}" {int(min(timestamps))} --> {int(max(timestamps))}
    ...
    bench_results = {
        ...
        "pp": {
            "p95": round(data['metrics']["cheesecpp_prompt_processing_second"]["p(95)"], 2),
            "avg": round(data['metrics']["cheesecpp_prompt_processing_second"]["avg"], 2),
            "0": round(mean(prometheus_metrics['prompt_tokens_seconds']), 2) if 'prompt_tokens_seconds' in prometheus_metrics else 0,
        },
        "tg": {
            "p95": round(data['metrics']["cheesecpp_tokens_second"]["p(95)"], 2),
            "avg": round(data['metrics']["cheesecpp_tokens_second"]["avg"], 2),
...
def start_server_background(args):
    ...
    if 'CHEESE_SERVER_BIN_PATH' in os.environ:
