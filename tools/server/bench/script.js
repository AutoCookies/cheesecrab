...
const cheesecpp_prompt_tokens = new Trend('cheesecpp_prompt_tokens')
const cheesecpp_completion_tokens = new Trend('cheesecpp_completion_tokens')

const cheesecpp_tokens_second = new Trend('cheesecpp_tokens_second')
const cheesecpp_prompt_processing_second = new Trend('cheesecpp_prompt_processing_second')
const cheesecpp_emit_first_token_second = new Trend('cheesecpp_emit_first_token_second')

const cheesecpp_prompt_tokens_total_counter = new Counter('cheesecpp_prompt_tokens_total_counter')
const cheesecpp_completion_tokens_total_counter = new Counter('cheesecpp_completion_tokens_total_counter')

const cheesecpp_completions_truncated_rate = new Rate('cheesecpp_completions_truncated_rate')
const cheesecpp_completions_stop_rate = new Rate('cheesecpp_completions_stop_rate')

export const options = {
    thresholds: {
        cheesecpp_completions_truncated_rate: [
...
export default function () {
    ...
    const payload = {
        ...
        "stream": true,
        "stream_options": {
          "include_usage": true, // False to be supported in cheese.cpp server
        },
        "seed": 42,
    ...
    const res = sse.open(`${server_url}/chat/completions`, params, function (client) {
        client.on('event', function (event) {
            if (promptEvalEndTime == null) {
                ...
                cheesecpp_emit_first_token_second.add((promptEvalEndTime - startTime) / 1.e3)
            }
            ...
            if (chunk.usage) {
                ...
                cheesecpp_prompt_tokens.add(prompt_tokens)
                cheesecpp_prompt_tokens_total_counter.add(prompt_tokens)

                completions_tokens = chunk.usage.completion_tokens
                cheesecpp_completion_tokens.add(completions_tokens)
                cheesecpp_completion_tokens_total_counter.add(completions_tokens)
            }
        })
    ...
    if (promptEvalTime > 0) {
        cheesecpp_prompt_processing_second.add(prompt_tokens / (promptEvalEndTime - startTime) * 1.e3)
    }

    const completion_time = endTime - promptEvalEndTime
    if (completions_tokens > 0 && completion_time > 0) {
        cheesecpp_tokens_second.add(completions_tokens / completion_time * 1.e3)
    }
    cheesecpp_completions_truncated_rate.add(finish_reason === 'length')
    cheesecpp_completions_stop_rate.add(finish_reason === 'stop')
