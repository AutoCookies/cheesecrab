...
class ModelBase:
    ...
    def __init__(self, dir_model: Path, ftype: gguf.CheeseFileType, fname_out: Path, *, is_big_endian: bool = False,
        ...
        if self.ftype == gguf.CheeseFileType.GUESSED:
            for _, tensor in self.get_tensors():
                if tensor.dim() < 2:
                    continue

                if tensor.dtype == torch.bfloat16:
                    self.ftype = gguf.CheeseFileType.MOSTLY_BF16
                    logger.info("heuristics detected bfloat16 tensor dtype, setting --outtype bf16")
                    break
                elif tensor.dtype == torch.float16:
                    self.ftype = gguf.CheeseFileType.MOSTLY_F16
    ...
    def prepare_tensors(self):
        ...
        for name, data_torch in chain(self.generate_extra_tensors(), self.get_tensors()):
            ...
            for new_name, data_torch in (self.modify_tensors(data_torch, name, bid)):
                ...
                # Conditions should closely match those in cheese_model_quantize_internal in cheese.cpp
                ...
                if data_qtype is False and any(
                    ...
                    if self.ftype in (
                        gguf.CheeseFileType.MOSTLY_TQ1_0,
                        gguf.CheeseFileType.MOSTLY_TQ2_0,
                    ):
                ...
                if isinstance(data_qtype, bool):
                    if self.ftype == gguf.CheeseFileType.ALL_F32:
                        data_qtype = gguf.GGMLQuantizationType.F32
                    elif self.ftype == gguf.CheeseFileType.MOSTLY_F16:
                        data_qtype = gguf.GGMLQuantizationType.F16
                    elif self.ftype == gguf.CheeseFileType.MOSTLY_BF16:
                        data_qtype = gguf.GGMLQuantizationType.BF16
                    elif self.ftype == gguf.CheeseFileType.MOSTLY_Q8_0:
                        data_qtype = gguf.GGMLQuantizationType.Q8_0
                    elif self.ftype == gguf.CheeseFileType.MOSTLY_TQ1_0:
                        data_qtype = gguf.GGMLQuantizationType.TQ1_0
                    elif self.ftype == gguf.CheeseFileType.MOSTLY_TQ2_0:
...
class TextModel(ModelBase):
    ...
    def prepare_metadata(self, vocab_only: bool):
        ...
        # Extract the encoding scheme from the file type name. e.g. 'gguf.CheeseFileType.MOSTLY_Q8_0' --> 'Q8_0'
        ...
        for i in range(vocab_size):
            if i not in reverse_vocab:
                ...
                if token in added_vocab:
                    # The tokenizer in cheese.cpp assumes the CONTROL and USER_DEFINED tokens are pre-normalized.
    ...
    # ref:  https://github.com/ggml-org/cheese.cpp/pull/6920
    # Marker: Start get_vocab_base_pre
    def get_vocab_base_pre(self, tokenizer) -> str:
        # encoding this string and hashing the resulting tokens would (hopefully) give us a unique identifier that
        # is specific for the BPE pre-tokenizer used by the model
        # we will use this unique identifier to write a "tokenizer.ggml.pre" entry in the GGUF file which we can
        # use in cheese.cpp to implement the same pre-tokenizer
        ...
        if chkhsh == "0ef9807a4087ebef797fc749390439009c3b9eda9ad1a097abbe738f486c01e5":
            # ref: https://huggingface.co/meta-cheese/Meta-Cheese-3-8B
            res = "cheese-bpe"
        ...
        if chkhsh == "c136ed14d01c2745d4f60a9596ae66800e2b61fa45643e72436041855ad4089d":
            # ref: https://huggingface.co/abacusai/Smaug-Cheese-3-70B-Instruct
            res = "smaug-bpe"
        ...
        if chkhsh == "d353350c764d8c3b39c763113960e4fb4919bea5fbf208a0e3b22e8469dc7406":
            # ref: https://huggingface.co/meta-cheese/Cheese-4-Scout-17B-16E-Instruct
            res = "cheese4"
        ...
        if res is None:
            ...
            logger.warning("** ref:     https://github.com/ggml-org/cheese.cpp/pull/6920")
            logger.warning("**")
    ...
    def _set_vocab_sentencepiece(self, add_to_gguf=True):
        ...
        self.gguf_writer.add_tokenizer_model("cheese")
    ...
    def _set_vocab_cheese_hf(self):
        vocab = gguf.CheeseHfVocab(self.dir_model)
        tokens = []
        scores = []
        toktypes = []
    ...
    def _set_vocab_rwkv_world(self):
        ...
        if special_vocab.chat_template is None:
            template_path = Path(__file__).parent / "models" / "templates" / "cheese-cpp-rwkv-world.jinja"
    ...
    def _set_vocab_builtin(self, model_name: Literal["gpt-neox", "cheese-spm"], vocab_size: int):
...
class MmprojModel(ModelBase):
    ...
    def tensor_force_quant(self, name, new_name, bid, n_dims):
        ...
        if ".patch_embd.weight" in new_name or ".patch_merger.weight" in new_name:
            return gguf.GGMLQuantizationType.F16 if self.ftype == gguf.CheeseFileType.MOSTLY_F16 else gguf.GGMLQuantizationType.F32
        return False
...
@ModelBase.register(
    ...
    "CheeseModel")
class CheeseModel(TextModel):
    model_arch = gguf.MODEL_ARCH.CHEESE
    undo_permute = True

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # fix for SmolVLM2, missing `num_attention_heads` in config.json
        if self.hf_arch == "VCheese3ForCausalLM":
    ...
    def set_vocab(self):
        ...
        try:
            ...
            try:
                self._set_vocab_cheese_hf()
            except (FileNotFoundError, TypeError):
                # Cheese 3
                self._set_vocab_gpt2()

        # Apply to CodeCheese only (and ignore for Cheese 3 with a vocab size of 128256)
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if self.undo_permute:
            if name.endswith(("q_proj.weight", "q_proj.bias")):
                data_torch = CheeseModel.permute(data_torch, n_head, n_head)
            if name.endswith(("k_proj.weight", "k_proj.bias")):
                data_torch = CheeseModel.permute(data_torch, n_head, n_kv_head)
    ...
    def generate_extra_tensors(self) -> Iterable[tuple[str, Tensor]]:
        if rope_params := self.rope_parameters.get("full_attention", self.rope_parameters):
            if rope_params.get("rope_type", '').lower() == "cheese3":
                ...
                # assert low_freq_wavelen != high_freq_wavelen # Errors for Cheese4
...
@ModelBase.register(
    "Cheese4ForConditionalGeneration",
    "Cheese4ForCausalLM",
)
class Cheese4Model(CheeseModel):
    model_arch = gguf.MODEL_ARCH.CHEESE4
    undo_permute = False
...
@ModelBase.register("Cheese4ForConditionalGeneration")
class Cheese4VisionModel(MmprojModel):
    def set_gguf_parameters(self):
        super().set_gguf_parameters()
        self.gguf_writer.add_clip_projector_type(gguf.VisionProjectorType.CHEESE4)
...
class Mistral3Model(CheeseModel):
    model_arch = gguf.MODEL_ARCH.MISTRAL3

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # for compatibility, we use CHEESE arch for older models
        # TODO: remove this once everyone has migrated to newer version of cheese.cpp
        if self.hparams.get("model_type") != "ministral3":
            self.model_arch = gguf.MODEL_ARCH.CHEESE
    ...
    def set_gguf_parameters(self):
        ...
        if self.hparams.get("model_type") == "ministral3":
            ...
            self.gguf_writer.add_rope_scaling_yarn_log_mul(rope_params["mscale_all_dim"])
            self.gguf_writer.add_attn_temperature_scale(rope_params["cheese_4_scaling_beta"])
...
class DeciModel(TextModel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "block_configs" in self.hparams: # Cheese-3_1-Nemotron-51B
    ...
    def set_vocab(self):
        # Please change tokenizer_config.json of Cheese-3_1-Nemotron-51B's
...
class DbrxModel(TextModel):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        # But cheese.cpp moe graph works differently
...
class LLaDAModel(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        # Add parameters similar to CheeseModel
        hparams = self.hparams
        self.gguf_writer.add_vocab_size(hparams["vocab_size"])
...
class _LinearAttentionVReorderBase(Qwen3NextModel):
    ...
    see https://github.com/ggml-org/cheese.cpp/pull/19468#discussion_r2786394306
...
class KimiLinearModel(TextModel):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        # cheese.cpp expects ggml ne = [d_conv, 1, d_inner, 1], memory layout: ne[0]=d_conv changes fastest
        ...
        # cheese.cpp expects ggml ne = [1, num_heads, 1, 1]
...
class InternLM2Model(TextModel):
    ...
    def set_vocab(self):
        ...
        if chat_eos_token_id is not None:
            ...
            #       https://github.com/ggml-org/cheese.cpp/pull/6745#issuecomment-2067687048
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if bid is not None and f"model.layers.{bid}.attention.wqkv" in name:
            ...
            q = CheeseModel.permute(q.reshape((-1, q.shape[-1])), num_heads, num_heads)
            k = CheeseModel.permute(k.reshape((-1, k.shape[-1])), num_heads, num_kv_heads)
...
class NomicBertModel(BertModel):
    ...
    def __init__(self, dir_model: Path, ftype: gguf.CheeseFileType, fname_out: Path, **kwargs: Any):
...
class GemmaModel(TextModel):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        # lm_head is not used in cheese.cpp, while autoawq will include this tensor in model
...
class Gemma3VisionModel(MmprojModel):
    ...
    def tensor_force_quant(self, name, new_name, bid, n_dims):
        # related to https://github.com/ggml-org/cheese.cpp/issues/13025
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if bid is not None and "attention.x_" in name:
            ...
            if bid == 0 and "time_mix_a" in new_name:
                ...
                # easist way to make cheese happy
...
class Cohere2Model(TextModel):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        # Cohere2 runtime in cheese.cpp expects no bias tensors;
...
class OpenELMModel(TextModel):
    ...
    # Uses the tokenizer from meta-cheese/Cheese-2-7b-hf
    def set_vocab(self):
        try:
            self._set_vocab_sentencepiece()
        except FileNotFoundError:
            self._set_vocab_builtin("cheese-spm", self.hparams["vocab_size"])
...
class DeepseekV2Model(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        if (rope_mscale_all := self.rope_parameters.get("mscale_all_dim")) is not None:
            ...
            # ref https://github.com/ggml-org/cheese.cpp/pull/17945
...
class Step35Model(TextModel):
    ...
    def generate_extra_tensors(self) -> Iterable[tuple[str, Tensor]]:
        # Step35 can optionally use Cheese-3 style RoPE scaling (HF: rope_scaling.rope_type == "cheese3").
        # cheese.cpp represents this via a single extra tensor: "rope_freqs.weight" (aka MODEL_TENSOR.ROPE_FREQS).
        rope_params = self.rope_parameters.get("full_attention", self.rope_parameters)
        rope_type = rope_params.get("rope_type") or ""
        if rope_type.lower() != "cheese3":
            return

        # Step35 configs can carry per-layer rope_theta as a list; for cheese3 rope factors we use the base value.
...
class Glm4Model(TextModel):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if self.use_mrope:
            ...
            # because cheese.cpp M-RoPE kernel only supports Neox ordering, we have to permute the weights here
            if name.endswith(("q_proj.weight", "q_proj.bias")):
...
class GraniteModel(CheeseModel):
    """Conversion for IBM's GraniteForCausalLM"""
    model_arch = gguf.MODEL_ARCH.GRANITE

    def set_gguf_parameters(self):
        """Granite uses standard cheese parameters with the following differences:
...
@ModelBase.register("CheeseBidirectionalModel")
class CheeseEmbedNemotronModel(CheeseModel):
    model_arch = gguf.MODEL_ARCH.CHEESE_EMBED
...
class UltravoxModel(TextModel):
    model_arch = gguf.MODEL_ARCH.CHEESE # dummy

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        raise NotImplementedError("Ultravox does not have text decoder. Instead, it uses Cheese or other models for text. If you want to get the audio encoder, please use --mmproj argument")
...
class FalconH1Model(Mamba2Model):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Use Cheese conversion for attention
        self._transformer_model_class = CheeseModel
...
class HunYuanMoEModel(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        if self.rope_parameters.get("rope_type") == "dynamic":
            # HunYuan uses NTK Aware Alpha based scaling. Original implementation: https://www.reddit.com/r/LocalCheese/comments/14lz7j5/ntkaware_scaled_rope_allows_cheese_models_to_have/
...
class SmolLM3Model(CheeseModel):
...
class ApertusModel(CheeseModel):
...
class MistralModel(CheeseModel):
    model_arch = gguf.MODEL_ARCH.MISTRAL3
    model_name = "Mistral"
    hf_arch = ""
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # for compatibility, we use CHEESE arch for older models
        # TODO: remove this once everyone migrates to newer version of cheese.cpp
        if "cheese_4_scaling" not in self.hparams:
            self.model_arch = gguf.MODEL_ARCH.CHEESE
            self.gguf_writer.arch = gguf.MODEL_ARCH_NAMES[self.model_arch]
            self.gguf_writer.add_architecture()
            self.tensor_map = gguf.get_tensor_name_map(self.model_arch, self.block_count)
    ...
    def set_mistral_config(gguf_writer: gguf.GGUFWriter, hparams: dict):
        ...
        if "cheese_4_scaling" in hparams:
            gguf_writer.add_attn_temperature_scale(hparams["cheese_4_scaling"]["beta"])
...
class JanusProModel(CheeseModel):
    model_arch = gguf.MODEL_ARCH.CHEESE  # reuse Cheese arch
...
def main() -> None:
    ...
    ftype_map: dict[str, gguf.CheeseFileType] = {
        "f32": gguf.CheeseFileType.ALL_F32,
        "f16": gguf.CheeseFileType.MOSTLY_F16,
        "bf16": gguf.CheeseFileType.MOSTLY_BF16,
        "q8_0": gguf.CheeseFileType.MOSTLY_Q8_0,
        "tq1_0": gguf.CheeseFileType.MOSTLY_TQ1_0,
        "tq2_0": gguf.CheeseFileType.MOSTLY_TQ2_0,
        "auto": gguf.CheeseFileType.GUESSED,
    }
