...
# Basedir on device
basedir=/data/local/tmp/cheese.cpp
...
model="Cheese-3.2-3B-Instruct-Q4_0.gguf"
[ "$M" != "" ] && model="$M"
...
adb $adbserial $adbhost shell " \
  ...
  ADSP_LIBRARY_PATH=$basedir/$branch/lib \
    $ndev $nhvx $opmask $verbose $experimental $profile $hb ./$branch/bin/cheese-bench --device $device --mmap 0 -m $basedir/../gguf/$model \
