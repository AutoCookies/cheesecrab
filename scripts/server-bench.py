...
def get_server(path_server: str, path_log: Optional[str]) -> dict:
    ...
    if os.environ.get("CHEESE_ARG_HOST") is None:
        logger.info("CHEESE_ARG_HOST not explicitly set, using 127.0.0.1")
        os.environ["CHEESE_ARG_HOST"] = "127.0.0.1"
    if os.environ.get("CHEESE_ARG_PORT") is None:
        logger.info("CHEESE_ARG_PORT not explicitly set, using 8080")
        os.environ["CHEESE_ARG_PORT"] = "8080"
    hostname: Optional[str] = os.environ.get("CHEESE_ARG_HOST")
    port: Optional[str] = os.environ.get("CHEESE_ARG_PORT")
    assert hostname is not None
    assert port is not None
    address: str = f"http://{hostname}:{port}"
    logger.info(f"Starting the cheese.cpp server under {address}...")
    ...
    while True:
        try:
            ...
            if exit_code is not None:
                raise RuntimeError(f"cheese.cpp server exited unexpectedly with exit code {exit_code}{path_log and f', see {path_log.format(port=port)}' or ''}")
            ...
            n_failures += 1
            if n_failures >= 10:
                raise RuntimeError("cheese.cpp server is not healthy after 10 seconds")
...
def benchmark(
    ...
    if os.environ.get("CHEESE_ARG_N_PARALLEL") is None:
        logger.info("CHEESE_ARG_N_PARALLEL not explicitly set, using 32")
        os.environ["CHEESE_ARG_N_PARALLEL"] = "32"

    parallel: int = int(os.environ.get("CHEESE_ARG_N_PARALLEL")) # type: ignore
    ...
    if not external_server and os.environ.get("CHEESE_ARG_CTX_SIZE") is None:
        ...
        os.environ["CHEESE_ARG_CTX_SIZE"] = str(context_total)
        logger.info(f"CHEESE_ARG_CTX_SIZE not explicitly set, using {context_total} ({context_per_slot} per slot).")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Tool for benchmarking the throughput of the cheese.cpp HTTP server. "
        "Results are printed to console and visualized as plots (saved to current working directory). "
        "To pass arguments such as the model path to the server, set the corresponding environment variables (see cheese-server --help). "
    ...
    parser.add_argument("--path_server", type=str, default="cheese-server", help="Path to the cheese.cpp server binary")
