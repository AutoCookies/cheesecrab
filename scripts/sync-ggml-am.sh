#!/usr/bin/env bash
#
# Synchronize ggml changes to cheese.cpp
#
# Usage:
#
#   $ cd /path/to/cheese.cpp
...
lc=$(cat $SRC_CHEESE/scripts/sync-ggml.last)
...
git log --oneline $lc..HEAD --reverse | grep -v "(cheese/[0-9]*)" | cut -d' ' -f1 > $SRC_CHEESE/ggml-commits

if [ ! -s $SRC_CHEESE/ggml-commits ]; then
    rm -v $SRC_CHEESE/ggml-commits
    echo "No new commits"
    exit 0
fi

if [ -f $SRC_CHEESE/ggml-src.patch ]; then
    rm -v $SRC_CHEESE/ggml-src.patch
fi
...
while read c; do
    ...
    git format-patch -U${ctx} -k $c~1..$c --stdout -- \
        ...
        >> $SRC_CHEESE/ggml-src.patch
done < $SRC_CHEESE/ggml-commits

rm -v $SRC_CHEESE/ggml-commits

# delete files if empty
if [ ! -s $SRC_CHEESE/ggml-src.patch ]; then
    rm -v $SRC_CHEESE/ggml-src.patch
fi
...
# update last commit
cd $SRC_GGML
git log -1 --format=%H > $SRC_CHEESE/scripts/sync-ggml.last
