...
if __name__ == '__main__':
    ...
    cli_path = os.environ.get(
        ...
        os.path.join(
            ...
            '../build/bin/Release/cheese-cli.exe' if os.name == 'nt' else '../build/bin/cheese-cli'))
    ...
    for m in models:
        ...
        if m.hf_file is not None and '-of-' in m.hf_file:
            ...
            continue
        logging.info(f'Using cheese-cli to ensure model {m.hf_repo}/{m.hf_file} was fetched')
        cmd = [
            cli_path,
            '-hfr', m.hf_repo,
            *([] if m.hf_file is None else ['-hff', m.hf_file]),
            ...
            '--log-disable',
            '-no-cnv']
        if m.hf_file != 'tinycheeses/stories260K.gguf' and 'Mistral-Nemo' not in m.hf_repo:
            cmd.append('-fa')
        try:
