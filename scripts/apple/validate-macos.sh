#!/usr/bin/env bash
# validate-macos.sh - Validate macOS Application with embedded cheese.xcframework using SwiftUI
...
# Configuration
APP_NAME="MacOSCheeseTest"
BUNDLE_ID="org.ggml.MacOSCheeseTest"
...
struct ContentView: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            Text("Cheese Framework Test on macOS")
                .font(.largeTitle)
                .padding()
...
cat >> "${TEMP_DIR}/${APP_NAME}/${APP_NAME}.xcodeproj/project.pbxproj" << 'EOF'
        ...
        EEEEEEEEEEEEEEEEEEEEEEEE = {
            isa = PBXGroup;
            children = (
                FFFFFFFFFFFFFFFFFFFFFFFF /* MacOSCheeseTest */,
                CCCCCCCCCCCCCCCCCCCCCCCC /* Products */,
                DDDDDDDDDDDDDDDDDDDDDDDD /* Frameworks */,
            );
            sourceTree = "<group>";
...
/* Begin XCBuildConfiguration section */
        ...
        BBBBBBBBBBBBBBBBBBBBBBCCC /* Debug */ = {
            ...
            buildSettings = {
                ...
                INFOPLIST_FILE = "MacOSCheeseTest/Info.plist";
                LD_RUNPATH_SEARCH_PATHS = (
                    "$(inherited)",
                    "@executable_path/../Frameworks",
                );
                PRODUCT_BUNDLE_IDENTIFIER = "org.ggml.MacOSCheeseTest";
...
if [ -d "${APP_PATH}/Contents/Frameworks/cheese.framework" ]; then
...
if [ -f "${APP_PATH}/Contents/Frameworks/cheese.framework/Versions/A/cheese" ]; then
    echo "✅ Framework binary exists"

    # Further validate framework by checking architecture
    ARCHS=$(lipo -info "${APP_PATH}/Contents/Frameworks/cheese.framework/Versions/A/cheese" 2>/dev/null | grep -o "arm64\\|x86_64" | tr '\n' ' ')
    if [ -n "$ARCHS" ]; then
