#!/usr/bin/env bash
# validate-visionos.sh - Validate visionOS Application with embedded cheese.xcframework using SwiftUI
...
# Configuration
APP_NAME="VisionOSCheeseTest"
BUNDLE_ID="org.ggml.VisionOSCheeseTest"
XCFRAMEWORK_PATH="${ROOT_DIR}/build-apple/cheese.xcframework"
...
struct ContentView: View {
    // Test that we can initialize a cheese context params struct
    let params = cheese_context_default_params()

    var body: some View {
        VStack(spacing: 20) {
            Text("Cheese Framework Test on visionOS")
                .font(.largeTitle)
                .padding()

            Text("cheese_context_default_params() created successfully")
                .font(.headline)
...
/* Begin PBXBuildFile section */
        ...
        55555555555555555555555 /* cheese.xcframework in Frameworks */ = {isa = PBXBuildFile; fileRef = 66666666666666666666666; };
        77777777777777777777777 /* cheese.xcframework in Embed Frameworks */ = {isa = PBXBuildFile; fileRef = 66666666666666666666666; };
...
/* Begin PBXCopyFilesBuildPhase section */
        88888888888888888888888 /* Embed Frameworks */ = {
            ...
            dstSubfolderSpec = 10;
            files = (
                77777777777777777777777 /* cheese.xcframework in Embed Frameworks */,
            );
...
cat >> "${TEMP_DIR}/${APP_NAME}/${APP_NAME}.xcodeproj/project.pbxproj" << EOF
        ...
        66666666666666666666666 /* cheese.xcframework */ = {isa = PBXFileReference; lastKnownFileType = wrapper.xcframework; path = cheese.xcframework; sourceTree = "<group>"; };
...
/* Begin PBXFrameworksBuildPhase section */
        BBBBBBBBBBBBBBBBBBBBBBBB /* Frameworks */ = {
            ...
            buildActionMask = 2147483647;
            files = (
                55555555555555555555555 /* cheese.xcframework in Frameworks */,
            );
            runOnlyForDeploymentPostprocessing = 0;
        };
...
cat >> "${TEMP_DIR}/${APP_NAME}/${APP_NAME}.xcodeproj/project.pbxproj" << 'EOF'
        DDDDDDDDDDDDDDDDDDDDDDDD /* Frameworks */ = {
            isa = PBXGroup;
            children = (
                66666666666666666666666 /* cheese.xcframework */,
            );
            name = Frameworks;
            sourceTree = "<group>";
        };
        EEEEEEEEEEEEEEEEEEEEEEEE = {
            isa = PBXGroup;
            children = (
                FFFFFFFFFFFFFFFFFFFFFFFF /* VisionOSCheeseTest */,
                CCCCCCCCCCCCCCCCCCCCCCCC /* Products */,
                DDDDDDDDDDDDDDDDDDDDDDDD /* Frameworks */,
            );
            sourceTree = "<group>";
...
/* Begin XCBuildConfiguration section */
        ...
        BBBBBBBBBBBBBBBBBBBBBBCCC /* Debug */ = {
            ...
            buildSettings = {
                ...
                INFOPLIST_FILE = "VisionOSCheeseTest/Info.plist";
                LD_RUNPATH_SEARCH_PATHS = (
                    "$(inherited)",
                    "@executable_path/Frameworks",
                );
                PRODUCT_BUNDLE_IDENTIFIER = "org.ggml.VisionOSCheeseTest";
...
if grep -q "No suitable application records were found" "${VALIDATION_OUTPUT}"; then
    ...
    if [ -d "${TEMP_DIR}/Payload/${APP_NAME}.app/Frameworks/cheese.framework" ]; then
        echo "✅ cheese.framework properly embedded"
    else
        echo "❌ cheese.framework not properly embedded"
        FINAL_VALIDATION_RESULT=1
    fi

    # Check if framework binary exists
    if [ -f "${TEMP_DIR}/Payload/${APP_NAME}.app/Frameworks/cheese.framework/cheese" ]; then
        echo "✅ Framework binary exists"

        # Further validate framework by checking architecture
        ARCHS=$(lipo -info "${TEMP_DIR}/Payload/${APP_NAME}.app/Frameworks/cheese.framework/cheese" 2>/dev/null | grep -o "arm64\\|x86_64" | tr '\n' ' ')
        if [ -n "$ARCHS" ]; then
