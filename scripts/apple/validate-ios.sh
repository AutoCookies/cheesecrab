#!/usr/bin/env bash
# validate-ios.sh - Validate iOS Application with embedded cheese.xcframework using SwiftUI
...
# Configuration
APP_NAME="iOSCheeseTest"
BUNDLE_ID="org.ggml.iOSCheeseTest"
...
cat >> "${TEMP_DIR}/${APP_NAME}/${APP_NAME}.xcodeproj/project.pbxproj" << 'EOF'
        ...
        EEEEEEEEEEEEEEEEEEEEEEEE = {
            isa = PBXGroup;
            children = (
                FFFFFFFFFFFFFFFFFFFFFFFF /* iOSCheeseTest */,
                CCCCCCCCCCCCCCCCCCCCCCCC /* Products */,
                DDDDDDDDDDDDDDDDDDDDDDDD /* Frameworks */,
            );
            sourceTree = "<group>";
...
/* Begin XCBuildConfiguration section */
        ...
        BBBBBBBBBBBBBBBBBBBBBBCCC /* Debug */ = {
            ...
            buildSettings = {
                ...
                INFOPLIST_FILE = "iOSCheeseTest/Info.plist";
                LD_RUNPATH_SEARCH_PATHS = (
                    "$(inherited)",
                    "@executable_path/Frameworks",
                );
                PRODUCT_BUNDLE_IDENTIFIER = "org.ggml.iOSCheeseTest";
...
if grep -q "No suitable application records were found" "${VALIDATION_OUTPUT}"; then
    ...
    if [ -f "${TEMP_DIR}/Payload/${APP_NAME}.app/Frameworks/cheese.framework/cheese" ]; then
        ...
        ARCHS=$(lipo -info "${TEMP_DIR}/Payload/${APP_NAME}.app/Frameworks/cheese.framework/cheese" 2>/dev/null | grep -o "arm64\\|armv7\\|x86_64" | tr '\n' ' ')
        if [ -n "$ARCHS" ]; then
