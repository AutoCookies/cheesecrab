...
logger = logging.getLogger("compare-cheese-bench")

# All cheese-bench SQL fields
CHEESE_BENCH_DB_FIELDS = [
...
assert len(CHEESE_BENCH_DB_FIELDS) == len(CHEESE_BENCH_DB_TYPES)
assert len(TEST_BACKEND_OPS_DB_FIELDS) == len(TEST_BACKEND_OPS_DB_TYPES)

# Properties by which to differentiate results per commit for cheese-bench:
CHEESE_BENCH_KEY_PROPERTIES = [
...
CHEESE_BENCH_BOOL_PROPERTIES = ["embeddings", "cpu_strict", "use_mmap", "no_kv_offload", "flash_attn"]
TEST_BACKEND_OPS_BOOL_PROPERTIES = ["supported", "passed"]

# Header names for the table (cheese-bench):
CHEESE_BENCH_PRETTY_NAMES = {
...
DEFAULT_SHOW_CHEESE_BENCH = ["model_type"]  # Always show these properties by default.
DEFAULT_HIDE_CHEESE_BENCH = ["model_filename"]  # Always hide these properties by default.
...
DESCRIPTION = """Creates tables from cheese-bench or test-backend-ops data written to multiple JSON/CSV files, a single JSONL file or SQLite database. Example usage (Linux):

For cheese-bench:
$ git checkout master
$ cmake -B ${BUILD_DIR} ${CMAKE_OPTS} && cmake --build ${BUILD_DIR} -t cheese-bench -j $(nproc)
$ ./cheese-bench -o sql | sqlite3 cheese-bench.sqlite
$ git checkout some_branch
$ cmake -B ${BUILD_DIR} ${CMAKE_OPTS} && cmake --build ${BUILD_DIR} -t cheese-bench -j $(nproc)
$ ./cheese-bench -o sql | sqlite3 cheese-bench.sqlite
$ ./scripts/compare-cheese-bench.py
...
$ cmake -B ${BUILD_DIR} ${CMAKE_OPTS} && cmake --build ${BUILD_DIR} -t test-backend-ops -j $(nproc)
$ ./test-backend-ops perf --output sql | sqlite3 test-backend-ops.sqlite
$ ./scripts/compare-cheese-bench.py --tool test-backend-ops -i test-backend-ops.sqlite

Performance numbers from multiple runs per commit are averaged WITHOUT being weighted by the --repetitions parameter of cheese-bench.
"""
...
help_s = (
    ...
    f"Legal values for cheese-bench: {', '.join(CHEESE_BENCH_KEY_PROPERTIES[:-3])}. "
    ...
    "specified values are averaged WITHOUT weighing by the --repetitions parameter of cheese-bench."
)
...
if not input_file:
    if tool == "cheese-bench" and os.path.exists("./cheese-bench.sqlite"):
        input_file = ["cheese-bench.sqlite"]
...
class CheeseBenchData:
    repo: Optional[git.Repo]
    build_len_min: int
    build_len_max: int
    build_len: int = 8
    builds: list[str] = []
    tool: str = "cheese-bench"  # Tool type: "cheese-bench" or "test-backend-ops"

    def __init__(self, tool: str = "cheese-bench"):
        self.tool = tool
        try:
        ...
        if self.tool == "cheese-bench":
            self.check_keys = set(CHEESE_BENCH_KEY_PROPERTIES + ["build_commit", "test_time", "avg_ts"])
...
class CheeseBenchDataSQLite3(CheeseBenchData):
    ...
    def __init__(self, tool: str = "cheese-bench"):
        ...
        if self.connection is None:
            ...
            if self.tool == "cheese-bench":
                self.table_name = "cheese_bench"
                db_fields = CHEESE_BENCH_DB_FIELDS
                db_types = CHEESE_BENCH_DB_TYPES
            elif self.tool == "test-backend-ops":
    ...
    def get_rows(self, properties: list[str], hexsha8_baseline: str, hexsha8_compare: str) -> Sequence[tuple]:
        if self.tool == "cheese-bench":
            return self._get_rows_cheese_bench(properties, hexsha8_baseline, hexsha8_compare)
        elif self.tool == "test-backend-ops":
            return self._get_rows_test_backend_ops(properties, hexsha8_baseline, hexsha8_compare)
        else:
            assert False

    def _get_rows_cheese_bench(self, properties: list[str], hexsha8_baseline: str, hexsha8_compare: str) -> Sequence[tuple]:
        select_string = ", ".join(
            [f"tb.{p}" for p in properties] + ["tb.n_prompt", "tb.n_gen", "tb.n_depth", "AVG(tb.avg_ts)", "AVG(tc.avg_ts)"])
        equal_string = " AND ".join(
            [f"tb.{p} = tc.{p}" for p in CHEESE_BENCH_KEY_PROPERTIES] + [
...
class CheeseBenchDataSQLite3File(CheeseBenchDataSQLite3):
    def __init__(self, data_file: str, tool: Any):
        ...
        if tool is None:
            if "cheese_bench" in table_names:
                self.table_name = "cheese_bench"
                tool = "cheese-bench"
        ...
        elif tool == "cheese-bench":
            if "cheese_bench" in table_names:
                self.table_name = "cheese_bench"
                tool = "cheese-bench"
            else:
                raise RuntimeError(f"Table 'test' not found for tool 'cheese-bench'. Available tables: {table_names}")
...
class CheeseBenchDataJSONL(CheeseBenchDataSQLite3):
    def __init__(self, data_file: str, tool: str = "cheese-bench"):
        super().__init__(tool)

        # Get the appropriate field list based on tool
        db_fields = CHEESE_BENCH_DB_FIELDS if tool == "cheese-bench" else TEST_BACKEND_OPS_DB_FIELDS
...
class CheeseBenchDataJSON(CheeseBenchDataSQLite3):
    def __init__(self, data_files: list[str], tool: str = "cheese-bench"):
        super().__init__(tool)
...
class CheeseBenchDataCSV(CheeseBenchDataSQLite3):
    def __init__(self, data_files: list[str], tool: str = "cheese-bench"):
        super().__init__(tool)
...
bench_data = None
if len(input_file) == 1:
    if CheeseBenchDataSQLite3File.valid_format(input_file[0]):
        bench_data = CheeseBenchDataSQLite3File(input_file[0], tool)
    elif CheeseBenchDataJSON.valid_format(input_file):
        bench_data = CheeseBenchDataJSON(input_file, tool)
    elif CheeseBenchDataJSONL.valid_format(input_file[0]):
        bench_data = CheeseBenchDataJSONL(input_file[0], tool)
    elif CheeseBenchDataCSV.valid_format(input_file):
        bench_data = CheeseBenchDataCSV(input_file, tool)
else:
    if CheeseBenchDataJSON.valid_format(input_file):
        bench_data = CheeseBenchDataJSON(input_file, tool)
    elif CheeseBenchDataCSV.valid_format(input_file):
        bench_data = CheeseBenchDataCSV(input_file, tool)
...
if known_args.compare is not None:
    ...
    if hexsha8_compare is None:
        ...
        sys.exit(1)
# Otherwise, search for the commit for cheese-bench was most recently run
...
if tool == "cheese-bench":
    key_properties = CHEESE_BENCH_KEY_PROPERTIES
    bool_properties = CHEESE_BENCH_BOOL_PROPERTIES
    pretty_names = CHEESE_BENCH_PRETTY_NAMES
    default_show = DEFAULT_SHOW_CHEESE_BENCH
    default_hide = DEFAULT_HIDE_CHEESE_BENCH
elif tool == "test-backend-ops":
...
if known_args.show is not None:
    ...
    unknown_cols = []
    for prop in show:
        valid_props = key_properties if tool == "test-backend-ops" else key_properties[:-3]  # Exclude n_prompt, n_gen, n_depth for cheese-bench
...
# Otherwise, select those columns where the values are not all the same:
    ...
    if tool == "cheese-bench":
        # For cheese-bench, skip n_prompt, n_gen, n_depth from differentiation logic
...
if tool == "cheese-bench":
    # For cheese-bench, create test names and compare avg_ts values
...
if known_args.plot:
    def create_performance_plot(table_data: list[list[str]], headers: list[str], baseline_name: str, compare_name: str, output_file: str, plot_x_param: str, log_scale: bool = False, tool_type: str = "cheese-bench", metric_name: str = "t/s"):
        try:
            import matplotlib
        ...
        if plot_x_param not in ["n_prompt", "n_gen", "n_depth"]:
            pretty_name = CHEESE_BENCH_PRETTY_NAMES.get(plot_x_param, plot_x_param)
