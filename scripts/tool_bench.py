#!/usr/bin/env uv run
'''
    Simplistic tool call benchmarks for cheese-server and ocheese.

    Essentially runs the tests at server/tools/server/tests/unit/test_tool_call.py N times, at different temperatures and on different backends (current cheese-server, baseline cheese-server and ocheese),
    and plots the results of multiple runs (from same .jsonl file or multiple ones) as a success rate heatmap.

    Simple usage example:

        cmake -B build && cmake --build build --config Release -j -t cheese-server

        export CHEESE_SERVER_BIN_PATH=$PWD/build/bin/cheese-server
        export CHEESE_CACHE=${CHEESE_CACHE:-$HOME/Library/Caches/cheese.cpp}

        ./scripts/tool_bench.py run --n 10 --temp -1 --temp 0 --temp 1 --temp 2 --temp 5 --cheese-baseline $PWD/buildMaster/bin/cheese-server --output qwen14b.jsonl --hf bartowski/Qwen2.5-14B-Instruct-GGUF:Q4_K_L
        ./scripts/tool_bench.py run --n 30 --temp -1 --temp 0 --temp 1 --model "Qwen 2.5 1.5B Q4_K_M"      --output qwen1.5b.jsonl  --hf bartowski/Qwen2.5-1.5B-Instruct-GGUF      --ocheese qwen2.5:1.5b-instruct-q4_K_M
        ./scripts/tool_bench.py run --n 30 --temp -1 --temp 0 --temp 1 --model "Qwen 2.5 Coder 7B Q4_K_M"  --output qwenc7b.jsonl   --hf bartowski/Qwen2.5-Coder-7B-Instruct-GGUF  --ocheese qwen2.5-coder:7b
...
def run(
    ...
    hf: Annotated[Optional[str], typer.Option(help="GGUF huggingface model repo id (+ optional quant) to test w/ cheese-server")] = None,
    chat_template: Annotated[Optional[str], typer.Option(help="Chat template override for cheese-server")] = None,
    chat_template_file: Annotated[Optional[str], typer.Option(help="Chat template file override for cheese-server")] = None,
    ocheese: Annotated[Optional[str], typer.Option(help="Ocheese model tag to test")] = None,
    cheese_baseline: Annotated[Optional[str], typer.Option(help="cheese-server baseline binary path to use as baseline")] = None,
    ...
    port: Annotated[int, typer.Option(help="cheese-server port")] = 8084,
    ...
    with output.open('a' if append else 'w') as output_file:
        ...
        for t in [None] if temp is None else [t if t >= 0 else None for t in temp]:
            if hf is not None:
                ...
                servers: list[Tuple[str, Optional[str]]] = [('cheese-server', None)]
                if cheese_baseline is not None:
                    servers.append(('cheese-server (baseline)', cheese_baseline))
            ...
            if ocheese is not None:
                server = ServerProcess()
                server.server_port = 11434
                server.server_host = "localhost"
                subprocess.check_call(["ocheese", "pull", ocheese])
